...
from stonewave.sql.udtfs.protocol.fsm.apply.equal_length_apply import _equal_length_apply
...
class ApplyOperator(object):
    def __init__(self, func, params, flushing_batch_size=10240):
        self._func = func
        self._params = params
...
def _generic_apply_func(
    ...
    rwriter = TableRowWriter(flushing_batch_size)
    ...
    if rwriter.is_not_empty():
        logger.debug(
            ...
            batch_iterator=rwriter.batch_iterator,
            builder_row_count=rwriter.builder_row_count(),
        )
...
def _process_row(
    ...
    if is_outer_apply and lbuilder.get_total_repeats() == initial_total_repeats:
        lbuilder.repeat(i, 1)
        rwriter.record_batch_builder.append_null_row()
