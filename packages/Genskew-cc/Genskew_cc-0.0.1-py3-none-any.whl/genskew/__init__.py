...
parser = argparse.ArgumentParser(
    description='This program calculates the nucleotide skew of a given genome. You can use it by calling python3 -m genskew and then using the arguments displayed here. Every Argument with * is required. If the '
                ...
                'locations by a space or input a folder name. Folders can only contain files of the chosen file type '
                'and you can only input one folder at a time. If multiple sequences are processed, specifying '
...
parser.add_argument('in_file_type', choices=["genbank", "gb", "gbk", "fasta"], metavar='FT', type=str, help='*the '
                                                                                                            'filetype')
...
inputfiles = gs.input_files(args.file_location, args.in_file_type)
# defining the sequence
sequence_list = []
for file in inputfiles:
    sequence1 = gs.gen_sequence(file, 'fasta')
