...
user imports like import tecton._internals.aws_helper instead of from
tecton._internals import aws_helper are not detected properly by the
underlying library we use for looking up dependencies. import json or other
top-level modeuls still works just fine, its just when you include multiple
path components that we don't detect the dependency properly
...
def _getsource(func):
    ...
    def process_functiontype(name, obj, imports, modules, code_lines, seen_args):
        ...
        if obj.__module__ in ("__main__", None) or getattr(obj, "_inlined", False) or "<" in obj.__qualname__:
            ...
            for dependency in sorted(objs.keys()):
                recurse(dependency, objs[dependency], imports, modules, code_lines, seen_args)
    ...
    def recurse(name, obj, imports, modules, code_lines, seen_args):
        ...
        if isinstance(obj, type):
            ...
            code_lines.append(f"{name} = StructType.fromJson(json.loads('{obj.json()}'))")
            modules.add("json")
            ...
            try:
                ...
                exec(repr_str)
                code_lines.append(repr_str)
            except Exception:
                raise Exception(f"Cannot evaluate object {obj} of type '{type(obj)}' for serialization")

    recurse(func.__name__, func, imports, modules, code_lines, seen_args)
