...
class FeaturePackageOrView(ABC):
    ...
    def _validate_and_return_use_materialized_data(self, use_materialized_data):
        ...
        if use_materialized_data and tc._is_overriding_datasources():
            logger.warning(
                "Calculating features from raw data source(s) since DataSourceOverrides are being used. "
                "This may result in slower feature computations."
            )
            use_materialized_data = False
        elif use_materialized_data and not (self.writes_to_offline_store and self.materialization_enabled):
            logger.warning(
            ...
            if self.is_in_materializable_workspace:
    ...
    @property
    def is_in_materializable_workspace(self):
        return _is_materializable_workspace(self.workspace)


def _is_materializable_workspace(workspace: str) -> bool:
    # This is a hacky duplication of sdk/tecton/_internals/workspace_utils.py#L5 since we can't use it
    # from tecton_spark
    return workspace == "" or workspace == "prod" or workspace.startswith("integration_testing_")
