...
class MaterializationPlan(object):
    offline_store_data_frame: DataFrame
    online_store_data_frame: DataFrame
    coalesce: int
...
def _get_batch_materialization_plan_for_aggregate_feature_view(
    ...
    return MaterializationPlan(offline_store_df, online_store_df, coalesce)
...
def _get_batch_materialization_plan_for_temporal_feature_view(
    ...
    offline_store_df, online_store_df = _materialize_interval_for_temporal_feature_view(
        spark,
        fpov,
        raw_data_time_limits,
        vds_proto_map,
    ...
    if feature_package_or_view and feature_package_or_view.is_feature_view:
        ...
        if feature_type:
            if feature_package_utils.is_temporal_aggregate(feature_type):
                ...
                pass
    return MaterializationPlan(None, spark_df, 0)
...
def get_feature_view_stream_materialization_plan(
    ...
    return MaterializationPlan(None, df, 0)
...
def get_materialization_schema(
    ...
    return schema.Schema.from_spark(
        get_batch_materialization_plan(
            spark=spark,
        ...
        ).online_store_data_frame.schema
    )
