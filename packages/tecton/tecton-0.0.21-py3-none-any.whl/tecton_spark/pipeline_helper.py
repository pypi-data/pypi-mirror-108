...
class _PipelineBuilder:
    ...
    _VALUE_TYPE = Union[DataFrame, _CONSTANT_TYPE]
    ...
    def __init__(
        ...
        self._raw_data_time_limits = raw_data_time_limits
    ...
    def _node_to_value(self, pipeline_node: PipelineNode) -> _VALUE_TYPE:
        ...
        elif pipeline_node.HasField("mock_data_node"):
            assert self._mock_inputs is not None
            return self._mock_inputs[pipeline_node.mock_data_node.input_name]
        elif pipeline_node.HasField("request_data_source_node") or pipeline_node.HasField("feature_view_node"):
    ...
    def _apply_transformation_function(self, transformation_node, args, kwargs) -> Union[DataFrame, pd.DataFrame]:
        ...
        if transformation.transformation_mode == TransformationMode.TRANSFORMATION_MODE_PYSPARK:
            ...
            raise KeyError(f"Unknown transformation mode: {transformation}")
        return res
    ...
    def _node_name(self, node) -> str:
        ...
        elif node.HasField("mock_data_node"):
            return node.mock_data_node.input_name
        else:
    ...
    def _get_time_window_from_data_source_node(self, data_source_node: DataSourceNode) -> Optional[pendulum.Period]:
        raw_data_limits = self._raw_data_time_limits
...
class _PandasPipelineBuilder(_PipelineBuilder):
    ...
    def _udf_node_to_value(self, pipeline_node: PipelineNode) -> pd.DataFrame:
