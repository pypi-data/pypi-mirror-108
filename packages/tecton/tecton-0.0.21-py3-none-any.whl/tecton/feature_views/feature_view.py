...
from tecton.feature_packages.feature_package_args import NewDatabricksClusterConfig
from tecton.feature_packages.feature_package_args import NewEMRClusterConfig
...
from tecton_proto.args.pipeline_pb2 import MockDataNode
...
from tecton_spark.pipeline_helper import FeatureDefinition
...
class OnDemandFeatureView(FeatureDefinition):
    ...
    def __init__(
        ...
        *,  # All arguments must be specified with keywords
        ...
        """
        Instantiates a new OnlineFeatureView.
    ...
    def run(self, **mock_inputs: pandas.DataFrame):
        ...
        if not self.inputs.keys() == mock_inputs.keys():
            raise FV_INVALID_MOCK_INPUTS(mock_inputs, self.inputs)

        inputs = _mock_inputs_to_mock_nodes(mock_inputs)
        mock_pipeline = Pipeline()
        mock_pipeline.root.CopyFrom(self.pipeline_function(**inputs))
        return run_mock_pandas_pipeline(
            mock_pipeline, [t._args for t in _GLOBAL_TRANSFORMATIONS_LIST], self.name, mock_inputs
        )
...
class MaterializedFeatureView(FeatureDefinition):
    def __init__(
        ...
        batch_materialization: Optional[Union[ExistingClusterConfig, NewDatabricksClusterConfig, NewEMRClusterConfig]],
    ...
    def run(self, spark: SparkSession, **mock_inputs: DataFrame):
        """
        Runs the feature view against passed in mock-data rather than the actual data sources.
        ...
        return pipeline_to_dataframe(
            spark, mock_pipeline, False, [], [t._args for t in _GLOBAL_TRANSFORMATIONS_LIST], None, None, mock_inputs
        )
    ...
    def _prepare_common_materialization_args(
        ...
        if batch_materialization:
            cluster_config = MaterializationConfig.cluster_config_to_proto(batch_materialization)
            args.batch_materialization.CopyFrom(cluster_config)
        if streaming_materialization:
            cluster_config = MaterializationConfig.cluster_config_to_proto(streaming_materialization)
...
def batch_feature_view(
    ...
    def decorator(user_function):
        ...
        featureView = MaterializedFeatureView(
            ...
            streaming_materialization=None,
...
def _mock_inputs_to_mock_nodes(inputs):
    kwargs = {}
    for ds_name, df in inputs.items():
        pipeline_node = PipelineNode()
        node = MockDataNode()
        node.input_name = ds_name
        pipeline_node.mock_data_node.CopyFrom(node)
...
def _test_binding_user_function(fn, inputs):
    ...
    try:
        ...
        raise TypeError(f"while binding inputs to pipeline function, {e}")
