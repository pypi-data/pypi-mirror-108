...
class TectonContext:
    ...
    def _set_datasource_overrides(self, overrides):
        assert len(overrides) == 0 or len(self._datasource_overrides) == 0
        self._datasource_overrides = overrides

    def _is_overriding_datasources(self):
        return len(self._datasource_overrides) > 0
    ...
    def _register_vds_temp_views_for_tables_in_sql(self, spine_sql: str, dsc_map: Dict[str, DataSourceConfig]):
        ...
        for table_name in table_names:
            ...
            self._register_temp_view_for_data_source(
                dsc.proto, dsc.virtual_data_source._proto, register_stream=False, allow_overrides=True
            )
    ...
    def _register_temp_views_for_feature_package_or_view(
        ...
        if feature_package_or_view.is_feature_view:
            self._register_temp_views_for_feature_view(
                feature_package_or_view, register_stream, allow_overrides, raw_data_time_limits
            )
        else:
            self._register_temp_views_for_feature_package(
                feature_package_or_view.fp, register_stream, allow_overrides, raw_data_time_limits
            )
    ...
    def _register_temp_view_for_data_source(
        ...
        # Don't allow validation operations to be done within a DataSourceOverride block
        if not allow_overrides:
            assert len(self._datasource_overrides) == 0

        name = virtual_data_source_proto.fco_metadata.name
        override_df = None
        if name in self._datasource_overrides:
            logger.debug(f"Overriding VDS {name}")
            override = self._datasource_overrides[name]
            if isinstance(override, DataFrame):
                override_df = override
            else:
                assert False, f"Error: Unsupported type for DataSourceOverride"
