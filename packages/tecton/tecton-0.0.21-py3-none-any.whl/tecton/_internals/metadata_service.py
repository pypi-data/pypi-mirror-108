...
def is_initialized():
    return _stub_instance is not None
...
def _get_host_port():
    return conf.get("METADATA_SERVICE")
...
def _init_stub_instance():
    ...
    conf._init_metadata_server_config()
...
class MetadataServiceInterceptor(grpc.UnaryUnaryClientInterceptor, grpc.StreamUnaryClientInterceptor):
    ...
    def _intercept_call(continuation, client_call_details, request_or_iterator):
        ...
        workspace = conf.get("TECTON_WORKSPACE")
        if workspace:
        ...
        if not token:
            token = conf.get("TECTON_API_KEY")
            authorization = f"Tecton-key {token}"
        # UNDONE: some databricks environments will still be using tecton bearer token until it's replaced
        if not token:
            token = conf.get("TECTON_BEARER_TOKEN")
            authorization = f"Bearer {token}"
