...
INTERNAL_ERROR = lambda message: TectonInternalError(
    f"Internal Tecton error, please contact support with error details: {message}"
)
...
UNSUPPORTED_OPERATION_IN_NON_PROD_WORKSPACE = lambda op: TectonValidationError(
    f"Operation '{op}' is not supported outside of the 'prod' workspace"
)
...
FP_GET_FEATURES_MATERIALIZATION_DISABLED = TectonValidationError(
    ...
    "or alternatively configure offline materialization for this FeaturePackage in the 'prod' Workspace."
)
...
FD_GET_FEATURES_FROM_DISABLED_MATERIALIZATION = lambda fd_name, workspace: TectonValidationError(
    f"Feature Definition {fd_name} in workspace {workspace} has materialization disabled. "
    "Please use from_source=True when getting features."
)

FD_GET_MATERIALIZED_FEATURES_FROM_NONMATERIALIZABLE_WORKSPACE = lambda fd_name, workspace: TectonValidationError(
    f"Feature Definition {fd_name} is in workspace {workspace}, which does not have materialization enabled. "
    "Please use from_source=True when getting features."
)
...
def OVERRIDE_VDS_NOT_VALID(vds_name: str, all_vds_names: List[str]):
    vds_list = ", ".join(all_vds_names)
    return TectonValidationError(
        f"VirtualDataSource '{vds_name}' referenced in DataSourceOverride was not found in registered data sources: [{vds_list}]"
    )


def OVERRIDE_INVALID_TYPE(typ: str, allowed_types: List[str]):
    return TectonValidationError(
        f"Unexpected DataSourceOverrides type {typ}. Expected one of " + ", ".join(allowed_types)
    )
...
def FS_BACKEND_ERROR(message):
    return TectonValidationError(f"Error calling Feature Service backend: {message}")
...
def FV_INVALID_MOCK_INPUTS(mock_inputs, inputs):
    return TectonValidationError(
        f"Mock input names {mock_inputs.keys()} do not match FeatureView's inputs {inputs.keys()}"
    )
...
class InvalidTransformInvocation(TectonValidationError):
    def __init__(self, transformation_name: str, got: str):
        super().__init__(
            ...
            f"Tecton Constant and a dataframe generated from a Transformation/VirtualDataSource. Got: '{got}'"
        )


# Dataset
DATASET_NAME_PROVIDED_WHEN_SAVE_FALSE = TectonValidationError("Param save_as cannot be set if save=False")
