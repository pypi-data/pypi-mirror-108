...
def get_online_features(
    features: Union[FeatureService, List[FeaturePackage]],
...
def _infer_timestamp(spine: Union[pysparkDF, pd.DataFrame]) -> Optional[str]:
...
def get_historical_features(
    features: Union[List[FeaturePackage], FeatureService],
    ...
    if not timestamp_key:
        timestamp_key = _infer_timestamp(spine)
    ...
    if isinstance(features, FeatureService):
        ...
        for feature in features:
            if not isinstance(feature, FeaturePackage):
                raise TectonValidationError(
                    "The `features` parameter can only be " "a FeatureService, or a List of FeaturePackages"
                )

        feature_set_config = FeatureSetConfig()
        for feature in features:
            feature_set_config._add(feature)
...
def get_features(
    ...
    entities: Union[pyspark.sql.dataframe.DataFrame, pd.DataFrame, None] = None,
    ...
    if not from_source and not feature_package_or_view.is_in_materializable_workspace:
        raise internal_errors.FD_GET_MATERIALIZED_FEATURES_FROM_NONMATERIALIZABLE_WORKSPACE(
