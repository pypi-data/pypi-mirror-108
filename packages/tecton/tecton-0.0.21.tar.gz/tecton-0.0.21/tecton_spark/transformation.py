...
class Transformation:
    ...
    def _get_data_source_configs(self, config_map: Dict[str, Tuple[DataSourceConfigProto, VirtualDataSourceProto]]):
        ...
        """
        from tecton import DataSourceConfig
        from tecton.interactive import VirtualDataSource as VirtualDataSourceInteractive

        if self.vds_pb2_map is not None:
            for vds_pb2 in self.vds_pb2_map.values():
                if vds_pb2.fco_metadata.name not in config_map:
                    vds = VirtualDataSourceInteractive._create_from_proto(vds_pb2)
                    data_source_config_proto = DataSourceConfig(vds).proto
                    config_map[vds_pb2.fco_metadata.name] = (data_source_config_proto, vds_pb2)
    ...
    def dataframe(
        ...
        data_source_overrides: Optional[Dict[str, DataFrame]] = None,
        ...
        :param data_source_overrides: Data source overrides to apply, keyed by source name

        :return: Spark DataFrame.
        """
        config_map: Dict[str, Tuple[DataSourceConfigProto, VirtualDataSourceProto]] = {}
        self._get_data_source_configs(config_map)
        data_source_overrides = data_source_overrides or {}
        for dsc, vds in config_map.values():
            name = vds.fco_metadata.name
            if name in data_source_overrides:
                override_df = data_source_overrides[name]
            else:
                override_df = None
            data_source_helper.register_temp_view_for_data_source(
                spark,
