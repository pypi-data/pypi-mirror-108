...
def get_offline_store_writer(
    params: OfflineStoreParams, package_config: args_proto.OfflineFeatureStoreConfig, version: int
):
...
class ParquetWriter(OfflineStoreWriter):
    ...
    def __init__(self, params: OfflineStoreParams):
        self._params = params

    def append_dataframe(self, data_frame):
        if self._params.is_continuous:
            aligned_time = _align_timestamp(functions.col(ANCHOR_TIME), functions.lit(86400))
            data_frame = data_frame.withColumn(TIME_PARTITION, aligned_time)
            partition_col = TIME_PARTITION
        else:
            partition_col = ANCHOR_TIME
        data_frame.write.partitionBy(partition_col).parquet(self._params.s3_path, mode="append")
...
def _align_timestamp(int_timestamp_col, window_size_seconds):
    return int_timestamp_col - (int_timestamp_col % window_size_seconds)
