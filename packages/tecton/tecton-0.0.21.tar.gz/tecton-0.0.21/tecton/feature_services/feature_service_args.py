...
class FeaturesConfig(object):
    ...
    By default, you can add all of the features in a FeaturePackage to a FeatureService by passing
    the FeaturePackage into the ``features`` parameter of a FeatureService. However, if you want to specify a subset.

    You can use the double-bracket notation ``my_feature_package[[<features>]]``
    as a short-hand for generating a FeaturesConfig from a FeaturePackage. This is the preferred way to select a subset of
    of the features contained in a FeaturePackage. As an example:

    .. highlight:: python
    .. code-block:: python

       from tecton import FeatureService
       from feature_repo.features import my_feature_package_1, my_feature_package_2

       my_feature_service = FeatureService(
           name='my_feature_service',
           features=[
               # Add all features from my_feature_package_1 to this FeatureService
               my_feature_package_1,
               # Add a single feature from my_feature_package_2, 'my_feature'
               my_feature_package_2[['my_feature']]
           ]
       )

    :param package: The Feature Package.
    :param features: The subset of features to select from the Feature Package.
    :param namespace (optional): A namespace used to prefix the features joined from this FeaturePackage.
        By default, namespace is set to the FeaturePackage name.
    :param override_join_keys: (advanced) map of spine join key to feature package join key overrides.
    """
