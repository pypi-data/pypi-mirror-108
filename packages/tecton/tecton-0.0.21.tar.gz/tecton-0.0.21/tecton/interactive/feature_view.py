...
class FeatureView(FeatureDefinition):
    ...
    def _validate_and_return_use_materialized_data(self, use_materialized_data):
        ...
        elif use_materialized_data and not self._writes_to_offline_feature_store:
            logger.warning(
                "Calculating features from raw data source(s) since materialization to offline feature store is not enabled. "
                "This may result in slow feature computations."
            )
            if self._proto.fco_metadata.workspace == "":
    ...
    def get_features(
        ...
        """
        Gets all features that are defined by this Feature View.
