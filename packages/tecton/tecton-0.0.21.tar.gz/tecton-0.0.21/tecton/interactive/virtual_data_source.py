...
class VirtualDataSource(Fco):
    ...
    virtual_ds: Optional[VirtualDataSourceProto] = None
    ...
    def is_streaming(self) -> bool:
        """
        Whether or not the VirtualDataSource contains a stream source or not.
        """
    ...
    def start_stream_preview(self, table_name: str) -> StreamingQuery:
        ...
        if not self.is_streaming:
            raise ValueError("start_stream_preview called on non-streaming data source")
...
@sdk_public_method
def get_virtual_data_source(name) -> VirtualDataSource:
    """
...
@sdk_public_method
def get_data_source(name) -> Union[BatchDataSource, StreamDataSource]:
    """
