...
def _get_time_limits_of_dataframe(df: DataFrame, time_key: str) -> Optional[pendulum.Period]:
    ...
    return pendulum.instance(time_end, tz=tz).add(microseconds=1) - pendulum.instance(time_start, tz=tz)
...
def _get_feature_dataframe_with_limits(
    ...
    if validate_time_key:
        if fpov.is_online:
            assert isinstance(spine_df, DataFrame), "Spine must a DataFrame for OnlineFeaturePackages"
        else:
            if fpov.is_temporal_aggregate:
    ...
    try:
        if fpov.is_temporal_aggregate:
            ...
            if fpov.is_feature_view:
                ...
                spark_df = fpov.final_transformation.online_transform_dataframe(
                    input_df=spine_df, join_keys=fpov.join_keys
                )
