...
def _align_feature_data_time_limits(
    ...
    time_end = convert_epoch_to_datetime(materialization_params.align_timestamp_left(time_end), version)
    return time_start, time_end
...
def construct_full_tfp_or_push_df(
    ...
    # Next, we run the "last" window function with (-infinity, 0) window while partitioning the data on fp.join_keys.
