...
class TrendReq(object):
    ...
    def __init__(self, hl='en-US', tz=300, geo='CO', timeout=(2, 5), proxies='',
    ...
    def interest_over_time(self):
        ...
        # print(req_json['default']['timelineData'])
    ...
    def trending_searches_last_days(self, pn='CO'):
        """Request data from Google last tree days"""
        forms = {'ns': 15, 'geo': pn, 'tz': '-180', 'hl': 'en-US'}
        req_json = self._get_data(
            url=TrendReq.TODAY_SEARCHES_URL,
            method=TrendReq.GET_METHOD,
        ...
        trending_json = []
        for trending_day in req_json:
            date = trending_day['date'][:4] + '-' + trending_day['date'][4:6] + '-' + trending_day['date'][6:8]
            for trending in trending_day['trendingSearches']:
                trending_info = {}
                trending_info['date'] = date
                trending_info['title'] = trending['title']['query']
                trending_info['formattedTraffic'] = trending['formattedTraffic']
                trending_info['articles'] = trending['articles']
                trending_json.append(trending_info)
        return json.dumps(trending_json, ensure_ascii=False)

    def trending_searches_date(self, pn='CO', date=None, hl='es', tz='300'):
        """Request data from Google Daily Trends section and returns a dataframe"""
        today = datetime.now()
        today = today.strftime("%Y%m%d") #google accepts YYYYMMDD
        if date:
            ## date in args YYYY-MM-DD, remove '-' for request google accepts YYYYMMDD
            date_trend = date.replace('-','')
        else:
            date_trend = today
        forms = {'ns': 15, 'geo': pn, 'tz': tz, 'hl': hl, 'ed': date_trend}
        req_json = self._get_data(
            url=TrendReq.TODAY_SEARCHES_URL,
            method=TrendReq.GET_METHOD,
            trim_chars=5,
        ...
        trending_json = []
        for trending_day in req_json:
            date_t = trending_day['date'][:4] + '-' + trending_day['date'][4:6] + '-' + trending_day['date'][6:8]
            if date_t != date:
                continue
            for trending in trending_day['trendingSearches']:
                trending_info = {}
                trending_info['date'] = date_t
                trending_info['title'] = trending['title']['query']
                trending_info['formattedTraffic'] = trending['formattedTraffic']
                trending_info['articles'] = trending['articles']
                trending_info['country'] = pn
                trending_info['numberTraffic'] = re.findall('\d+', trending_info['formattedTraffic'] )[0]
                if 'k' in trending_info['formattedTraffic'] or 'K' in trending_info['formattedTraffic']:
                    trending_info['numberTraffic'] = int(trending_info['numberTraffic']) * 1000
                if 'm' in trending_info['formattedTraffic'] or 'M' in trending_info['formattedTraffic']:
                    trending_info['numberTraffic'] = int(trending_info['numberTraffic']) * 1000000
                trending_json.append(trending_info)
        return json.dumps(trending_json, ensure_ascii=False).replace(u'\xa0', u' ')
    ...
    def suggestions(self, keyword):
        ...
        temp_list = list(map(lambda x: {i:x[i] for i in x if i!='mid'} ,req_json))
        return json.dumps(temp_list,ensure_ascii=False)
