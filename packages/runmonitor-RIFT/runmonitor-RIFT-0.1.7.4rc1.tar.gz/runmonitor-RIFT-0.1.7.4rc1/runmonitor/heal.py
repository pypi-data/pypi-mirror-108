...
def identify_slot(process_id,rd=None):
	odir = os.getcwd()
	if rd == None:
		rd = odir
	os.chdir(rd)
	cmd = f"find -name *{process_id}*"
	pipe = subprocess.Popen(cmd,stdout=subprocess.PIPE,shell=True)
	text = pipe.communicate()
	print(text)
	text = str(text[0].strip()).split('\\n')[1]
	fpath = os.path.join(rd,text)
...
def check_encodings_error(rd=None,check_node=False):
	...
	for i,line in enumerate(error_list):
		...
		if "Node return val: -6" in line:
			...
			failid = idline.split("(")[1].split(")")[0].split(".")[0] # hacky string parsing, assuming no other parentheses
			failids += [failid]
...
def identify_crasher_fail(rd=None):
	if rd == None:
		rd = os.getcwd()
	...
	for i,line in enumerate(error_list):
		...
		if "Node return val: 9" in line:
			...
			idline = error_list[i+1]
			failid = idline.split("(")[1].split(")")[0].split(".")[0] # hacky string parsing, assuming no other parentheses
			crasher_fail_ids += [failid]

	for failid in crasher_fail_ids:
		failslot = identify_slot(failid)# see above
...
def find_max_extr_possible(rd=None):
	...
	cmd = "ls | grep 'ile' | sort -n -t '_' -k2 | tail -n2 | head -n1"
...
def detect_and_correct_catjob(rd=None, thres_frac = 1. / 40.):
	...
	if num_fails < threshold:
		os.chdir(rd)
		cmd = "./catjob.sh"
		catproc = subprocess.Popen(cmd,stdout=PIPE,stderr=PIPE,env=envinfo)
