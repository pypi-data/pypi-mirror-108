...
class TestResult(unittest.TestResult):
    ...
    def addFailure(self, test, err):
        ...
        test.state = '失败'
        test.warning_log("{}执行——>【失败】\n".format(test.__doc__ or test))
        test.info_log("当前运行环境全局变量有：\n{}\n".format(json.dumps(ENV, ensure_ascii=False, indent=2)))
        test.debug_log("当前运行环境局部变量有：\n{}\n".format(json.dumps(getattr(test, 'env'), ensure_ascii=False, indent=2)))
    ...
    def addError(self, test, err):
        ...
        test.state = '错误'
        test.error_log("{}执行——>【错误Error】\n".format(test.__doc__ or test))
        test.error_log('\n' + "".join(traceback.format_exception(*err)))
        test.debug_log("当前运行环境全局变量有：\n{}\n".format(json.dumps(ENV, ensure_ascii=False, indent=2)))
        test.debug_log("当前运行环境局部变量有：\n{}\n".format(json.dumps(getattr(test, 'env'), ensure_ascii=False, indent=2)))
        test.exception_Log(err)
...
class ReRunResult(TestResult):
    ...
    def addFailure(self, test, err):
        ...
        if test.count < self.count:
            test.count += 1
            test.error_log("{}执行——>【失败Failure】\n".format(test))
            test.exception_log(err)
            test.error_log("================{}重运行第{}次================\n".format(test, test.count))
            time.sleep(self.interval)
            test.run(self)
        else:
            super().addFailure(test, err)
            if test.count != 0:
                test.debug_log("================重运行{}次完毕================\n".format(test.count))
