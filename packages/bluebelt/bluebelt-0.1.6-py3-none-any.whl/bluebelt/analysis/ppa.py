...
from bluebelt.analysis.subgroups import get_subgroups
...
class Summary():
    ...
    def plot(self, **kwargs):
        ...
        ax2_xticks = bluebelt.helpers.boxplot.get_boxplot_quantiles(self.series, whis=1.5)
        ax2.set_xticks(ax2_xticks)
        ax2.xaxis.set_major_formatter(FormatStrFormatter(f'%{bluebelt.helpers.text.format_digits(ax2_xticks, digits=3)}'))
        ...
        ax3.xaxis.set_major_formatter(FormatStrFormatter(f'%{bluebelt.helpers.text.format_digits(ax3_xticks, digits=3)}'))
        ...
        ax4.xaxis.set_major_formatter(FormatStrFormatter(f'%{bluebelt.helpers.text.format_digits(ax4_xticks, digits=3)}'))
        ...
        for index, value in self.series.iteritems():
            ...
            if index == self.series.index[0]: # set the first value
                ...
                if run_up_or_down_length > longest_run_up_or_down:
                    ...
                    longest_runs_up_or_down = [self.series.loc[:index].iloc[-(longest_run_up_or_down+1):-1]]
...
class ProcessCapability():
    ...
    def calculate(self):
        ...
        self.mean = self.series.mean()
        self.std = self.series.std()
        self.subgroups = get_subgroups(self.series, subgroups=self.subgroups, subgroup_size=self.subgroup_size)
        self.std_within = bluebelt.statistics.std.std_within(self.subgroups)
    ...
    def get_expected_lt_lsl_within(self):
        return (1 - stats.norm.cdf((self.mean - self._lsl) / self.std_within, loc=0, scale=1)) * 1000000 

    def get_expected_gt_usl_within(self):
        return (1 - stats.norm.cdf((self._usl - self.mean) / self.std_within, loc=0, scale=1)) * 1000000 
    ...
    def get_expected_lt_lsl_overall(self):
        return (1 - stats.norm.cdf((self.mean - self._lsl) / self.std, loc=0, scale=1)) * 1000000 

    def get_expected_gt_usl_overall(self):
        return (1 - stats.norm.cdf((self._usl - self.mean) / self.std, loc=0, scale=1)) * 1000000 
    ...
    def get_cpm(self):
        ...
        # get subgroups
        subgroups = get_subgroups(self.series, subgroup_size=5)
    ...
    def __str__(self):
        ...
        # process data
        digits = f'{bluebelt.helpers.text.format_digits([self.lsl, self.usl, self.target, self.mean, self.std_within, self.std], 6)}'

        str_target = f'{"target":{fill}}{self.target}'
        str_lsl = f'{"LB" if self.lb else "LSL":{fill}}{self.lsl:{digits}}' 
        str_usl = f'{"UB" if self.ub else "USL":{fill}}{self.usl:{digits}}'
        ...
        # within capability
        str_cp = f'{"Cp":{fill}}{self.cp:1.2f}'
        str_cpl = f'{"Cpl":{fill}}{self.cpl:1.2f}'
        str_cpu = f'{"Cpu":{fill}}{self.cpu:1.2f}'
        str_cpk = f'{"Cpk":{fill}}{self.cpk:1.2f}'
        str_ccpk = f'{"CCpk":{fill}}{self.ccpk:1.2f}'

        # overall capability
        str_pp = f'{"Pp":{fill}}{self.pp:1.2f}'
        str_ppl = f'{"Ppl":{fill}}{self.ppl:1.2f}'
        str_ppu = f'{"Ppu":{fill}}{self.ppu:1.2f}'
        str_ppk = f'{"Ppk":{fill}}{self.ppk:1.2f}'
        str_cpm = f'{"Cpm":{fill}}{self.cpm:1.2f}'
        ...
        str_expected_gt_usl_within = f'{"PPM > USL":{fill}}{self.expected_lt_lsl_within:1.0f}'
