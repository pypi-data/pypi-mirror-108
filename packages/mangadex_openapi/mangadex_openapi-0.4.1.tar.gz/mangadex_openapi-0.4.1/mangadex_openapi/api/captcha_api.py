...
class CaptchaApi(object):
    ...
    def post_captcha_solve(self, **kwargs):  # noqa: E501
        """Solve Captcha  # noqa: E501

        Captchas can be solved explicitly through this endpoint, another way is to add a `X-Captcha-Result` header to any request. The same logic will verify the captcha and is probably more convenient because it takes one less request.  Authentication is optional. Captchas are tracked for both the client ip and for the user id, if you are logged in you want to send your session token but that is not required.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.post_captcha_solve(async_req=True)
        >>> result = thread.get()
        ...
        if kwargs.get("async_req"):
            return self.post_captcha_solve_with_http_info(**kwargs)  # noqa: E501
        else:
            (data) = self.post_captcha_solve_with_http_info(**kwargs)  # noqa: E501
            return data

    def post_captcha_solve_with_http_info(self, **kwargs):  # noqa: E501
        """Solve Captcha  # noqa: E501

        Captchas can be solved explicitly through this endpoint, another way is to add a `X-Captcha-Result` header to any request. The same logic will verify the captcha and is probably more convenient because it takes one less request.  Authentication is optional. Captchas are tracked for both the client ip and for the user id, if you are logged in you want to send your session token but that is not required.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.post_captcha_solve_with_http_info(async_req=True)
        >>> result = thread.get()
        ...
        for key, val in six.iteritems(params["kwargs"]):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method post_captcha_solve" % key
                )
            params[key] = val
        del params["kwargs"]
