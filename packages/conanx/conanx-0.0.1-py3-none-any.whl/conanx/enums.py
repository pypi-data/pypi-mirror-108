...
# url:  https://gitlab.freedesktop.org/gstreamer/cerbero/enums.py
#  
# ------------------------------------------------------------
...
from conanx.errors import FatalError
...
# Exit codes for conanx command:
class ExitCode:
    SUCCESS = 0                           # 0: Success (done)
    ERROR_GENERAL = 1                     # 1: General ConanXException error (done)
    ERROR_MIGRATION = 2                   # 2: Migration error
    USER_CTRL_C = 3                       # 3: Ctrl+C
    USER_CTRL_BREAK = 4                   # 4: Ctrl+Break
    ERROR_SIGTERM = 5                     # 5: SIGTERM
    ERROR_CONFIGURATION = 6               # 6: Invalid configuration ConanX ConfigError(done)
    ERROR_CONAN_GENERAL = 7                       # 7: General conan ConanException error
    ERROR_CONAN_INVALID_CONFIGURATION = 8 # 8: Invalid configuration in Conan ConfigError(done)
