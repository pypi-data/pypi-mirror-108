...
def atomlistToChannels(atomNames, hashing="TPL_Hashing", device="cpu"):
	"""
	...
	hashing : "TPL_Hashing" or "Element_Hashing" or dict
		...
		TPL_Hashing: uses the hashing of torch protein library
		Element_Hashing: groups atoms in accordnce with the element only: C -> 0, N -> 1, O ->2, everything else -> 3
		...
		The default encoding is the one defined in Torch Protein Library (https://github.com/lupoglaz/TorchProteinLibrary)
	...
	if hashing == "TPL_Hashing":
		...
		hashing = hashings.contracted_hashing
	else:
