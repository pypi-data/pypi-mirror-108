...
class MLtasksCls(object):
    ...
    def estAccMLmodel(cls, locargs, shutup=False):
        if args.MLprog.lower() in cls.mlatom_alias and not args.optlines:
            t_train, t_pred, results = interface_MLatomF.ifMLatomCls.run(locargs)
        else:
            cls.prepareData(locargs)
    ...
    def useMLmodel(cls, locargs, shutup=False):
        ...
        rmsedict = {}
        if args.MLprog.lower() in cls.mlatom_alias:
            _, wallclock, rmsedict = interface_MLatomF.ifMLatomCls.run(locargs,shutup=shutup)
        else:
            cls.prepareData(locargs)
    ...
    @classmethod
    def training(cls, locargs, setname=''):
        if args.MLprog.lower() in cls.mlatom_alias:
            ...
            with open('%si%s.dat'%(cls.index_prefix, setname),'r')as f:
                ntrain=0
                for line in f:
                    ntrain+=1
            ...
            mlatom_arg=addReplaceArg('itrainin','itrainin=%si%s.dat'%(cls.index_prefix, setname),mlatom_arg)
            ...
            wallclock, _, _ = interface_MLatomF.ifMLatomCls.run(mlatom_arg,shutup=False)
            print(' \n .............................................\n \n\n\tTraining Time: \t\t\t\t%f s\n' % wallclock)
        else:
            print(' Training started\n .............................................')
            starttime = time.time()
            if args.MLprog.lower() in cls.deepmd_alias:
                wallclock = interface_DeePMDkit.DeePMDCls.createMLmodel(locargs, cls.subdatasets)
            elif args.MLprog.lower() in cls.gap_alias:
                wallclock = interface_GAP.GAPCls.createMLmodel(locargs, cls.subdatasets)
            elif args.MLprog.lower() in cls.ani_alias:
                wallclock = interface_TorchANI.ANICls.createMLmodel(locargs, cls.subdatasets)
            elif args.MLprog.lower() in cls.physnet_alias:
                wallclock = interface_PhysNet.PhysNetCls.createMLmodel(locargs, cls.subdatasets)
            elif args.MLprog.lower() in cls.sgdml_alias:
                wallclock = interface_sGDML.sGDMLCls.createMLmodel(locargs, cls.subdatasets)
            endtime = time.time()
            wallclock = endtime - starttime
            print(' .............................................\n Training finished\n\n\tTraining Time: \t\t\t\t%f s\n' % wallclock)
            sys.stdout.flush()
    ...
    @classmethod
    def optraining(cls, locargs, shutup=False):
        ...
        def getloss(opt): # function that to plug in hyperopt.fmin later
            ...
            _, rmsedict, report = cls.estimate(optargs, 'validate', shutup=True)
            rmses = list(rmsedict.values())
            if len(rmses)>1:
                if args.hyperopt.losstype.lower() == 'geomean': loss = np.sqrt(np.prod(rmses))
                elif args.hyperopt.losstype.lower() == 'weighted': loss = rmses[0] + args.hyperopt.w_grad*rmses[1]
                else: 
            ...
            if loss < bestsofar:
                bestsofar = loss
                bestreport = report
                os.system('for file in `ls %s* `;do mv $file _optmp_${file}; done > /dev/null 2>&1' % args.mlmodelout)
                os.system('for file in `ls *est.dat_validate`;do mv $file ${file}_optmp_; done > /dev/null 2>&1')# move mlmodel to tmp
            print('\tLoss:\t\t\t\t\t%s\n\n' % loss)
            return loss
        ##############################################################################################
        starttime = time.time()
        ...
        if args.hyperopt.algorithm.lower() == 'tpe':
            bestpara=hyperopt.fmin(fn=getloss, space=optspace, algo=hyperopt.tpe.suggest, max_evals=args.hyperopt.max_evals)
        ...
        os.system('for file in `ls *_optmp_*`; do mv $file `echo $file |sed "s/_optmp_//"`; done')
    ...
    @classmethod
    def estimate(cls, locargs, setname, shutup=False):
        if args.MLprog.lower() in cls.mlatom_alias and args.optlines:
            ...
            with open('%si%s.dat'%(cls.index_prefix, setdict[setname][0]),'r')as f:
                ntrain=0
                for line in f:
                    ntrain+=1
            ...
            useargs=addReplaceArg('createmlmodel','estaccmlmodel',locargs)
            ...
            useargs=addReplaceArg('itrainin','itrainin=%si%s.dat'%(cls.index_prefix, setdict[setname][2]),useargs)
            useargs=addReplaceArg('itestin','itestin=%si%s.dat'%(cls.index_prefix, setname),useargs)
            ...
            if args.Yfile or True:    useargs = addReplaceArg('YestFile', 'YestFile=yest.dat_'+setname, useargs)
            if args.YgradXYZfile or True: useargs = addReplaceArg('YgradXYZestFile', 'YgradXYZestFile=gradest.dat_'+setname, useargs)
    ...
    def prepareData(cls, locargs):
        ...
        if args.MLprog in cls.model_needs_subvalid or args.optlines: 
    ...
    @classmethod
    def splitDataset(cls, issizeone, indices, filein, fileout):
        ...
        with open(filein,'r') as fi, open(fileout,'w') as fo:
            if issizeone:
                ...
                for line in fi:
                    ...
                    if i in idx: fo.write(line+'\n')
                    for j in range(limit):
                            if i in idx: fo.write(fi.readline())
                            else: fi.readline()
