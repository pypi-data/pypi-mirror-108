...
RE_JROSUBDIR = r"(?P<optchar>[dDpP]{1})(?P<year>[0-9]{4})(?P<doy>[0-9]{3})"
...
class FileHandler(object):
    ...
    def __init__(self, path, file_pattern, folder_pattern=None, startDate=None, endDate=None, expLabel='', online=True):
        """
        """

        if folder_pattern is None:
            self.re_folder = False
        else:
            self.re_folder = re.compile("^" + folder_pattern + "$")

        self.re_file = re.compile("^" + file_pattern + "$")
        self.path = path
    ...
    def check_date(self, pattern, filename):
        """
        """

        regex = pattern.match(filename)
        if regex:
            self.date_info = regex.groupdict()
            if 'doy' in self.date_info:
                dt = datetime.datetime(int(self.date_info['year']), 1, 1) + datetime.timedelta(int(self.date_info['doy'])-1)
            elif 'timestamp' in self.date_info:
                dt = datetime.datetime(1970, 1, 1) + datetime.timedelta(seconds=int(self.date_info['timestamp']))
            else:
                dt = datetime.datetime(int(self.date_info['year']), int(self.date_info['month']), int(self.date_info['day']))
    ...
    def __iter__(self):
        ...
        if self.re_folder:
            self.folders = [f for f in os.listdir(self.path) if os.path.isdir(os.path.join(self.path, f)) and self.re_folder.match(f)]
            self.folders.sort()
            if self.online:
        ...
        for folder in self.folders:
            if folder:
                if not self.check_date(self.re_folder, folder):
                    log.log('Skiping folder {}'.format(folder), 'Reading')
                    continue
            files = [f for f in os.listdir(os.path.join(self.path, folder, self.expLabel)) if self.re_file.match(f)]
            files.sort()
            if self.online:
                yield os.path.join(self.path, folder, self.expLabel, files[-1])
            else:
                for filename in files:
                    if self.check_date(self.re_file, filename):
                        yield os.path.join(self.path, folder, self.expLabel, filename)
                    else:
                        log.log('Skiping file {}'.format(filename), 'Reading')
                        continue
...
class Reader(object):
    ...
    def find_files(self, folders, ext, filefmt, startDate=None, endDate=None, 
        ...
        for path in folders:            
            files = glob.glob1(os.path.join(path, expLabel), '*{}'.format(ext))
            files.sort()
            if last:
                if files:                    
                    fo = files[-1]
