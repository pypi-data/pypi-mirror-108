...
def sql_create_update_case_statement(df, layer, df_id_col, db_id_col, new_val_col, excluded_ids=[],
                                 old_val_col=None, old_col_name=None, show_prev=False, show_proposed=False) -> str:
    """
    ...
    if show_proposed and show_prev:
        raise Exception("sql_create_update_case_statement: "
    ...
    try:
        ...
        if e.args[0] == 'The specified module could not be found.\r\n':
            if os.path.exists(MOD_SPATIALITE_PATH):
                ...
                print("""Download mod_spatialite extension from http://www.gaia-gis.it/gaia-sins/windows-bin-amd64/ 
...
def sqlite_replace_or_add_table(db,
                         dst_table_name,
                         src_table_name,
    ...
    try:
        ...
        if exists == 0:
            ...
            creation_statement = _sql_get_creation_statement_from_table(src_table_name=src_table_name,
