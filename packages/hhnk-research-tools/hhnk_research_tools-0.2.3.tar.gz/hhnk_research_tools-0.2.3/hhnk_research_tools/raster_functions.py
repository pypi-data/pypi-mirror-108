    ...
    try:
        ...
        if gdal_file:
            if return_array:
                ...
                raster_array = _get_array_from_bands(gdal_file, band_count, window, raster_source)
            else:
                raster_array=None
...
def save_raster_array_to_tiff(output_file, raster_array, nodata, metadata, datatype=GDAL_DATATYPE,
    ...
    """
    try:
        target_ds = create_new_raster_file(file_name=output_file, nodata=nodata,
                                           meta=metadata, datatype=datatype,
