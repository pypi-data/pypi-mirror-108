...
class Worker:
    ...
    async def send_sqs_message(
        self, queue_url, message, wait=True, high_throughput=True
    ):
    ...
    async def send_result(self, result, context=None, result_channel=None):
        context_topic = None
        ...
        if context:
            ...
            elif self._sqs_client and context.get("queue_url"):
                await self.send_sqs_message(context.get("queue_url"), result_data)

        # always check the result_channel as it is usually used for data persistence
        if result_channel:
            if result_channel.startswith("https://sqs"):
                await self.send_sqs_message(result_channel, result_data)
            elif self._producer and context_topic != result_channel:
                await self._producer.send(result_channel, result_data)
