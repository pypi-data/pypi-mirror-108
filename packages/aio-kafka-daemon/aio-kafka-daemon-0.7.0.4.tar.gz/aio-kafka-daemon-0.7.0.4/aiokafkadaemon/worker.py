...
class Worker:
    ...
    def __init__(
        ...
        try:
            ...
            # only initiate here (low footprint), whether to use is implemented in on_run by subclass
            self._sqs_client = boto3.client("sqs")
        except Exception as e:
            logger.warning(f"Unable to connect to boto3 client: {e}")

        if create_consumer and not self._sqs_opts.get("queue_url"):
    ...
    async def send_result(self, result, context=None, result_channel_or_topic=None):
        context_topic = None
        ...
        if context:
            ...
            elif self._redis and context.get("redis_channel"):
                redis_channel = context.get("redis_channel")
        ...
        # always check the result_channel_or_topic as it is usually used for data persistence
        if result_channel_or_topic:
            if result_channel_or_topic.startswith("https://sqs"):
                if result_channel_or_topic.endswith(".fifo"):
                    nowait_delete = asyncio.get_running_loop().run_in_executor(
                        None,
                        partial(
                            self._sqs_client.delete_message,
                            QueueUrl=result_channel_or_topic,
                            MessageBody=result_data,
                            MessageDeduplicationId=f"{time.time()}",
                            MessageGroupId=self._uuid,
                        ),
                    )
                else:
                    nowait_delete = asyncio.get_running_loop().run_in_executor(
                ...
                asyncio.ensure_future(nowait_delete)
            elif self._producer and context_topic != result_channel_or_topic:
                await self._producer.send(result_channel_or_topic, result_data)
