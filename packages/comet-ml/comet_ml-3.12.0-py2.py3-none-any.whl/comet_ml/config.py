...
class Config(object):
    ...
    def _set_settings(self, settings, environ=False):
        for setting in settings:
            # TODO: Validate that we match valid config keys and warn if not
            if environ:
                name = self.get_setting_name(setting)
                os.environ[name] = str(settings[setting])
            else:
                key = self.get_setting_key(setting)
                self[key] = settings[setting]
...
def init(directory="~/", __should_prompt_fn=None, **settings):
    """
    ...
    * timeout_upload
    ...
    if __should_prompt_fn is None:
        __should_prompt_fn = is_interactive
