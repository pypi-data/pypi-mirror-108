...
class FileUploadThread(threading.Thread):
    def __init__(
        ...
        on_complete_message=None,
    ):
        # type: (Any, Any, Any, bool, Optional[str], Optional[str]) -> None
        super(FileUploadThread, self).__init__(target=target, args=args, kwargs=kwargs)
    ...
    def run(self):
        try:
            super(FileUploadThread, self).run()
            if self.on_complete_message is not None:
                LOGGER.info(self.on_complete_message)
        except Exception:
            self.has_failed = True
            if self.on_error_message is not None:
                LOGGER.warning(self.on_error_message)


class FileUploadManager(object):
    def __init__(self):
        self.upload_threads = []  # type: List[FileUploadThread]

    def upload_file_thread(self, critical=False, **kwargs):
        new_thread = FileUploadThread(
            target=upload_file, args=[], kwargs=kwargs, critical=critical
        )
        new_thread.start()
    ...
    def upload_file_like_thread(self, critical=False, **kwargs):
        new_thread = FileUploadThread(
            target=upload_file_like, args=[], kwargs=kwargs, critical=critical
        )
        new_thread.start()
        self.upload_threads.append(new_thread)
        LOGGER.debug("Upload threads %s", self.upload_threads)
    ...
    def upload_remote_asset_thread(self, critical=False, **kwargs):
        new_thread = FileUploadThread(
            target=upload_remote_asset, args=[], kwargs=kwargs, critical=critical
        )
        new_thread.start()
        self.upload_threads.append(new_thread)
        LOGGER.debug("Upload threads %s", self.upload_threads)
    ...
    def all_done(self):
        # type: () -> bool
        return all(thread.is_alive() is False for thread in self.upload_threads)
