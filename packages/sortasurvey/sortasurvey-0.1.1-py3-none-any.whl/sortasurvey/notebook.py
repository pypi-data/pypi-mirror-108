...
class Survey:
    ...
    def __init__(self, verbose=True, path_sample='info/TOIs_perfect.csv', path_survey='info/survey_info.csv',
                 path_priority='info/high_priority.csv', path_ignore='info/no_no.csv', iter=1,
                 hours_per_night=10., pool=50., progress=True, save=True, unique=['1B'], note=''):
        ...
        self.get_sample(path_survey, path_sample)
        self.get_programs(path_survey, path_ignore, path_priority, unique)
        self.get_seeds()
        if self.iter > 1:
            self.emcee = True
    ...
    def get_sample(self, path_survey, path_sample, dec=-30., ruwe=2.):
        """
    ...
    def get_programs(self, path_survey, path_ignore, path_priority, unique):
        ...
        if unique is not None:
            self.get_special(unique)
        else:
            self.special = {}


    def get_special(self, unique, special=[]):
        """
