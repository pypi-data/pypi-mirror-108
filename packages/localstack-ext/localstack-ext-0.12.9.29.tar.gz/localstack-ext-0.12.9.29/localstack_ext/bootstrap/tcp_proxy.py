...
class AttrDict(iOCkw):
 def __init__(self,*args,**kwargs):
  iOCka(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and iOCkG(socket,'IPV6_V6ONLY'):
...
class Half(iOCkE):
 def __init__(self,opt,sock,addr,iOCkm):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.iOCkm=iOCkm
  self.name='peer' if self.iOCkm else 'client'
  self.queue=[]
  self.dest=iOCkI
  self.err=iOCkI
  self.ready=iOCkM
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':iOCkM,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':iOCkM,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':iOCkM,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':iOCkM,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
