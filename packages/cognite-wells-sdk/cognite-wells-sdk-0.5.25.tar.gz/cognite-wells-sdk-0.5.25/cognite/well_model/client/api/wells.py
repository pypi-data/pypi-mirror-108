...
class WellsAPI(BaseAPI):
    ...
    def limits(self) -> WellsLimits:
        """ Get limits (min/max) of wells attributes in the project"""
        path: str = self._get_path("/wells/limits")
        response: Response = self.wells_client.get(url_path=path)
        limits: WellsLimits = WellsLimits.parse_raw(response.text)
        return limits
