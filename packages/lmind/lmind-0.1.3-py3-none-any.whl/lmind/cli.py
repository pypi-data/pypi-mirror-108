...
def parse_ids(origs_dict, val):
    ...
    ids =[i.strip() for i in str(val).split('-')]
    for i in ids:
        if i == '0':
            continue
...
def parse_values(values):
    ...
    for value in value_list:
        ...
        if kv[0] not in UPDATE_FIELDS + UPDATE_FIELDS_ABB:
            raise Exception('update field must be one of (%s)' % UPDATE_FIELDS + UPDATE_FIELDS_ABB)
...
def parse_where(where):
    ...
    for w in where_list:
        ...
        if kv[0] not in WHERE_FIELDS + WHERE_FIELDS_ABB:
            raise Exception('where field must be one of (%s)' % WHERE_FIELDS + WHERE_FIELDS_ABB)
        if kv[0] == 'deadline' and not check_deadline_format(kv[1]):
            raise Exception('deadline must be one of format (%s)' % DATE_FORMAT)
...
def parse_order(order):
    ...
    for o in order_list:
        ...
        if kv[0] not in ORDER_FIELDS + ORDER_FIELDS_ABB:
            raise Exception('order field must be one of (%s)' % ORDER_FIELDS + ORDER_FIELDS_ABB)
...
def main():
    ...
    parser.add_argument('-conf', nargs=2, 
                        ...
                        usage: lmd -conf show-info 'path&&na&&exe&&dl&&desc&&fd&&cd'
                        """, 
                        default=None, required=False)
    ...
    if args.show:
        ids = parse_ids(origs_dict, args.show[0])
        ...
        if args.all:
            ...
            ids = parse_ids(origs_dict, args.delete[0])
