...
def printVersion(ctx, param, value):
    if hasattr(testindatadev, "VERSION"):
        click.echo(f'Version {testindatadev.VERSION}')
        return
...
@click.option('--version', is_flag=True, callback=printVersion, expose_value=False, is_eager=True)
@click.option('--debug/--no-debug', default=False, help="using debug mod or not, default is False")
@click.pass_context
def main(ctx, access_key, host, version, debug):
