...
def suggest_featurizers_timeseries(
    ...
    if remove_lookback:
        ...
        lag_lead_op = _transforms.get(TimeSeriesInternal.LAG_LEAD_OPERATOR)  # type: Optional[LagLeadOperator]
        rolling_window = _transforms.get(
            TimeSeriesInternal.ROLLING_WINDOW_OPERATOR)  # type: Optional[RollingWindow]
        if lag_lead_op is not None:
            timeseries_param_dict[TimeSeriesInternal.LAGGING_COLUMNS] = lag_lead_op.preview_column_names(tsdf)
        if rolling_window is not None:
            timeseries_param_dict[TimeSeriesInternal.ROLLING_WINDOW_COLUMNS] = rolling_window.preview_column_names(
                tsdf)
    return (
