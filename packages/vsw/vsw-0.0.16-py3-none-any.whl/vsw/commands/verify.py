        ...
        while True:
            try:
                ...
                if state == "verified":
                    ...
                    revealed_attr_groups = msg["presentation"]["requested_proof"]["revealed_attr_groups"]
                    if "0_software_certificate_uuid" in revealed_attr_groups:
                        revealed_software_certificate = revealed_attr_groups["0_software_certificate_uuid"]["values"]
                        if "attr::softwareurl::value" in software_credential_search:
                            software_hash = vsw.utils.generate_digest(software_credential_search["attr::softwareurl::value"])
                            if software_hash != revealed_software_certificate["softwarehash"]["raw"]:
                                logger.info("Verified error, softwareHash is not matched!")
                                break;
                        if "attr::sourceurl::value" in software_credential_search:
                            source_hash = vsw.utils.generate_digest(software_credential_search["attr::sourceurl::value"])
                            if source_hash != revealed_software_certificate["sourcehash"]["raw"]:
                                logger.info("Verified error, sourceHash is not matched!")
                                break;
