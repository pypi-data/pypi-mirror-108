...
def model_service_update_subgraph_by_multi_hop_multi_edge(data, config_params):
    ...
    if "node_name_list" in data.keys():
        node_name_list = data["node_name_list"]
    else:
        node_name_list = ["tcpflow", "flow"]
        ...
        for h in range(step):
            ...
            for j in range(len(edge_name_list)):
                ...
                if len(graphEdges) == len(edge_name_list):
                    ...
                    graph_edge["detail"] = graph_edge["detail"] + res_tem
                else:
                    graph_edge = {"detail": res_tem}

                graph_edge["type"] = edge_name_list[j]
                schema_tem = res[h][j].columns.values.tolist()
                graph_edge["schema"] = schema_tem
                if len(graphEdges) < len(edge_name_list):
                    graphEdges.append(graph_edge)
        result = model_get_vertex_by_edge(config_params, graph_name, graphEdges, node_name_list)
        return result
    except Exception as e:
        print(e)
...
def model_get_vertex_by_edge(config_params, graph_name, graphEdges, node_name_list):
    node_tem = []
    import numpy as np
    for edge_one in range(len(graphEdges)):
        node_one = np.array(graphEdges[edge_one]["detail"])[:, 0:2].flatten().tolist()
        node_tem += node_one
...
def model_service_update_subgraph_by_find_path_multi_edge(data, config_params):
    ...
    # result = {}
    try:
        res = graph.update_subgraph_by_find_path_multi_edge(subgraph_name,
                                                            start_vertex_list,
                                                            end_vertex_list,
                                                            edge_name_list,
                                                            edge_con_list_list,
                    ...
                    for h in range(step_limit):
                        ...
                        for j in range(len(edge_name_list)):
                            if h < len(res[i - 1]):
                                ...
                                #     result["s_" + str(i)]["hop_" + str(h + 1)][edge_name_list[j]] = []
                                #     result["s_" + str(i)]["hop_" + str(h + 1)][edge_name_list[j]] = []
                                # mds.ModelService.search_subgraph_by_condition()
