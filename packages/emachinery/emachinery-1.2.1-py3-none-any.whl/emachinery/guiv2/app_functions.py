...
import os, sys, json, re, copy, pkg_resources
import pandas as pd
...
from acmsimpyv1 import acmsimpy
...
class AppFunctions:
    ...
    def save_path2acmsimc(self):
        self.configlob['path2acmsimc'] = self.path2acmsimc = self.ui.lineEdit_path2acmsimc.text()
    ...
    def decode_labelsAndSignals(self):
        ...
        if len(list__number_of_traces_per_subplot) != len(labels):
            print('[Error] missing label or plotting variable.')
    ...
    def compile_and_run(self, output_fname='main'):
        ...
        if output_fname == 'main':
            ...
            # PA调用，可能会同时编译出好几个main.exe，这样上一次放着还没跑完的话，就会报占用错误。
    ...
    def runPyBasedSimulation(self, bool_realtime=True):
        ...
        self.NUMBER_OF_SAMPLE_TO_SHOW = 50000
        self.TIME_SLICE = 0.1
        self.TIME_SLICE_MS = self.TIME_SLICE * 1e3
        ...
        def update_line_data(end_time, line, ydata):
            ...
            xdata = np.arange(end_time*self.SAMPLING_RATE-len(line.ydata), end_time*self.SAMPLING_RATE, 1) * self.CL_TS # 整数代替浮点数，精度高否则会出现xdata长3001，ydata长3000的问题。
        ...
        def ACMアニメ4Py(ii):
            if ii>10000 or self.ui.lineEdit_AtTop.text() == 's':
                self.anim.event_source.stop()
                print('Stop ACMPlot animation.\n----------\n\n')
                self.bool_PybasedSimulationDone = True
                UIFunctions.labelTitle(self, 'Main Window - emachinery - Simulation Done')
            ...
            t0 = ii*self.TIME_SLICE
            control_times, id, iq, ia, ib, speed, = \
                acmsimpy.ACMSimPyIncremental(1, t0=t0, TIME=self.TIME_SLICE, \
                                                ACM=ACM,\
                                                CTRL=CTRL,\
                                                reg_id=reg_id,\
                                                reg_iq=reg_iq,\
            ...
            update_line_data(end_time, line01, speed)
            update_line_data(end_time, line02, ia)
            time_text.set_text('time = %.1f' % end_time)

            if self.NUMBER_OF_SAMPLE_TO_SHOW is None:
                first_ax.set_xlim([0, end_time])
            else:
                first_ax.set_xlim([end_time-self.NUMBER_OF_SAMPLE_TO_SHOW*self.CL_TS, end_time])
        ...
        self.ui.MplWidget_ACMPlot.canvas.figure.clf()
        number_of_subplot = 2
        first_ax = self.ui.MplWidget_ACMPlot.canvas.figure.add_subplot(number_of_subplot, 1, 1+0, autoscale_on=False, sharex=None)
        i = 1
        ax       = self.ui.MplWidget_ACMPlot.canvas.figure.add_subplot(number_of_subplot, 1, 1+i, autoscale_on=False, sharex=first_ax)

        first_ax.set_ylabel('Speed [rpm]')
        ax.set_ylabel('phase current [A]')
        ax.set_xlabel('Time [s]')
        # ax.legend(loc='lower right').set_zorder(202)

        line01, = first_ax.plot([], [], '-', lw=1)
        line01.ax = first_ax
        line01.ydata = np.array(0)
        line01.MIN = -1e-5
        line01.MAX =  1e-5
        j = 0
        line02, =       ax.plot([], [], linestyle=self.cjh_linestyles[j], color=self.cjh_colors[j], lw=1.5, label='', alpha=0.5, zorder=100+0)
        line02.ax = ax
        line02.ydata = np.array(0)
        line02.MIN = -1e-5
        line02.MAX =  1e-5
        ...
        # init
        ACM       = acmsimpy.The_AC_Machines()
        CTRL      = acmsimpy.The_Motor_Controller(self.CL_TS, 5*self.CL_TS)
        ...
        self.anim = animation.FuncAnimation(self.ui.MplWidget_ACMPlot.canvas.figure, ACMアニメ4Py, interval=2*self.TIME_SLICE_MS) #, blit=True) blit=True means only re-draw the parts that have changed. #https://stackoverflow.com/questions/14421924/matplotlib-animate-does-not-update-tick-labels
