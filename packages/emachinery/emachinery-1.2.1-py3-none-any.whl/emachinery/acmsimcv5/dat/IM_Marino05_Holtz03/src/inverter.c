...
void inverter_model(){
    ...
    // 根据给定电压CTRL.O->uab_cmd[0]和实际的电机电流ACM.ial，计算畸变的逆变器输出电压ACM.ual。
        ...
        InverterNonlinearity_SKSul96(CTRL.O->uab_cmd[0]*sqrt(CLARKE_TRANS_TORQUE_GAIN), \
                                     CTRL.O->uab_cmd[1]*sqrt(CLARKE_TRANS_TORQUE_GAIN), \
                                     ACM.ial, \
                                     ACM.ibe);
        ACM.ual = UAL_C_DIST;
        ACM.ube = UBE_C_DIST;
    ...
    #else
                            // 考虑控制器和电机所用Clarke变换不同导致的系数变化
        ACM.ual = CTRL.O->uab_cmd[0]; //*sqrt(CLARKE_TRANS_TORQUE_GAIN); *AMPL2POW
        ACM.ube = CTRL.O->uab_cmd[1]; //*sqrt(CLARKE_TRANS_TORQUE_GAIN); *AMPL2POW
    #endif
