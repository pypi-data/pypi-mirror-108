...
void COMM_intertiaId(REAL id_fb, REAL iq_fb, REAL cosPark, REAL sinPark, REAL omg_elec_fb){
    ...
    REAL error = (SPEED_COMMAND_BIAS+SPEED_COMMAND_RANGE*cos(M_PI+2*M_PI*TEST_SIGNAL_FREQUENCY*CTRL.timebase))*RPM_2_ELEC_RAD_PER_SEC - SPEED_SIGNAL;
    // CTRL.speed_ctrl_err = error;
    ...
    if(FALSE){ // Open loop
        ...
        if(vc_count++ == SPEED_LOOP_CEILING){
            ...
            pid1_spd.Ref = (SPEED_COMMAND_BIAS+SPEED_COMMAND_RANGE*cos(M_PI+2*M_PI*TEST_SIGNAL_FREQUENCY*CTRL.timebase))*RPM_2_ELEC_RAD_PER_SEC;
...
void StepByStepCommissioning(){
    ...
    if(bool_comm_status == 0){
        ...
        // // 计算转速（lidozzi2007）
        // if(EXP.bool_position_correction_pending == TRUE){
        //     EXP.bool_position_correction_pending = FALSE;
        //     local_theta_d = EXP.theta_d_hall;
        // }
        // CTRL.S->cosT = cos(local_theta_d);
        // CTRL.S->sinT = sin(local_theta_d);
        // REAL Vq = AB2T(US_C(0), US_C(1), CTRL.S->cosT, CTRL.S->sinT);
        // REAL iq = AB2T(IS_C(0), IS_C(1), CTRL.S->cosT, CTRL.S->sinT);
        ...
        COMM_PMFluxId(AB2M(IS_C(0), IS_C(1), CTRL.S->cosT, CTRL.S->sinT), AB2T(IS_C(0), IS_C(1), CTRL.S->cosT, CTRL.S->sinT), CTRL.I->omg_elec);
        ...
        CTRL.S->cosT = cos(CTRL.I->theta_d_elec);
        CTRL.S->sinT = sin(CTRL.I->theta_d_elec);
        COMM_intertiaId(AB2M(IS_C(0), IS_C(1), CTRL.S->cosT, CTRL.S->sinT), AB2T(IS_C(0), IS_C(1), CTRL.S->cosT, CTRL.S->sinT), CTRL.S->cosT, CTRL.S->sinT, CTRL.I->omg_elec);
