...
#define OFFSET_VOLTAGE_ALPHA (1*-0.1 *(CTRL.timebase>0)) // (0.02*29*1.0) // this is only valid for estimator in AB frame. Use current_offset instead for DQ frame estimator
#define OFFSET_VOLTAGE_BETA  (1*-0.2 *(CTRL.timebase>0)) // (0.02*29*1.0) // this is only valid for estimator in AB frame. Use current_offset instead for DQ frame estimator
...
void simulation_only_flux_estimator(){
    ...
    if(TRUE){
        // 
        simvm.emf[0] = CTRL.O->uab_cmd[0] - CTRL.motor->rs*CTRL.I->iab[0] + 1*(CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA;
        simvm.emf[1] = CTRL.O->uab_cmd[1] - CTRL.motor->rs*CTRL.I->iab[1] + 1*(CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA;
        ...
        temp_Alpha1 += CL_TS * ( CTRL.O->uab_cmd[0] - CTRL.motor->rs*CTRL.I->iab[0] + 1*(CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA - 0*exact.u_offset[0] - 0.5*exact.u_offset[0] - 1*exact.offset_voltage_compensation[0])     + 0*GAIN_OHTANI * ( CTRL.I->cmd_psi_ABmu[0] - (temp_Alpha1-CTRL.motor->Lsigma*CTRL.I->iab[0]) );
        temp_Beta1  += CL_TS * ( CTRL.O->uab_cmd[1] - CTRL.motor->rs*CTRL.I->iab[1] + 1*(CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA  - 0*exact.u_offset[1] - 0.5*exact.u_offset[1] - 1*exact.offset_voltage_compensation[1])     + 0*GAIN_OHTANI * ( CTRL.I->cmd_psi_ABmu[1] - (temp_Beta1 -CTRL.motor->Lsigma*CTRL.I->iab[1]) );
...
void general_4states_rk4_solver(pointer_flux_estimator_dynamics fp, double t, double *x, double hs){
    #define NS 4
...
void general_5states_rk4_solver(pointer_flux_estimator_dynamics fp, double t, double *x, double hs){
    #define NS 5
    double k1[NS], k2[NS], k3[NS], k4[NS], xk[NS];
    double fx[NS];
    int i;
...
void rhf_Ohtani1992_Dynamics(double t, double *x, double *fx){
    REAL emf[2];
    emf[0] = US(0) - CTRL.motor->rs*IS(0) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA + VM_OHTANI_CORRECTION_GAIN_P * ( CTRL.I->cmd_psi_ABmu[0] - (x[0]-CTRL.motor->Lsigma*IS(0)) );
    emf[1] = US(1) - CTRL.motor->rs*IS(1) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA  + VM_OHTANI_CORRECTION_GAIN_P * ( CTRL.I->cmd_psi_ABmu[1] - (x[1]-CTRL.motor->Lsigma*IS(1)) );
    fx[0] = emf[0];
    fx[1] = emf[1];    
}
void VM_Ohtani1992(){
...
void rhf_Holtz2002_Dynamics(double t, double *x, double *fx){
    ...
    REAL emf[2];
    emf[0] = US(0) - CTRL.motor->rs*IS(0) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA + VM_OHTANI_CORRECTION_GAIN_P * ( CTRL.I->cmd_psi*cos_rho - psi_2[0] );
    emf[1] = US(1) - CTRL.motor->rs*IS(1) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA  + VM_OHTANI_CORRECTION_GAIN_P * ( CTRL.I->cmd_psi*sin_rho - psi_2[1] );
    fx[0] = emf[0];
    fx[1] = emf[1];
}
...
/* Lascu and Andreescus 2006 TODO 非常好奇Lascu的方法会怎样！和我们的xRho校正项对比！ */
void rhf_LascuAndreescus2006_Dynamics(double t, double *x, double *fx){
    ...
    REAL emf[2];
    emf[0] = US(0) - CTRL.motor->rs*IS(0) + 1*(CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA \
        /*P*/+ 1*VM_PROPOSED_PI_CORRECTION_GAIN_P * rotor_flux_error[0] \
        /*I*/+ 1*x[2];
    emf[1] = US(1) - CTRL.motor->rs*IS(1) + 1*(CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA  \
...
/* A. Exact Compensation Method (Proposed) */
...
void rhf_CmdErrFdkCorInFrameRho_Dynamics(double t, double *x, double *fx){
    ...
    REAL emf[2];
    emf[0] = US(0) - CTRL.motor->rs*IS(0) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA \
        /*P*/+ VM_PROPOSED_PI_CORRECTION_GAIN_P * rotor_flux_error[0] \
        /*I*/+ x[2];
    emf[1] = US(1) - CTRL.motor->rs*IS(1) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA  \
...
void rhf_ClosedLoopFluxEstimator_Dynamics(double t, double *x, double *fx){
    ...
    REAL emf[2];
    emf[0] = US(0) - CTRL.motor->rs*IS(0) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA \
        /*P*/+ CLOSED_LOOP_ESTIMATOR_GAIN_KP * current_error[0] \
        /*I*/+ x[3];
    emf[1] = US(1) - CTRL.motor->rs*IS(1) + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA  \
        /*P*/+ CLOSED_LOOP_ESTIMATOR_GAIN_KP * current_error[1] \
        /*I*/+ x[4];
    fx[0] = emf[0];
    fx[1] = emf[1];
    /* Lascu CLE */
    ...
    //       + CLOSED_LOOP_ESTIMATOR_GAIN_KCM*(cos_rho*current_error[0] + sin_rho*current_error[1]);
    /* MRAS */
    REAL ids = IS(0)*cos_rho + IS(1)*sin_rho;
    fx[2] = CTRL.motor->rreq*ids - CTRL.motor->alpha*x[2]\
          + CLOSED_LOOP_ESTIMATOR_GAIN_KCM*(cos_rho*current_error[0] + sin_rho*current_error[1]);
    fx[3] = CLOSED_LOOP_ESTIMATOR_GAIN_KI * current_error[0];
    fx[4] = CLOSED_LOOP_ESTIMATOR_GAIN_KI * current_error[1];
}
...
void Holtz2003_init(){
    ...
    for(ind=0;ind<2;++ind){
        ...
        htz.gain_off = 5; // 50.1 // 20 -> too large then speed will oscillate during reversal near zero
...
void rhf_Holtz2003_Dynamics(double t, double *x, double *fx){
    htz.emf_stator[0] = US(0) - htz.rs_est*IS(0) - htz.u_off[0] + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_ALPHA;
    htz.emf_stator[1] = US(1) - htz.rs_est*IS(1) - htz.u_off[1] + (CTRL.timebase>0.0)*OFFSET_VOLTAGE_BETA ;
...
void VM_Saturated_ExactOffsetCompensation_WithAdaptiveLimit(){
    ...
    htz.emf_stator[0] = US_C(0) - htz.rs_est*IS_C(0) - htz.u_off[0];
    htz.emf_stator[1] = US_C(1) - htz.rs_est*IS_C(1) - htz.u_off[1];
    ...
    // 积分方法：（从上面的程序来看，u_off的LPF的输入是每半周更新一次的。
        ...
        htz.u_off[0] += htz.gain_off * CL_TS * INTEGRAL_INPUT(0);
        htz.u_off[1] += htz.gain_off * CL_TS * INTEGRAL_INPUT(1);
    #endif
    ...
    // 低通
        ...
        htz.u_off[0] = _lpf( LPF_INPUT(0), htz.u_off[0], TAU_OFF_INVERSE);
        htz.u_off[1] = _lpf( LPF_INPUT(1), htz.u_off[1], TAU_OFF_INVERSE);
    #endif

    // 差分
    // 别傻，不是在这里更新的，此处更新频率是1/CL_TS啊…… 
    // htz.u_off[0] += DIFFERENCE_INPUT(0);
    // htz.u_off[1] += DIFFERENCE_INPUT(1);

    // 直通
    // htz.u_off[0] = htz.u_off_direct_calculated[0];
    // htz.u_off[1] = htz.u_off_direct_calculated[1];
