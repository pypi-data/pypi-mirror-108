// 新发现，也可以用typedef定义函数指针帮忙写状态机，下次可以试试看：
...
int event_PMFluxLinkageId(void){
    ...
    COMM.KE = (uq_cmd - COMM.R*CTRL.iq__fb) / (rpm_speed_command*RPM_2_ELEC_RAD_PER_SEC) - COMM.L*CTRL.id__fb;
...
int event_inertiaId(void){
    ...
    #define ROTOR_FLUX_ALPHA MT2A(COMM.KE, 0.0, CTRL.S->cosT, CTRL.S->sinT)
    #define ROTOR_FLUX_BETA  MT2B(COMM.KE, 0.0, CTRL.S->cosT, CTRL.S->sinT)
    #define NORMINAL_INERTIA (0.10) // 随便给个数量级差不多的数，不同数值只对画图有影响
    #define SPEED_SIGNAL (CTRL.I->omg_elec)
