...
void Machine_init(){
    ...
    // IM.x[4] = 20*ELEC_RAD_PER_SEC_2_RPM;
    ...
    IM.Lmu    = IM_MAGNETIZING_INDUCTANCE   * (MISMATCH_LMU*0.01);
    IM.Lsigma = IM_TOTAL_LEAKAGE_INDUCTANCE * (MISMATCH_LSIGMA*0.01);
    ...
    IM.rs    = IM_STAOTR_RESISTANCE * (MISMATCH_RS*0.01);
    IM.rreq  = IM_ROTOR_RESISTANCE * (MISMATCH_RREQ*0.01);
...
void IM_saturated_Dynamics(double t, double *x, double *fx){
    ...
    fx[2] =            - IM.alpha*(IM.Lm+IM.Llr)*IM.idr - x[4]*x[3]; // d-axis
    fx[3] =            - IM.alpha*(IM.Lm+IM.Llr)*IM.iqr + x[4]*x[2]; // q-axis
...
int machine_simulation(){
    ...
    ACM.rpm = ACM.x[4] * ELEC_RAD_PER_SEC_2_RPM; // 机械转速 [mech. r/min]
