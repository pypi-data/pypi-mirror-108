...
double load_model(){
    ...
    if(CTRL.timebase>5){
        if(fabs(ACM.rpm)>10 && load_state==0){
            load_state = 1;
            Tload = VISCOUS_COEFF*ACM.rpm*RPM_2_ELEC_RAD_PER_SEC + LOAD_TORQUE*sign(ACM.rpm);
        }else if(fabs(ACM.rpm)<-10 && load_state==1){
            load_state = 0;
            // TODO this is not working?!
            Tload = VISCOUS_COEFF*ACM.rpm*RPM_2_ELEC_RAD_PER_SEC + 1000* LOAD_TORQUE*sign(ACM.rpm);
        // }else{
        //     Tload = VISCOUS_COEFF*ACM.rpm*RPM_2_ELEC_RAD_PER_SEC;
        }        
    }
