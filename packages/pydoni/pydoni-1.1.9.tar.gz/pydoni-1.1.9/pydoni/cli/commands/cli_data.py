...
@click.option('--max-dir-size', type=float, default=None, required=False,
              ...
              specified, after the dump is complete, then check if the total directory
...
def pg_dump(backup_dir,
    ...
    if pg_user is not None and pg_dbname is not None:
        pg = pydoni.Postgres(pg_user=pg_user, pg_dbname=pg_dbname)
    else:
    ...
    subdir = pydoni.systime(compact=True) + '_' + pg.dbname
