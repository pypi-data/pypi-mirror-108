...
def list_attached_volumes(external_only: bool=False):
    """
...
def delete_empty_dirs(dpath: typing.Union[str, pathlib.Path], recursive: bool=False):
    """
...
class AudioFile(object):
    ...
    def convert(self, output_fpath=None, output_format=['mp3', 'wav']):
        """
        ...
        if self.ext == '.m4a' and output_format == 'mp3':
            ...
            self.pydub_sound.export(output_fpath, format=output_format)

        logger.info(f'Exported file "{output_fpath}" in format {output_format}')
...
def convert_audible(fpath: typing.Union[str, pathlib.Path], fmt: str, activation_bytes: str):
    ...
    cmd = '{ffmpeg_bin} -activation_bytes {activation_bytes} -i "{fpath}" -vn -c:a copy "{output_fpath}" -y'
    syscmd(cmd)

    if fmt == 'mp3':
        # Convert to mp3
        logger.info('Converting MP4 to MP3 at 256k "{output_fpath}"')
        mp4_to_mp3(output_fpath, bitrate=256)
...
class Postgres(object):
    ...
    def connect(self):
        ...
        if self.dbuser is None and self.dbname is None:
            ...
            logger.warning('No credentials supplied, attempting to parse ~/.pgpass file')
    ...
    def build_delete(self,
        ...
        if isinstance(pkey_value, list):
            pkey_value_lst = [self.__single_quote__(x) for x in pkey_value]
            sql = ['delete from {}', 'where {} in ({})']
            pkey_value_str = ', '.join([str(x) for x in pkey_value_lst])
        else:
            pkey_value_str = self.__single_quote__(pkey_value)
    ...
    def dump(self, backup_dir: typing.Union[str, pathlib.Path]):
        ...
        output_fpath = f'{backup_dir}/{self.dbname}.sql'
        cmd = f'{bin} --user {self.dbuser} {self.dbname} > "{output_fpath}"'
...
def check_network_connection(abort: bool=False):
    """
