...
class TestPydoni(unittest.TestCase):
    ...
    def test_Postgres(self):
        ...
        if connected:
            ...
            # Method: __single_quote__()
            self.assertEqual(pg.__single_quote__(5), 5)
            self.assertEqual(pg.__single_quote__('test'), "'test'")
            self.assertEqual(pg.__single_quote__("test's"), "'test''s'")
    ...
    def test_AudioFile(self):
        ...
        # conversation_text = audio.transcribe()  # Uncomment when billing re-enabled https://console.cloud.google.com/billing
