...
class TestComponentInfo(unittest.TestCase):
    ...
    def test_quick(self):
        data =  [' peter like to parrty albertoandreottiATgmail', 'peter lieks lardy! ', 'of wara ', 'asd asd as','asdasd asd asd asd ', 'asd asd ']
        p = nlu.load('ner.onto.bert.tiny dep.typed',verbose=True)
        p.viz(data,viz_type='dep')
        res = p.predict(data,multithread=False)
