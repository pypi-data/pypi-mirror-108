...
data:
  ...
  fluent.conf: |
    ...
    <match /^kubernetes.var.log.containers.spell-run-.+_spell-run_.*/>
      @type file
      path /tmp/benno-spell-run-logs
    </match>

    # TODO(Benno): uncomment this and remove the `@type file` above when autossh is set up
    # <match /^kubernetes.var.log.containers.spell-run-.+_spell-run_.*/>
    #   @type forward
    #   <server>
    #     host 127.0.0.1
    ...
    <match /^kubernetes.var.log.containers.model-serving-.+_serving_.*/ /^kubernetes.var.log.containers.model-serving-.+_spell-serving_.*/>
        @type elasticsearch
        @id out_es
        @log_level info
