...
class DiskMonitor:
    def __init__(self, disk_mon_data, cloud_vendor="Amazon"):
        ...
        for fs in disk_mon_data.get('filesystems', {}):
            self.filesystems[create_zesty_filesystem_id(cloud=self.cloud_vendor, fs_id=fs)] = FileSystem(
                fs_data=disk_mon_data.get('filesystems', {}).get(fs, {}),
                fs_id=fs,
...
class FileSystem:
    def __init__(self, fs_data, fs_id, cloud_vendor="Amazon"):
        self.cloud_vendor = cloud_vendor
        self.fs_id = create_zesty_filesystem_id(cloud=self.cloud_vendor, fs_id=fs_id)
        ...
        for dev in fs_data.get('devices'):
            self.devices[dev] = self.BlockDevice(
                block_device_data=fs_data.get('devices', {}).get(dev, {}),
                cloud_vendor=self.cloud_vendor
            )

    class BlockDevice(IDeviceHF):
        def __init__(self, block_device_data, cloud_vendor="Amazon"):
            self.cloud_vendor = cloud_vendor
            self.size = block_device_data.get('size')
            self.btrfs_dev_id = block_device_data.get('btrfs_dev_id')
            self.volume_id = create_zesty_id(cloud=self.cloud_vendor, resource_id=block_device_data.get('volume_id'))
            ...
            self.lun = block_device_data.get('lun')
    ...
    class Usage:
        def __init__(self, usage_data):
            self.total = usage_data.get('total', 0)
            self.used = usage_data.get('used', 0)
            self.free = usage_data.get('free', 0)
            self.percent = usage_data.get('percent', 0)
