...
class SpecValidator:
            ...
            for sublist, message in zip(
                ...
                if len(sublist) > 0:
                    ...
                    for k in sorted(sublist):
                        logger.error(f"{str(k):<10}:\t{validation_errors[k]}")
    ...
    def _return_BytesIO(validated_headers, input_headers, fits_cards) -> BytesIO:
        ...
        """
        if isinstance(input_headers, (pathlib.PosixPath, BytesIO)):
    ...
    @staticmethod
    def _return_HDU(validated_headers, input_headers, fits_cards):
        """
