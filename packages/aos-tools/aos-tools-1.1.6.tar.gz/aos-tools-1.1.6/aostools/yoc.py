        ...
        for comp in comps_list:
            ...
            for dep_dict in comp.depends:
                ...
                if (name not in exclude_names) and (cond.find('?') < 0):
                    ...
                    tmp = {}
                    tmp["comp_name"] = name
                    tmp["count"] = parent_found["count"] + 1
                    tmp["condition"] = parent_found["condition"]
                    depends.append(tmp)
                    # print(tmp["count"])
                    # print("add depend:%s with inherrited cond:%s for %s" % (name, parent_found["condition"], comp.name))
                ...
                if (name not in exclude_names) and (cond.find('?') >= 0):
                    ...
                    if len(v) > 1:
                        ...
                        if x:
                            ...
                            tmp["comp_name"] = name
                            tmp["count"] = parent_found["count"] + 1
                            tmp["condition"] = parent_found["condition"].copy()
                            for cond in conds:
                                cond = cond.strip()
                                if cond and cond not in parent_found["condition"]:
                                    tmp["condition"].append(cond)
                            depends.append(tmp)
                            # print(tmp["count"])
                            # print("add depend:%s with inherrited cond:%s and append cond: %s for %s" % (name, parent_found["condition"], conds, comp.name))
