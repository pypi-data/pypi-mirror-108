...
RiscvCPU = ['rv32emc', 'rv32ec', 'rv32i', 'rv32iac', 'rv32im', 'rv32imac', 'rv32imafc', 'rv64imac', 'rv64imacxcki', 'rv64imafdc', 'e902', 'e902m', 'e902t', 'e902mt', 'e906', 'e906f', 'e906fd', 'c906', 'c906fd', 'c906v', 'c910', 'c910v', 'c906d']
    ...
    def set_cpu(self, cpu):
        ...
        if self.CPU in CkcoreCPU:
            ...
            if self.CPU == 'c910v':
                ...
                flags.append('-mtune=c910')
            if self.CPU == 'c906d':
                flags.append('-march=rv64imafdcxthead')
                flags.append('-mabi=lp64d')
                flags.append('-mcmodel=medlow')
                flags.append('-mtune=c906')
            ...
            if not script_result or not SCons.GetOption('verbose'):
                ...
                for line in lines:
                    try:
                        ...
                        if(encode["encoding"] == "ISO-8859-9") or (encode["encoding"] == "ISO-8859-1"):
                            encode["encoding"] = ""
                        if not encode["encoding"]:
                            if self.env['PLATFORM'] == 'win32':
                                encode["encoding"] = 'gbk'
