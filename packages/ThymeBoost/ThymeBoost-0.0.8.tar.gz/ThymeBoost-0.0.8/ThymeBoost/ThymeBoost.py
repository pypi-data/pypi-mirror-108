...
from ThymeBoost.ThymeBoostOptimizer import ThymeBoostOptimizer
from ThymeBoost.FitExogenous import FitExogenous
from ThymeBoost.CostFunctions import calc_cost
from ThymeBoost.skylasso import SkyLasso
import warnings
...
class ThymeBoost:
    ...
    while boost_:
      ...
      if (round_cost <= cost and self.n_rounds == -1) or i < self.n_rounds:
        if cost > 0:
            ...
            cost = round_cost + self.cost_penalty*round_cost - .000000000000001
  ...
  def init_fit_classes(self):
    self.trend_class = FitTrend(
                            ...
                            connectivity_constraint = self.connectivity_constraint
                            )
  ...
  def optimize(self, 
      ...
      self.optimization = ThymeBoostOptimizer(**optimize_params)
      optimization_predictions = self.optimization.predict()

      self.output = optimization_predictions
