...
@patch(f"{class_path}._get_unspecifiedAsN_value", return_value=[])
@patch(f"{class_path}.most_relevant_measurement_value", return_value=[])
def test_should_run(mock_most_relevant_measurement_value, mock_unspecifiedAsN):
    ...
    mock_unspecifiedAsN.return_value = [10]
    should_run, *args = _should_run(cycle)
    assert not should_run
