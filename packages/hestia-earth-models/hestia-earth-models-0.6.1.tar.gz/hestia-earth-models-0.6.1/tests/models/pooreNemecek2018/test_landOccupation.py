...
def test_should_run(mock_get_product):
    # with a cycle and functionalUnitMeasure = 1 ha => no run
    impact = {'cycle': {'functionalUnitMeasure': CycleFunctionalUnitMeasure._1_HA.value}}
    should_run, *args = _should_run(impact)
    assert not should_run

    # with product value and economicValueShare => run
    mock_get_product.return_value = {'value': [10], 'economicValueShare': 80}
    should_run, *args = _should_run(impact)
    assert should_run
