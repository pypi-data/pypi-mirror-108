...
def _run_with_unspecified(temperature: float, soilPh: float, unspecifiedAsN_value: float, country_id: str):
...
def _should_run(cycle: dict):
    ...
    unspecifiedAsN = _get_unspecifiedAsN_value(cycle)
    country_id = site.get('country', {}).get('@id')
    lookup = download_lookup(COUNTRY_LOOKUP_TABLE, True)
    has_country_data = country_id in list(lookup.termid)
    logger.debug('Found lookup data for Term: %s? %s', country_id, has_country_data)

    logger.debug('unspecifiedAsN=%s', unspecifiedAsN)

    should_run = valid_site_type(cycle, True) \
        and len(temperature) > 0 \
        and len(soilPh) > 0 \
        and (
            (len(unspecifiedAsN) > 0 and has_country_data) or
            (len(unspecifiedAsN) == 0 and len(N_inputs) > 0)
    )
    logger.info('model=%s, term=%s, should_run=%s', MODEL, TERM_ID, should_run)

    return should_run, list_average(temperature), list_average(soilPh), N_inputs, unspecifiedAsN, country_id


def run(cycle: dict):
    should_run, temperature, soilPh, N_inputs, unspecifiedAsN, country_id = _should_run(cycle)
    return (
        _run_with_unspecified(temperature, soilPh, list_sum(unspecifiedAsN), country_id) if len(unspecifiedAsN) > 0
