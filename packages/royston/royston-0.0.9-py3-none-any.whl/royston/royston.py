...
class Royston:
    ...
    def get_history_period(self, history_days = None, trend_days = None):
        ...
        history_end = dt.now() - dateutil.relativedelta.relativedelta(days = trend_days)
    ...
    def get_trend_period(self, trend_days = None):
        if trend_days == None:
            trend_days = self.options['trend_days']
        return [dt.now(), dt.now() - dateutil.relativedelta.relativedelta(days = trend_days)]

    def set_periods(self):
        [start, end] = self.get_trend_period()
    ...
    def train_doc2vec(self):
        ...
        #train_corpus = self.doc2vec_docs
        print(self.doc2vec_docs)
    ...
    def find_docs(self, ngram, options):
        ...
        #print('find_docs')
        #print(list(self.ngram_history.keys())[0:5])
        if (not ngram in self.ngram_history):
            return list(self.ngram_history.keys())
    ...
    def trending(self, options = {}):
        ...
        for trend in trend_phrases:
            print(trend['phrases'])

        def doc2vec_distance(trend_i, trend_j):
            distances = []
            #print('trend_i', trend_i)
            for doc_i_id in trend_i['docs']:
                doc_i_tokens = self.doc2vec_tokens[doc_i_id]
                #print('doc_i_tokens', doc_i_tokens)
                #doc_i_tokens = list(ingest_corpus([doc_i], True))
                for doc_j_id in trend_j['docs']:
                    doc_j_tokens = self.doc2vec_tokens[doc_j_id]
                    print('doc_j_tokens', doc_j_tokens)

                    #print('-----')
                    #print(doc_i_tokens[0])
                    #print(doc_j_tokens[0])
