...
def LineXPolygon2D(P0,L,Polygon):
    ...
    for i in range(n):
        ...
        if P is None :
            pass
        elif P == 'Line':                       # the edge is part of line
            P1,P2=(x1,y1),(x2,y2)
            return P1,P2
        else:
            if P1 is None:
...
# (2) None, no intersection;
# (3) 'Line', plus a warning message on console: there are two points of intersection
def LineXPolygon(P0,L,Polygon):
    ...
    if Point is None:
        return None
    if Point == 'Line':                # If the line lies in the plane, it goes into 2D space
        PL=P0+L
