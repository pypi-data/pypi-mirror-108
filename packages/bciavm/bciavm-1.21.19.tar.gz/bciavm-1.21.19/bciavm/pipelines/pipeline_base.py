...
class PipelineBase(ABC, metaclass=PipelineBaseMeta):
    ...
    def avm(self, X, batch_sample_sets=1, latest_production_version=1, max_sample_sz=15, min_sample_sz=5, pred_interval=.68, conf_min=.5):
    ...
    def _parallel_predict(self, X):
        ...
        class avm_wrapper(mlflow.pyfunc.PythonModel):
            ...
            def predict(self, X):
                return self.model.predict(X, context=self.context).values
            
        dask_component_graph = ParallelPostFit(avm_wrapper(model))
        return dask_component_graph.predict(X) 


    def parallel_predict(self, X, model_name='avm', model_version='Production', context={'conf_min':0.5, 'batch_sample_sets':1, 'min_sample_sz':5, 'max_sample_sz':15, 'pred_interval':0.68}, cols=['unit_id','avm','avm_lower','avm_upper','conf','ts','latest_production_version','latest_staging_version']):
        """Make predictions using selected features.
