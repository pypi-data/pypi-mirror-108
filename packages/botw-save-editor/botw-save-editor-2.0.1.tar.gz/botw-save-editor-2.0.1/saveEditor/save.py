...
def parseSaveFile(savefile, skip_bools=False):
    ...
    data = f.read()
    f.close()
    if (data[4:0xC] == b'\xff\xff\xff\xff\x00\x00\x00\x01' and data[-4:] == b'\xff\xff\xff\xff'):
        return parseData_be(data, skip_bools)
    elif (data[4:0xC] == b'\xff\xff\xff\xff\x01\x00\x00\x00' and data[-4:] == b'\xff\xff\xff\xff'):
        return parseData_le(data, skip_bools)
    else:
        return(None)

def parseData_be(data, skip_bools):
    parsed_data = {}
    i = 0xC
    while i < len(data)-4:
...
def parseData_le(data, skip_bools):
    parsed_data = {}
    i = 0xC
    while i < len(data)-4:
        hashvalue, entrydata = struct.unpack('<i4s', data[i:i+8])
...
def writeSaveFile_be(json_data, savefile):
    f = open(savefile,'rb')
    data = list(f.read())
    f.close()
    try:
        ...
        print('Error writing savefile from json data; writeSaveFile_be')
        return(None)
...
def writeSaveFile_le(json_data, savefile):
    f = open(savefile,'rb')
    data = list(f.read())
    f.close()
    try:
        assert data[4:0xC] == list(b'\xff\xff\xff\xff\x01\x00\x00\x00')
        assert data[-4:] == list(b'\xff\xff\xff\xff')
    except:
        print('Error writing savefile from json data; writeSaveFile_le')
        return(None)
