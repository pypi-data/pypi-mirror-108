...
# 提供一些有用的信息
URL = 'https://github.com/liushuailong/utils_slliu'    # 通常会附上 GitHub 地址
NAME = 'utils_slliu'                                     # Python 包的名称，即在 pip install 时后面跟的包名
VERSION = '0.0.2'                                             # 包的版本，每次上传到 PyPI 都需要改变这个版本号，否则只会往存储空间增加新内容，无法达到预期
DESCRIPTION = 'The python package of liushuailong for high quality code.'                   # 关于该包的剪短描述
if os.path.exists('README.md'):                               # 如果需要，可以加入一段较长的描述，比如读取 README.md，该段长描述会直接显示在 PyPI 的页面上
    with open('README.md', encoding='utf-8') as f:
        LONG_DESCRIPTION = f.read()
else:
    LONG_DESCRIPTION = DESCRIPTION
AUTHOR = 'liushuailong'                                               # 留下大名
AUTHOR_EMAIL = 'liushuailong001@163.com'                              # 留下邮箱
...
CONSOLE_SCRIPT = 'my-cmd=utils_slliu.my_cmd:main'                  # 如果想在 pip install 之后自动生成一个可执行命令，就靠它了:
