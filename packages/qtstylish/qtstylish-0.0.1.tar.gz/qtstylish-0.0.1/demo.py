...
class ThemeSwitcher(QtWidgets.QWidget):
    def __init__(self, widget_to_style):
        ...
        btn.clicked.connect(lambda: self.setStyleSheet(qtstylish.light(hot_reload=True)))
        btn_layout.addWidget(btn)

        btn = QtWidgets.QPushButton("Dark")
        btn.clicked.connect(lambda: self.setStyleSheet(qtstylish.dark(hot_reload=True)))
        btn_layout.addWidget(btn)
