...
<template>
  <div class="socket"
       :class="[type, socket.name, used ? 'used' : '', incompatible ? 'incompatible' : '']"
...
export default {
  ...
  methods: {
    mouseenter() {
      this.editor.trigger('socketenter', this.socket);
    },
    mouseleave() {
  ...
  mounted() {
    this.editor.on('socketenter', (socket) => {
      if (this.type === 'input') {
        this.incompatible = !socket.compatibleWith(this.socket);
      } else {
        this.incompatible = !this.socket.compatibleWith(socket);
      }
    });
    this.editor.on('socketleave', () => this.incompatible = false);
  },
};
</script>
