...
class ReportOne:
    """
    Eval클래스와 Score클래스를 같이 계산하여 다양한 방식의 출력구조에 맞춰 형식을 반환한다.
    즉 Eval과 Score를 외부에서 직적 사용할 필요없이 ReportOne클래스를 사용한다.
    """
    seperate_line = '\n' + ('-' * 65) + '\n'
...
def make_eval_df_all() -> pd.DataFrame:
    ...
    from stock_core.load_db import Settings
    setting = Settings(echo=True)
    setting.set_mongo(mongo_addr='mongodb://192.168.0.173:27017')
