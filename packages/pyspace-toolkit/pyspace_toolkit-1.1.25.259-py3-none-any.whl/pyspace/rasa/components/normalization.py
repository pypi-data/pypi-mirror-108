...
class EntityNormalization(Component):
    ...
    def process(self, message: Message, **kwargs: Any):
        ...
        tokens = message.get(TOKENS_NAMES[TEXT])
        if self.component_config["normalization_write_attribute"] is not False and isinstance(tokens, list):
            message.set(self.component_config["normalization_write_attribute"], " ".join([token.text for token in tokens]), add_to_output=True)
