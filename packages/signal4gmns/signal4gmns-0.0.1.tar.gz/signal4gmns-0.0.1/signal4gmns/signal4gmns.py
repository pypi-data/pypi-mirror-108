...
class CSignalNode:
    ...
    def Calculate_the_x_c_Output(self):
        ...
        self.x_c_output = (self.y_StageMax * self.cycle_length) / (self.cycle_length - self.l_value)
    ...
    def Calculate_Green_Time_for_Stages(self):
        ...
        for s in range(1, self.stage_Actual_Size + 1):
            self.green_Time_Stage_Array[s] = max(self.minGreenTime, self.y_Max_Stage_Array[s] * self.cycle_length / self.y_StageMax)
...
def Read_Movement_and_Node_Files():
    ...
    loggings.info("Step 1.0: Configuration", 2)
        ...
        if len(movementStr) > 0 and movementStr != 'nan':
            if main_node_id !='nan':
                if main_node_id not in g_node_map.keys():
                    ...
                    g_node_map[main_node_id] = CSignalNode(main_node_id,xcoord,ycoord,_config_Parameters,reference_cycle_length)
...
def Get_Signal_Timing_Movement_Info():
    ...
    for node_id, sn in g_node_map.items():
        ...
        cycle_time_in_sec = int(max(10, sn.cycle_length, max_green_time))
...
def Output_Singal_Timing_Movement_Files():
    ...
    signal_phase_mvmt_list_df=pd.DataFrame(signal_phase_mvmt_list,
                                             columns=['signal_phase_mvmt_id',  'controller_id','mvmt_txt_id',
                                                      'signal_phase_num','stage_no', 'timing_plan_id','mvmt_id','node_id','osm_node_id', 'ib_link_id',
...
def Output_Intermediate_Files(label_name):
    ...
    for osmID,signal_Node in g_node_map.items():
        ...
        new_Output_Node=CSignal_Node_Inter(signal_Node_Code,osmID, x_coord=signal_Node.xcoord, y_coord=signal_Node.ycoord,reference_cycle_length=signal_Node.reference_cycle_length)
    ...
    signal_Node_info_List_df=pd.DataFrame(signal_Node_info_List,
                                             columns=['signal_code','osm_node_id',  'x_coord',
                                                      'y_coord','reference_cycle_length'])
    signal_Movement_info_List_df=pd.DataFrame(signal_Movement_info_List,
                                             columns=['movement_code',  'signal_node_code',
                                                      'signal_link','stageNo_in_Order','osm_node_id','node_id','groupNo', 'directionNo','ib_link_id', 'ob_link_id','ib_osm_node_id','ob_osm_node_id','lanes', 'mvmt_txt_id',
                                                      'volume','geometry'])
...
def Input_Intermediate_Files(label_name):
    ...
    parser = pd.read_csv(f"signal_node_{str(label_name)}_log.csv",converters = {'osm_node_id':str})
        ...
        if len(movementStr) > 0 and movementStr != 'nan':
            if osm_node_id !='':
                if osm_node_id not in g_node_map.keys():
                    loggings.info("Step 2.1.1: Reading intermediate file node.csv...", 2)
                    ...
                    g_node_map[osm_node_id] = CSignalNode(osm_node_id,xcoord,ycoord,_config_Parameters,reference_cycle_length)
