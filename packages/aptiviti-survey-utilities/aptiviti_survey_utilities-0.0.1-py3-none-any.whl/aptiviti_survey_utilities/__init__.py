...
class survey_utilities:
    ...
    def get_tsis_fundamentals(self, vendors = None, industries = None, metrics = ['ADOPTION', 'REPLACING', 'INCREASE', 'DECREASE', 'FLAT'], limit=None):
        ...
        if vendors == None:
            ...
            vendor_query = f"[Vendor] in ('{vendor_list}') AND "
        ...
        FROM
            [Survey].[dbo].[TSIS_Response_Warehouse] rw
        INNER JOIN
            [Survey].[dbo].[TSIS_Demographics_Warehouse] dw
            ON CONCAT(dw.[First],dw.[Last]) = CONCAT(rw.[First],rw.[Last])
    ...
    def calculate_fundamental_metrics(self, survey_data, pivot_index=['period'], pivot_columns=['Metric'], percent_change=False):
        ...
        merged_data['period'] = merged_data['period'].dt.to_period('Q')
        ...
        pivoted = merged_data.pivot_table(index=pivot_index, columns=pivot_columns)
