...
def start_multi_process(function, iterable, processes=None, nestable=False):
    ...
    try:
        for i in tqdm(p.imap_unordered(worker, iterable),
...
def handle_multi_process_write_to_file_and_return_results(
        function, results_function, iterable, path, processes=None):
    with open(path, 'w') as f:
        f.write("[\n")
        try:
            p = Pool(processes=get_no_of_processes(processes),
                     initializer=worker_init, initargs=(function,))
            for result in tqdm(p.imap_unordered(
                    worker, iterable), total=len(iterable), colour=tanuki):
