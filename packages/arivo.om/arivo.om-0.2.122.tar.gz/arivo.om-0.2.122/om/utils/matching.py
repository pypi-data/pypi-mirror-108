...
@related.immutable
class MatchingUtilsConfig(object):
    directory = related.StringField(required=False, default="/data/schemes/schemes")
    revision_file = related.StringField(required=False, default="/data/schemes/revision")
    default_scheme = related.StringField(required=False, default="LEGACY")
    default_version = related.IntegerField(required=False, default=0)
    default_country_order = related.SequenceField(str, required=False, default=[])
    legacy_in_default_country_order = related.BooleanField(required=False, default=False)
    edit_distance = related.IntegerField(required=False, default=1)
    use_alternatives = related.BooleanField(required=False, default=False)
    only_best = related.BooleanField(required=False, default=True)
    add_other_country_always = related.BooleanField(required=False, default=True)
...
class PlateYamlSelector(object):
    def __init__(self, yaml_dir, revision_file="", default_country_order=None, legacy_in_default_country_order=False,
                 always_add_other_country_to_plate_for_matching=True, priority_schemes=()):
        if default_country_order is None:
        ...
        self.priority_schemes = [{"code": scheme, "confidence": 1.0} for scheme in priority_schemes]
        self.schemes = {}
    ...
    def create_plate_for_matching(self, plate, country="OTHER", plate_confidence=0.5, country_confidence=0.5,
        ...
        if alternatives is None:
            alternatives = []
        country = self.priority_schemes + [{"code": country, "confidence": country_confidence}]
    ...
    def create_plate_for_matching_from_medium_message(self, medium_payload):
        """
        If given medium message is of type 'lpr' then a valid plate for `match_plate` will be generated.
        :param medium_payload: medium message
        :return: valid plate for `match_plate`
        """
        if medium_payload.get("type", "").lower() == "lpr":
            country = medium_payload.get("country", {})
            return self.create_plate_for_matching(
                str(medium_payload.get("id", "")), country.get("code", "OTHER"),
                medium_payload.get("confidence", 0.5), country.get("confidence", 0.5),
                medium_payload.get("alternatives")
            )
        return {}
