...
class ValidateRedshiftGPU(Validator):
    def run(self, layername):
        ...
        if "redshift" != current_renderer:
            return
        name = self._submitter.attr("instanceTypeName").get()
        instance_type = next(
            (it for it in coredata.data()["instance_types"] if it["name"] == name), None
        )

        description = instance_type["description"] if instance_type else "undefined"
...
class ValidateFailedRetries(Validator):
    def run(self, _):
        attr = self._submitter.attr("retriesWhenFailed")
        retries = attr.get()
        if retries > 0:
            self.add_warning(
                "You have 'Retries-when-Failed' set above 0. Most render failures are not solved by retrying. We recommend you set this setting to 0 and if the render fails, try to troubleshoot or contact support."
            )
