...
class conductorRender(om.MPxNode):
    ...
    def make_retries_atts(cls):
        ...
        cls.aRetriesWhenFailed = nAttr.create(
            "retriesWhenFailed", "rwf", om.MFnNumericData.kInt, 0
        )
        nAttr.writable = True
        nAttr.keyable = True
        nAttr.storable = True
    ...
    def get_context(self, data):
        ...
        try:
            context["WorkspacePath"] = Path(pm.workspace(
                q=True, rd=True)).fslash(with_drive=False)
        except BaseException:
            pass

        try:
            context["OutputPath"] = Path(data.inputValue(
                self.aDestinationDirectory).asString().strip()).fslash(with_drive=False)
        except BaseException:
            pass
    ...
    def get_destination_directory(cls, data):
        ...
        """
        output_path = data.inputValue(
            cls.aDestinationDirectory).asString().strip()
