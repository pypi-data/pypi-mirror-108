...
class OptionsHandler(HandlerMixin):
    def __init__(self, strategy):
        ...
        * adaptive:
             Updating coefficients over the iteration.
    ...
    def adaptive(self, start_opts, opt, **kwargs):
        """Adaptive hyperparameter. Starting with a strong c1, strong w, and weak c2 to increase the
           exploration of the search space, we want to tend towards a weak c1, weak w, and strong c2 to
           exploit the best results after exploration by converging towards the global minimum.

           Ref: G. Sermpinis, K. Theofilatos, A. Karathanasopoulos, E. F. Georgopoulos, & C. Dunis,
        ...
        try:
            ...
            n = kwargs["itermax"]
            if opt == 'w':
                new_val = (0.4/n**2) * (t - n) ** 2 + 0.4
            elif opt == 'c1':
                new_val = -3 * t / n + 3.5
            elif opt == 'c2':
                new_val = 3 * t / n + 0.5
            else:
                self.rep.logger.exception("Invalid opt" +  opt)
        except KeyError:
