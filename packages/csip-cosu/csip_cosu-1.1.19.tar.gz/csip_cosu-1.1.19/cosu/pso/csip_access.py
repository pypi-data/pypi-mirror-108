...
def csip_worker(reqq: queue.Queue, thread_no: int, stop, full_trace,
                url, files, arg_params, conf: Dict) -> None:
    ...
    while not stop():
        try:
            (r, s, i, i_particle, x, step_param_names, calib_params, objfunc, resq) = reqq.get(True, 0.5)
            ...
            try:
                ...
                if save_resp:
                    res.save_to(os.path.join(save_resp, 'r{}s{}i{}p{}'.format(r, s, i, i_particle) + '.json'))
