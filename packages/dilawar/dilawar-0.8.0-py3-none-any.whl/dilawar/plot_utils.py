...
def nx_draw(graph, ax=None, edge_labels: Optional[str] = None, **kwargs):
...
def _matrix_plot(img, xvec, yvec, ax=None, **kwargs):
    ...
    if not xticks:
        ...
        xticks = [(i, xvec[int(i)]) for i in np.linspace(0, len(xvec), nticks)[:-1]]
    ...
    if not yticks:
        ...
        yticks = [(i, yvec[int(i)]) for i in np.linspace(0, len(yvec), nticks)[:-1]]
...
def pgfplots(df, xname, yname, ax, **kwargs):
    ...
    defaultLegendOptions = dict(
        bbox_to_anchor=(1, 1), loc="upper left", fontsize="x-small"
    )
