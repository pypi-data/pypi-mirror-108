...
class ConfReg():
    def __init__(self):
        self._run_mode = ADS1018.STOP
        self._mux_mode = ADS1018.DIFF_0_1
        self._fsr_value = ADS1018.FSR_2048
        self._acq_mode = ADS1018.SINGLE_SHOT
        self._acq_rate =ADS1018.RATE_1600SPS
        self._adc_mode = ADS1018.ADC_MODE
        self._input_mode = ADS1018.INT_PULLUP
        self._cfg_mode = ADS1018.VALID_CFG
    ...
    def get_reg_values(self):
        return [ADS1018.RESERVED, self._cfg_mode, self._input_mode, self._adc_mode, self._acq_rate, self._acq_mode,
                           self._fsr_value, self._mux_mode, self._run_mode]

    def _compose_reg_bits_list(self):
        config_reg_bits_lists = [_itoblist(ADS1018.RESERVED, 1),_itoblist(self.cfg_mode, 2),_itoblist(self.input_mode, 1),
...
if __name__ == '__main__':
    conf_reg = ConfReg()
    conf_reg.run_mode = ADS1018.START_NOW
    conf_reg.acq_mode = ADS1018.CONTINUOUS
    conf_reg.adc_mode = ADS1018.TEMP_MODE
