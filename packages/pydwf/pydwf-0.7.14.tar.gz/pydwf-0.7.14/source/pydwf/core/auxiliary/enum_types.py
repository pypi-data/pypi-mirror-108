...
class DwfEnumFilter(_FixReprMethod, enum.Enum):
    ...
    This type is used by the |DwfLibrary.deviceEnum.enumerateDevices:link| method to constrain
...
class DwfDeviceID(_FixReprMethod, enum.Enum):
    ...
    This type is used by the |DwfLibrary.deviceEnum.deviceType:link| method to report on a selected device.
...
class DwfDeviceVersion(_FixReprMethod, enum.Enum):
    ...
    This type is used by the |DwfLibrary.deviceEnum.deviceType:link| method to report the
...
class DwfEnumConfigInfo(_FixReprMethod, enum.Enum):
    """Enumeration type for configuration parameter type constants.

    This type lists the device parameters that can vary between different configurations
    of the same device.
    ...
    The user can specify which device configuration to use when opening the device; the configuration
    is then fixed during use, until the device is closed.

    For many use cases, the default configuration that provides a balanced tradeoff works fine. User
    programs that need more control can use the |DwfLibrary.deviceEnum.configInfo:link| method during
    device enumeration to inspect the different configurations available, and use the information
    obtained to pick the best configuration.
...
@enum.unique
class DwfTriggerType(_FixReprMethod, enum.Enum):
...
@enum.unique
class DwfTriggerLengthCondition(_FixReprMethod, enum.Enum):
...
@enum.unique
class DwfParameter(_FixReprMethod, enum.Enum):
