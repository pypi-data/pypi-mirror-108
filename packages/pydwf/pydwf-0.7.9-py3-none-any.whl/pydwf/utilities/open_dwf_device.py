...
def open_dwf_device(
            ...
            configuration_fitness_func: Optional[Callable[[Dict[DwfEnumConfigInfo, Any]], Optional[int]]] = None
        ) -> DwfDevice:
    ...
    1. The first step this function performs is to select a device for opening.
       ...
       (for example, only Analog Discovery 2 devices, or Dgitial Discovery devices). If omitted (the default),
    ...
    2. The next step is to select a *device configuration* for the selected device.
       ...
       If no *configuration_fitness_func* is provided, this default configuration will be used.

       If a *configuration_fitness_func* is provided, it should be a function (or lambda expression) that
       ...
       that reflects the 'desirability' or 'fitness' of that particular configuration for the task at hand.
       ...
       *configuration_fitness_func* does not return None), it will select the configuration with the highest score.
       ...
       .. code-block:: python
          ...
          from pydwf.utilities import open_dwf_device

          def maximize_analog_in_buffer_size(config_parameters):
              return config_parameters[DwfEnumConfigInfo.AnalogInBufferSize]

          with open_dwf_device(dwf,
                    configuration_fitness_func = maximize_analog_in_buffer_size) as device:
    ...
    Parameters:
        ...
        configuration_fitness_func (Optional[Callable[[Dict[DwfEnumConfigInfo, Any]]):
            A function to score the configurations of the selected device.
    ...
    if configuration_fitness_func is None:
        # If no 'configuration_fitness_func' was specified, just open the first (default) one.
    ...
    for configuration_index in range(num_config):
        ...
        configuration_info = {
            configuration_parameter: dwf.deviceEnum.configInfo(configuration_index, configuration_parameter)
