...
class DynamoDBClient(MLSClient):
    ...
    def put_item(self, table_name: str, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._MLSClient__runtime_env == MLSRuntimeENV.MMS:
            table = dynamodb.Table(table_name)
            try:
                if table_name.startswith("user-profile") and not table_name.endswith("nogzip"):
                    compressed = gzip.compress(json.dumps(item).encode())
                    item = {
                        "user_id": item["user_id"],
                        "values": Binary(compressed),
                        "ttl": int(time() + 90 * 24 * 60 * 60),
                    }
                elif table_name.startswith("profile-") and not table_name.endswith("nogzip"):
                    compressed = gzip.compress(json.dumps(item).encode())
                    item = {"id": item["id"], "values": Binary(compressed), "ttl": int(time() + 90 * 24 * 60 * 60)}
                elif table_name.startswith("user-profile") or table_name.startswith("profile-"):
