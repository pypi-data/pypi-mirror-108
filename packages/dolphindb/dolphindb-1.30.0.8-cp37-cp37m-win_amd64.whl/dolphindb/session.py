...
class session(object):
    ...
    def subscribe(self, host, port, handler, tableName, actionName="", offset=-1, resub=False, filter=None, batchSize=0, throttle=1):
        if filter is None:
            filter = np.array([],dtype='int64')
        if batchSize > 0:
            self.cpp.subscribeBatch(host, port, handler, tableName, actionName, offset, resub, filter, batchSize, throttle)
        else:
