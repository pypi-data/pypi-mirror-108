...
def _orca_window_op(func, numeric_only, use_moving_template=False):
    def wfunc(self):
        # print(func)
        if func in ['skew','kurtosis','quantile',]:
...
class WindowOpsMixin(metaclass=abc.ABCMeta):
    ...
    quantile = _orca_window_op("quantile", numeric_only=False)
...
class Rolling(_InternalAccessor, WindowOpsMixin, metaclass=abc.ABCMeta):
    ...
    def __init__(self, session, internal, index, window, min_periods, on, where_expr, name, rolling_on_temporal=None):
...
class GroupbyRolling(Rolling):
    ...
    def __init__(self, session, internal, index, window, min_periods, on, where_expr, name, groupby_list, is_groupby,groupkeys,rolling_on_temporal=None):
        self._session = session
        self._internal = internal
        self._index = index
        self._on = on
    ...
    def __getitem__(self, key):
        ...
        new_odf = self._internal[key]
        return klass(self._session, new_odf, self._index, self._window, self._min_periods,
                     self._on, self._where_expr, name, self._groupby_list,self._is_groupby, self._groupkeys,self._rolling_on_temporal)
