...
class App:
	...
	def route(self, regex, method="GET", name='', description='', query_class=None, request_class=None, response_class=None):
		def _decorator(func):
	...
	def handle_wsgi(self, environ, start_response):
		...
		for regex, method_handler in self.request_handlers:
			...
			func = method_handler.get(environ["REQUEST_METHOD"].upper()) or method_handler.get("GET")
	...
	def start(self, port=80, handlers=[], use_wsgi=False, **ssl_args):
		...
		for handler in handlers:
			if(isinstance(handler, (list, tuple))):
				if(len(handler) < 2 or len(handler) > 3):
					continue
				handlers_with_methods.append({"regex": handler[0], "func": handler[1], "method": "GET"})
			elif(isinstance(handler, dict)):
				_handler = {"method": "GET"} # defaults
		...
		for handler in reversed(self.route_handlers):
			...
			method_handler_exists = existing_regex_method_handlers.get(method)
			if(method_handler_exists):
				existing_regex_method_handlers[method] = handler
			else:
				regexes_map[regex] = existing_regex_method_handlers = {method: handler}
			...
			try:
				...
				for regex, method_handlers in self.request_handlers:
					...
					if(args != None):
						...
						if(not handler and request_type != "GET"):
