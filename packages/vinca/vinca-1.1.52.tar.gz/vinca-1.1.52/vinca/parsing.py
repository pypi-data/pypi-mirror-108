...
def deck_type(arg):
	...
	assert arg in deckdict.keys(), '\n\n\t  Invalid deck. Use [d] to list decks'
	return deckdict[arg]
def card_type(arg):
	assert arg.isdigit()
	assert int(arg) in collection.id_list, 'Invalid card id. Use [q] to search for a card.' 
...
parser.set_defaults(decks = [], cards = [], decks_or_cards = [], func = 'statistics')
subparsers = parser.add_subparsers()
# commands which take a deck as an argument
one_liner_parser = subparsers.add_parser('one_liner',aliases=['1'], help='add a basic card quickly')
one_liner_parser.add_argument('decks',type=deck_type,nargs='*')
one_liner_parser.set_defaults(func = 'one_liner')

linear_add_parser = subparsers.add_parser('linear_add',aliases=['l'], help='for lyrics, poetry, oratory, etc.')
...
query_parser.add_argument('-i', '--id_only',action='store_true',help='only output the id of the match')
...
delete_parser.add_argument('-t','--toggle_delete',action='store_true',help='restore a card if it was previously deleted')
...
backup_parser = subparsers.add_parser('backup',aliases=['b','export'], help='backup all cards')
