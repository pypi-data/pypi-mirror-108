...
class Engine:
    ...
    def run_pipeline(
        ...
        **options
        ...
        self.runtime.pre_prepare_run_pipeline(pipeline, root_pipeline_data, root_pipeline_context, **options)

        process_id = self.runtime.prepare_run_pipeline(pipeline, root_pipeline_data, root_pipeline_context, **options)
        # execute from start event
        self.runtime.execute(process_id, pipeline["start_event"]["id"])

        self.runtime.post_prepare_run_pipeline(pipeline, root_pipeline_data, root_pipeline_context, **options)
    ...
    def schedule(
        ...
        try:
            ...
            with self._schedule_lock_keeper(schedule_id):
                ...
                schedule_result = handler.schedule(process_info, state.loop, schedule, callback_data)
