...
class Publish:
    ...
    def setup(self, add_arg):
        add_arg("rule", choices=["patch", "minor", "major"],
    ...
    def run(self, stdin, log):
        try:
            ...
            return 1
        try:
            print(sh.poetry("run", "pytest"))
        except sh.ErrorReturnCode_5:
            pass
        env = os.environ.copy()
        env.update(NO_COLOR="1", GH_PAGER="cat")
        private = json.loads(str(sh.gh("repo", "view", "--json",
                                       "isPrivate", _env=env)))["isPrivate"]
        ...
        print(sh.git("push"))
        asset = str(sh.tail(sh.grep(sh.ls("-1tr", "dist"), ".whl"),
                    "-n1")).strip()
        try:
            previous_version_string = str(sh.git("describe", "--tags",
                                                 "--abbrev=0")).strip() + ".."
        except sh.ErrorReturnCode_128:
            previous_version_string = ""
        changelog = sh.git("--no-pager", "log", "--no-color",
                           f"{previous_version_string}HEAD^", "--oneline",
                           "--no-decorate")
        print(sh.gh("release", "create", version, f"dist/{asset}",
                    "--notes", changelog))
        print(sh.git("pull"))
