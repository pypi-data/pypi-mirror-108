...
class ImacControllerRts:
    ...
    async def rotary_switch_update(self, data, timestamp):
        if data + 1 != self.logical_number:
            log.warning(
                f"{self.device_id} has has the logical number changed from {self.logical_number} to {data + 1}. Restarting pyaware"
            )
            pyaware.stop()
    ...
    async def serial_number_update(self, data, timestamp):
        if data != self.serial_number:
            log.warning(
                f"{self.device_id} has has the serial number changed from {self.serial_number} to {data}. Restarting pyaware"
            )
            pyaware.stop()
    ...
    async def fieldbus_number_update(self, data, timestamp):
        if data != self.fieldbus_address:
            log.warning(
                f"{self.device_id} has has the fieldbus number changed from {self.fieldbus} to {data}. Restarting pyaware"
            )
            pyaware.stop()
