...
def build_description(name: str, metrics: Dict[str, float]) -> str:
    ...
    if metrics:
        ...
        for metric_name in metrics:
            desc += f"{metric_name}: {round_to_n(metrics[metric_name], 4):6}, "
        desc = desc[:-2]
...
class PytorchTrainer:
    ...
    def __init__(
        ...
        Args:
            ...
            save_mode ("never"|"always"|"best"): When to do checkpoints.
                ...
                best: Keep only the best checkpoint (best.ckpt)
                all: A checkpoint is made for each epoch ({epoch}.ckpt). A best checkpoint is also done.
        """
        self.model = model
    ...
    def train(
        ...
        while self.epoch < n_epochs:
            ...
            # Use val metrics is possible (and only the avg train loss from train loop)
            metrics["Avg Train Loss"] = loss_cum / epoch_size  # Assume that batch are evenly sized
            print(
                self.epoch_description.format(
                    desc=build_description(f"Epoch {self.epoch}", metrics), step=self.epoch + 1, total_step=n_epochs,
                ),
                flush=True,
            )
