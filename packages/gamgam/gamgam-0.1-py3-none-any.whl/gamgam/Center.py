...
class ProjectConsole:
    ...
    def Log(data: str):
        ...
        if Type == Vector2:
            ...
            print(f"position: ({data.position.x}, {data.position.x}), rotation: {data.rotation}")
...
global Input, Console, StandardColor, default_icon
Input = InputManager()
Console = ProjectConsole()
StandardColor = Color(is_standard=True)
default_icon = Image(os.path.abspath(__file__)[0: len(os.path.abspath(__file__)) - 9] + "Default_Icon.PNG")
...
def run(fps: int = 60):
    global isGameRun, NowCamera, Objects
    if len(Scenes) <= 0:
...
def find_GameObject(game_object_name: str):
    for i in GameObjects[SceneNow]:
        if i.name == game_object_name:
            return i


def find_GameObject(game_object_id: str):
    for i in GameObjects[SceneNow]:
        if i.id == game_object_id:
            return i


def set_GameObject(game_object_id, game_object):
    for i in range(len(GameObjects[SceneNow])):
        if GameObjects[SceneNow][i].id == game_object_id:
            GameObjects[SceneNow][i] = game_object


def find_GameObjects(game_object_name: str):
    re_game_object = []
    for i in GameObjects[SceneNow]:
        if i.name == game_object_name:
...
def find_GameObjects(game_object_id: str):
    re_game_object = []
    for i in GameObjects[SceneNow]:
        if i.id == game_object_id:
            re_game_object.append(i)
...
def find_GameObject_Num(game_object_id: str):
    for i in range(len(GameObjects[SceneNow])):
        if GameObjects[SceneNow][i].id == game_object_id:
            return i
