...
class GameSoundManager:
    ...
    def shoot(self, name: str, oop:int = 0, max_time:int = 0, fade_ms:int = 0, volume:float = 0.5):
    ...
    def out(name: str, milliseconds:int = 0):
        if name in self.effect:
            if milliseconds == 0:
            ...
            Console.Error("No Effect loaded")
    def stop(name: str, milliseconds:int = 0):
        if name in self.background:
            if milliseconds == 0:
                mixer.music.stop()
            else:
...
global Input, Console, StandardColor, default_icon, SoundManager
Input = InputManager()
