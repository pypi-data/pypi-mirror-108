...
class GameObject:
    def __init__(self, name: str, images: Image = Image(os.path.dirname(os.path.realpath(__file__)) + "\\Default_Icon.PNG"), size: Vector2 = Vector2(10, 10),
                 transforms: Transform = Transform(), order: int = 1):
        if images == Image(os.path.dirname(os.path.realpath(__file__)) + "\\Default_Icon.PNG"):
            images = Image = Image(os.path.dirname(os.path.realpath(__file__)) + "\\Default_Icon.PNG")
        self.name = name
        self.order = order
        self.id = makeId()
        self.image = transform.scale(images.img, (size.x, size.y))
        self.size = size
        ...
        self.object = transform.rotate(self.image, self.transform.rotation)
        self.object = transform.scale(self.object, (self.size.x, self.size.y))
    ...
    def crash_event(self):
        ...
        for i in GameObjects[now_scene()]:
            if i.type == "GameObject":
                if i != self:
                    if i.transform.position.x - (i.size.x / 2) <= self.transform.position.x + (
                            self.size.x / 2) <= i.transform.position.x + (i.size.x / 2):
                        if i.transform.position.y + (i.size.y / 2) >= self.transform.position.y - (
                                self.size.y / 2) >= i.transform.position.y - (i.size.y / 2):
                            self.crashed_direction.append(Vector2(1, -1))
                            self.crashed_object.append(i)
                            self.is_crashed = True
                        elif i.transform.position.y - (i.size.y / 2) <= self.transform.position.y + (
                                self.size.y / 2) <= i.transform.position.y + (i.size.y / 2):
                            self.crashed_direction.append(Vector2(1, 1))
                            self.crashed_object.append(i)
                            self.is_crashed = True
                    elif i.transform.position.x + (i.size.x / 2) >= self.transform.position.x - (
                            self.size.x / 2) >= i.transform.position.x - (i.size.x / 2):
                        if i.transform.position.y + (i.size.y / 2) >= self.transform.position.y - (
                                self.size.y / 2) >= i.transform.position.y - (i.size.y / 2):
                            self.crashed_direction.append(Vector2(-1, -1))
...
class Button:
    def __init__(self, name: str, button_text: TextBox = TextBox("NewButtonTextBox", "Button", 32),
                 ...
                 button_image: Image = Image(
                     os.path.dirname(os.path.realpath(__file__)) + "\\Default_Button.png")
                 , order: int = 1):
        self.name = name
        self.id = makeId()
        self.order = order
        self.text_id = button_text.id
        define_GameObject(button_text)
        ...
        GameObjects[now_scene()][find_GameObject_Num(self.text_id)].transform = self.transform
        GameObjects[now_scene()][find_GameObject_Num(self.text_id)].order = self.order - 1
    ...
    def Show(self):
        if self.isClicked is True or self.isClicking is True:
            if not (self.transform.position.x - (
                    self.size.x / 2) <= Input.mouse_position.x <= self.transform.position.x + (
                            self.size.x / 2)):
                self.isClicked = False
                self.isClicking = False
            if not (self.transform.position.y - (
                    self.size.y / 2) <= Input.mouse_position.y <= self.transform.position.y + (
                            self.size.y / 2)):
                self.isClicked = False
                self.isClicking = False
        ...
        if Input.GetKey("MOUSE_LEFT") and self.isClicking is False:
            if self.transform.position.x - (
                    self.size.x / 2) <= Input.mouse_position.x <= self.transform.position.x + (
                    self.size.x / 2):
                if self.transform.position.y - (
                        self.size.y / 2) <= Input.mouse_position.y <= self.transform.position.y + (
                        self.size.y / 2):
                    self.isClicked = True
...
class InputField:
    def __init__(self, name: str, input_text: TextBox = TextBox("NewInputFiledTextBox", "", 32),
        ...
        self.isClicked = False
        GameObjects[now_scene()][find_GameObject_Num(self.text_id)].content = self.text
        GameObjects[now_scene()][find_GameObject_Num(self.text_id)].transform = self.transform
        GameObjects[now_scene()][find_GameObject_Num(self.text_id)].order = self.order - 1
        self.object = transform.rotate(self.image.img, self.transform.rotation)
        self.object = transform.scale(self.object, (self.size.x, self.size.y))
        ...
        if self.isClicked:
            if Input.GetKeyUp("MOUSE_LEFT"):
                ...
                if self.isClicked is False:
                    return
            if Input.GetKey("LSHIFT") or Input.GetKey("RSHIFT") or self.CapsLock:
