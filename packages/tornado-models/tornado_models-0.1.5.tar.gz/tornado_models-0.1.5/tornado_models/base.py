...
class BaseRequestHandler(RedisMixin, SessionMixin, RequestHandler):
    ...
    def initialize(self):
        self.session_config = self.settings
        return super(BaseRequestHandler, self).initialize()
...
class BaseDBModel(SessionMixin):
    def __init__(self, db:SQLAlchemy=None):
        if db is not None: self.session_config['db'] = db
        super(BaseDBModel, self).__init__()


class BaseRedisModel(RedisMixin):
    def __init__(self, redis:Redis=None):
        if redis is not None: self.session_config['redis'] = redis
