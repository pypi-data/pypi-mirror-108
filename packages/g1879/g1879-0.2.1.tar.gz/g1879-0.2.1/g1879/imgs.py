...
def img_to_jpg(path: str, del_src: bool = True) -> None:
    ...
    base_name = get_valid_name(str(folder), f'{img_file.stem}.jpg')
...
def zoom_img(path: str, new_x: int = None, new_y: int = None, rename: bool = False):
    ...
    # 因不支持中文路径，保存也要用复杂的方式
    cv2.imencode(ext_name, new_img)[1].tofile(f'{folder_path}\\{name}{ext_name}')
...
def crop_img(path: Union[str, Path], lit_x: int = None, lit_y: int = None, img_name: str = None):
    ...
    # 读取图像文件，获取xy值
    img = cv2.imdecode(np.fromfile(to_path, dtype=np.uint8), -1)
    y, x = img.shape[:2]
    ...
    # 保存图片
    cv2.imencode(ext_name, cropped)[1].tofile(to_path)
