...
class QueryMerchantRewardByDateRange(OutletRewardingQueryBase):
    def prepare_query(self, **kwrgs):
        ...
        query = '''
            SELECT FORMAT_DATETIME('%Y-%m-%d', RewardedDateTime) as date_range, RewardFormatKey, sum(TotalRewardAmount) as sumRewardAmount, count(transactionId) as transactionCount
            FROM (
            
                SELECT transactionId, RewardedDateTime, RewardFormatKey, SUM(RewardAmount) as TotalRewardAmount
                        FROM (
                    ...
                    GROUP BY transactionId, RewardedDateTime, RewardFormatKey
            )
            GROUP BY date_range, RewardFormatKey    
            order by date_range        
