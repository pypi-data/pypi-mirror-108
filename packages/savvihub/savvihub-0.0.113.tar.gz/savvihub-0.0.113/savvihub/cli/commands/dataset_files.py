...
def ls(
    ...
    dataset_full_name: str = typer.Argument(..., metavar='DATASET',
                                            help='{organization}/{dataset}@{snapshot} format '
                                                 'where organization and snapshot are optional.'),
    ...
    """
    organization_name_override, dataset_name, snapshot_name = parse_dataset(dataset_full_name)
    ...
    if snapshot_name != 'latest':
        try:
            ...
            raise ExitException(f'Invalid dataset snapshot: {dataset_full_name}\n'
                                f'Please check your dataset and snapshot exist in organization `{organization_name}`.')
    volume_file_copy(
        ctx,
...
def cp(
    ...
    """
    organization_name_override, dataset_name, src_snapshot_name = parse_dataset(dataset_full_name)
    if organization_name_override:
        organization_name = organization_name_override
