...
class ParametersCOSIM:
    ...
    def makeGenericIOPortFiles(self, CoilSections, ModelFolder, CoSimFolder: str, PSpiceExecutable: str, LEDETExecutable: str, verbose: bool = False,
                               ...
                               convergenceElement = 'x_mag_1', CombinedModel = 0):
        ...
        self.t0 = t_0
        self.tEnd = t_end
        self.tStepMax = t_step_max
        # Components which are used in the Ports
        if not convergenceElement: convergenceElement = 'x_mag_1'
        ...
        # If CoilSection-Variable is anti-symmetrical, it's assumed to represent the whole Simulation-Sections
        split = 0
        if len(CoilSections)>1:
            for j in range(len(CoilSections)):
                tempLength = len(CoilSections[j])
                for i in range(len(CoilSections)):
                    if len(CoilSections[i]) != tempLength:
                        split = 1

        # Variables used for convergence, for now its the first magnet current and the LEDET Voltages
        if N_LEDET == 1:
            convergenceVariables = ["I(" + convergenceElement + "." + Components[0] + "_1)", "U_inductive_dynamic_" + Components[1]+"_1"]
        else:
            convergenceVariables = ["I(" + convergenceElement + "." + Components[0] + "_1)", "U_inductive_dynamic_" + Components[1]+"_1"]
        ...
        if N_LEDET == 1:
            ...
            if len(SimulationNumber) != N_LEDET:
                ...
                if verbose: print("Simulation Number 0 is used for all LEDET models")
            for i in range(1,N_LEDET+1):
                ...
                with open(filename, 'r') as f:
                    ...
                    if executionOrder.count(2) > 1:
                        ...
                        idxLEDET2 = LEDETExecutable.find('LEDET_v')
                        idxLEDET3 = LEDETExecutable[idxLEDET:].find('\\')
                        data["solverPath"] = LEDETExecutable[0:idxLEDET + idxLEDET3]+"\\LEDET" + "_"+str(i) + LEDETExecutable[idxLEDET2-1:]
        ...
        if N_LEDET == 1:
            ...
            if split:
                lenCSections = []
                newCSections = []
                for i in range(len(CSections)):
                    for j in range(len(CSections[i])):
                        newCSections.append(CSections[i][j])
                    lenCSections.append(len(CSections[i]))
                CSections = newCSections
            ...
            for l in range(0,N_LEDET):
                ...
                for i in range(len(CoilSections)):  # Number of Ports
                    if split:
                        if i != SplitCount - SectionCount: continue
                        if i>lenCSections[l]-1: continue
                        SplitCount = SplitCount + 1
                        if SplitCount == len(CSections):
                            i = SplitCount-1
                    struct = {}
                    if SplitCount == len(CSections):
                        struct["name"] = "Port_" + str(i + 1) + "_" + str(0)
                    ...
                    struct["components"] = []
                    if split:
                        struct["components"].append(CSections[i])
                    else:
                        for n in range(len(CSections[i])):  # All Coilsections
                            struct["components"].append(CSections[i][n])
                    struct["inputs"] = []
                    for j in range(len(couplingParameterLedet[0])):  # Number of In's per respective Port
                        ...
                        substruct_in["labels"] = []
                        if split:
                            substruct_in["labels"].append(couplingParameterLedet[0][j][0] + "_" + CSections[i])
                        else:
                            for n in range(len(CSections[i])):  # Append all Coilsections as Labels to Input
                                substruct_in["labels"].append(couplingParameterLedet[0][j][0] + "_" + CSections[i][n])
                        substruct_in["types"] = couplingParameterLedet[0][j][1]
                    ...
                    for k in range(len(couplingParameterLedet[1])):  # Number of Out's per respective Port
                        ...
                        if split:
                            if (couplingParameterLedet[1][k][0] == "U"):  # If Output is U
                                substruct_out["labels"].append("U_inductive_dynamic" + "_" + CSections[i])
                            else:  # If output is I or R
                                substruct_out["labels"].append(couplingParameterLedet[1][k][0] + "_" + CSections[i])
                        else:
                            for n in range(len(CSections[i])):  # Append all Coilsections as Labels to Output
                                if (couplingParameterLedet[1][k][0] == "U"):  # If Output is U
                                    substruct_out["labels"].append("U_inductive_dynamic" + "_" + CSections[i][n])
                    ...
                    struct_total.append(struct)
                if split:
                    SectionCount = SectionCount + lenCSections[l]
        ...
        for n in range(0, N_LEDET):
            ...
            if not 'x_mag_' in convergenceElement:
                convEl = convergenceElement
                DM = ''
            else: convEl = convergenceElement[:-1]

            for i in range(len(CoilSections)):  # Number of Ports
                if split and len(CoilSections[n]) != len(CoilSections):
                    if i+1 != n: continue
                for j in range(2):  # 2Ports in PSpice
                    struct = {}
                    struct["name"] = "Port_" + str(i+n*len(CoilSections)+1) + "_" + str(j)
                    if CombinedModel: LSections[i] = Components[0] + "_" + str(i+n*len(CoilSections)+1)
                    struct["components"] = [LSections[i]]
                    Number = LSections[i][-1]
                    struct["inputs"] = []
                    for l in range(len(couplingParameterPSpice[j][0])):  # Number of In's
                        ...
                        if (j == 0):  # Second Port in PSpice is U/U, check on that here
                            ...
                            substruct_in["labels"] = [cPPSpice + "_field_"+M_add + str(Number) + "_stim"+quench]  # First Port for Field
                        else:  # Second Port with circuit
                            substruct_in["labels"] = ["V_circuit_" +M_add+ str(Number) + "_stim"+quench]
                    ...
                    for k in range(len(couplingParameterPSpice[j][1])):  # Number of Out's
                        ...
                        if (j == 0):  # Check again for outputs for which of the 2Ports for Pspice one is in
                            ...
                            substruct_out["labels"] = [
                                cPPSpice + "("+ convEl + str(DM)+ "." + Components[0] + "_" + str(Number) + ")"]
                        else:
                            substruct_out["labels"] = ["V("+convEl + str(DM)+ "." + str(Number) + "_v_l_diff)"]
    ...
    def copyCOSIMfiles(self, nameFileSING, StimulusFile, nameMagnet, N_LEDET=1, LEDET_only = 0, ManuallyStimulusFile = ''):
        ...
        if not LEDET_only:
            ...
            if not StimulusFile.endswith( 'ExternalStimulus.stl'):
                ...
                try:
                    if self.executionOrder[i+1] == num:
                        ...
                        for k in range(len(self.t0)):
                            Snew = Snew + (self.tEnd[k]-self.t0[k])/self.tStepMax[k][0]
                        steps = steps + Snew
                except:
                    TurnNumber = Tnew
                    steps = Snew
    ...
    def prepareSingleLEDETFile(self, file):
        ...
        if 'R_el_ht' not in a.Variables.variableToSaveTxt:
            a.Variables.variableToSaveTxt = np.append(a.Variables.variableToSaveTxt, 'R_el_ht')
            a.Variables.typeVariableToSaveTxt = np.append(a.Variables.typeVariableToSaveTxt, 1)

        a.writeFileLEDET(file, verbose = False)
    ...
    def writeCOSIMBatch(self, COSIMfolder, COSIMexe, Destination = '', LEDET_exe = '', Append: bool = False):
        ...
        if LEDET_exe:
            ...
            stub = LEDET_exe[:idx]+'\\'
        batch_name = f'COSIM_nb_Batch.bat'
        if Destination: fileName = os.path.join(Destination,batch_name)
        else: fileName = batch_name
        if Append: fN = open(fileName, 'a+')
        else: fN = open(fileName, 'w+')
        ...
        print(' ')
        if not Append:
            print('Simulation-Batch containing ',len(COSIMfolder),' simulations created.')
        else:
            print(len(COSIMfolder), ' simulations to batch-file appended.')
        print('Time stamp: ' + str(currentDT))

    def StampBatch(self, Stamp, Folder, Destination):
        bat_core = 'COSIM_nb_Batch'
        bat_ext = ".bat"
        new_bat_name = f"{bat_core}_{Stamp}{bat_ext}"
        new_bat_path = os.path.join(Destination, new_bat_name)
        if os.path.exists(new_bat_path):
            os.remove(new_bat_path)
        os.rename(os.path.join(Destination, f"{bat_core}{bat_ext}"), new_bat_path)
        if Folder != '':
            model_folder = os.path.join(Destination, Folder)
            new_bat_loc = os.path.join(model_folder, new_bat_name)
            if os.path.exists(new_bat_loc):
                os.remove(new_bat_loc)
            shutil.move(new_bat_path, model_folder)