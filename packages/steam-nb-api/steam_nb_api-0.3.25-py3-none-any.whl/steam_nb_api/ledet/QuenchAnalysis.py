...
class QuenchPlanAnalysis():
    ...
    def __init__(self, FileNameTDMS, FileNameSIMData, beforePC = 0.4, verbose: bool = False):
    ...
    def PrepareSimulation_FromMeasurements(self, RefFile, OutputDirectory, MagnetName = '', QHpairs = [[]], nQHseries = 0):
        ...
        if np.any(self.TDMSdata.t_QH):
            if not QHpairs[0] and nQHseries == 0:
                ...
                for i in range(len(QHpairs)):
                    if len(QHpairs[i]) != nQHseries:
                        print("Provided Quench Heater pairs are not symmetric. I'm not sure how to handle this.")
            U0_QH = np.max(self.TDMSdata.U_QH,axis=1)
        ...
        if not np.isnan(self.TDMSdata.t_CLIQ):
            ...
            for i in range(len(a.Inputs.directionCurrentCLIQ)):
                idx_cs = np.where(a.Inputs.GroupToCoilSection == i+1)[0]
        ...
        a.writeFileLEDET(os.path.join(OutputDirectory, MagnetName + '_' + str(0) + '.xlsx'))
    ...
    def __CreateTDMSDataObject(self):
        ...
        self.TDMSdata.U_EE = self.TDMSdata.U_EE[trigger_shoot-1:]
    ...
    def __ExtractElectricalParameters(self):
        ...
        I_crowbar = signal.medfilt(self.TDMSdata.I_DCCT_MF[:,0], kernel_size = 101)
        I_crow = abs(np.sum(I_crowbar[ranges+idx_add:])/len(I_crowbar[ranges+idx_add:]))
        U_remaining = abs((np.sum(self.TDMSdata.U_PC[ranges+idx_add:, 0])-self.TDMSdata.Ud_crow)/len(self.TDMSdata.U_PC[ranges+idx_add:,0]))
        if self.verbose:
            print('PRELIMINARY (unused): Measured Crowbar resistance: ', U_remaining/I_crow)
            print('Measured Ud_crowbar: ', self.TDMSdata.Ud_crow)

        # Find R_EE
        U_EE = signal.medfilt(self.TDMSdata.U_EE, kernel_size=3)
        self.TDMSdata.R_EE = np.max(U_EE)/self.TDMSdata.I_DCCT_MF[np.argmax(U_EE)]
        if self.verbose:
        ...
        if np.any(IQHmax <1):
            ...
            if self.verbose:
                ...
                print('Measured max. U_QH: ', np.max(self.TDMSdata.U_QH,axis=1))
        else:
    ...
    def ExtractFeatures(self, SIM= 1, TDMS=1):
        ...
        if SIM:
            ...
            t_PC_SIM = self._FindFiring(dIdt_Coil, self.SIMdata.TimeFrame)
            print('t_PC_Sim: ', t_PC_SIM)
        if TDMS:
            ...
            self.TDMSdata.t_PC = self._FindFiring(dIdt_Coil, self.TDMSdata.TimeFrame_MF, dI_tol=10000)[0]
            print('t_PC_Meas: ', self.TDMSdata.t_PC)
        # # Extract t_QH
        if SIM:
