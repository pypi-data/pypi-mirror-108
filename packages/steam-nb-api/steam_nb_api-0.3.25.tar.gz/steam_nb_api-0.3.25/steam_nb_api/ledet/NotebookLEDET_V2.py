...
class Notebook_LEDET:
    def __init__(self, nameMagnet, typeMagnet = 'Cos-Theta', verbose = False, recalc_field=True, headerLines =1):
        self.AvailableConductor = ['Type2', 'W1', 'PSU1sn', 'PSU2ti']
        ...
        self.cart_labels = {'x': "x (m)", 'y': "y (m)"}
        self.rad_labels = {'x': "r (m)", 'y': "z (m)"}

        if self.typeMagnet == 'solenoid':
            self.solenoid_data = read_yaml('magnet', self.nameMagnet)
            self.out_dir = os.path.join(Path(os.getcwd()).parent, self.nameMagnet)
        ...
        if self.typeMagnet == 'solenoid':    # this can be only done after _load_ROXIE has been called.
    ...
    def _setUp_Solenoid(self, recalc_field):
        ...
        field_map_file = os.path.join(self.out_dir, f"{self.nameMagnet}_All_NoIron_NoSelfField.map2d")
        Ind_matrix_file = os.path.join(self.out_dir, f"{self.nameMagnet}_selfMutualInductanceMatrix.csv")
        if recalc_field:
        ...
        if np.all(np.isclose(self.Solenoid.current, self.Solenoid.current[0])):
            self.Magnet.Inputs.I00 = self.Solenoid.current[0]
        else:
            raise Exception("Issue with different current in each coil block, this might be okay, but never checked")
        self.Magnet.Inputs.M_m = np.loadtxt(Ind_matrix_file, delimiter=',', skiprows=1)
        seq = np.arange(self.Solenoid.section.size)
        sec_uni = list(np.int_(np.unique(self.Solenoid.section)))
        ...
        for sec_n in sec_uni:
            for sec_m in sec_uni:
                idx_n = seq[np.isin(self.Solenoid.section, sec_n)]
                idx_m = seq[np.isin(self.Solenoid.section, sec_m)]
                chunk = self.Magnet.Inputs.M_m[np.ix_(idx_n, idx_m)]
                res = np.sum(chunk)
                M_M[sec_n - 1, sec_m - 1] = res
        self.Magnet.Inputs.M_InductanceBlock_m = self.Magnet.Inputs.M_m
        self.Magnet.Inputs.M_m = M_M
    ...
    def _load_ROXIE(self):
        ...
        if self.typeMagnet == 'solenoid' and self.solenoid_data['flagIron']==0 and self.solenoid_data['flagSelfField']==0:
            fileName = os.path.join(self.out_dir, f"{self.nameMagnet}_All_NoIron_NoSelfField.map2d")
    ...
    def load_ConductorData(self, Conductor_L, Turns_L):
        ...
        for i in range(len(Conductor_L)):
            ...
            if not Conductor in self.AvailableConductor:
                ...
                return
            conductor_data = read_yaml('conductor', Conductor)
        ...
        if self.typeMagnet == 'solenoid':
            ...
            for block_name, block_dict in self.solenoid_data['blocks'].items():
                conductor_data = read_yaml('conductor', block_dict['wire'])
        ...
        if self.typeMagnet == 'Cos-Theta':
            ...
            self.Magnet.Inputs.GroupToCoilSection = self.Solenoid.section
            #self.Magnet.Inputs.GroupToCoilSection = self.nGroups * [1]
            self.nCoilSections = np.int_(np.max(self.Magnet.Inputs.GroupToCoilSection))
    ...
    def load_Options(self, *Type):
        ...
        if not Type in self.AvailableOptions:
            ...
            return
        option_data = read_yaml('options', Type)
    ...
    def load_VariablesToStore(self, *Type):
        ...
        if not Type in self.AvailableVariables:
            ...
            return
        option_data = read_yaml('store', Type)
        for key in option_data.keys():
    ...
    def load_PlotOptions(self, *Type):
        ...
        if not Type in self.AvailablePlotOptions:
            ...
            return
        option_data = read_yaml('plots', Type)
        for key in option_data.keys():
            if key in self.Magnet.Variables.__annotations__:
                Values = option_data[key]
    ...
    def _solenoid_locals(self):
        magnet_data = read_yaml('magnet', self.nameMagnet)
        circuit_data = read_yaml('circuit', magnet_data['circuit'])
        protection_data = read_yaml('protection', magnet_data['protection'])

        R_EE_triggered = (protection_data['U_EE'] - self.Magnet.Inputs.I00 * (
                circuit_data['R_circuit'] + circuit_data['R_crowbar']) - circuit_data['Ud_crowbar']) / self.Magnet.Inputs.I00  # Resistance of the energy-extraction system [Ohm]
        if R_EE_triggered < 0:
            R_EE_triggered = 0
        time_v_array = [item for sublist in [[rng['start'], rng['step'], rng['stop']] for rng in
                        magnet_data['simul_time']] for item in sublist]
        dict_block = {
        ...
        'directionCurrentCLIQ': [1]* self.nCoilSections,    # Direction of the introduced current change for the chosen CLIQ configuration
        ...
        'fL_I': [0, 1000],
        'fL_L': [1, 1],
        'tStartQuench': magnet_data['tStartQuench'],
        'tQuench': magnet_data['tStartQuench'],
        'iStartQuench': magnet_data['iStartQuench'],
        'tStart': time_v_array[0],      # this is needed for power supply LUT
    ...
    def writeLEDETFile(self, nameFileLEDET, locals):
        if self.typeMagnet == 'solenoid':
            ...
            nameFileLEDET = os.path.join(self.out_dir, nameFileLEDET)
    ...
    def write_start_bat(self, model_no):
        script_lines = ['for %%a in ("%~p0../../..") do set version=%%~nxa',
                        'pushd ..',
                        'pushd ..',
                        'for %%x in ("%CD%") do set folder=%%~dpxLEDET',
                        'pushd ..',
                        'for %%x in ("%CD%") do set ex=%%~dpx%version%\%version%.exe',
                        'cd %~dp0',
                        f'start %ex% %folder% {self.nameMagnet} {int(model_no)}']
        batch_file_path = os.path.join(self.out_dir, f"{self.nameMagnet}_{int(model_no)}.bat")
        with open(batch_file_path, "w") as outfile:
            outfile.write("\n".join(str(line) for line in script_lines))

    def run_LEDET_bat_parallel(self, model_no):
        os.chdir(self.magnet_folder)
        call_string = f"{self.magnet_name}_{int(model_no)}.bat"
        subprocess.call([call_string])

    def run_LEDET_bat_sequentially(self, model_no):
        os.chdir(self.magnet_folder)
        # LEDET_exe_path = os.path.join(LEDET_dir, LEDET_exe)
        batch_file_path = os.path.join(os.getcwd(), f"{self.nameMagnet}_{model_no}.bat")
    ...
    def plot_Conductors(self):
        ...
        label = {'z': ""}
        if self.typeMagnet == 'solenoid':
            label.update(self.rad_labels)
        else:
            label.update(self.cart_labels)
