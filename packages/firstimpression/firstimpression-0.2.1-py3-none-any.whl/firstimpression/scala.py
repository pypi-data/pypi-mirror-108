...
try:
    ...
    try:
        ...
        current_version = PYTHON3_MAIN


def variables():
    if current_version == PYTHON2:
        return scalalib.sharedvars()
    elif current_version == PYTHON3:
        return scalascript.variables()
    elif current_version == PYTHON3_MAIN:
        warnings.warn("Cannot grab variables from scala when running in Python 3 from main", RuntimeWarning)
        return None
    else:
...
def install_content(abspath, subfolder=None, autostart=True):
    if current_version == PYTHON2:
        ...
        warnings.warn("Cannot install content in Python 3 when running from main", RuntimeWarning)
    else:
        raise RuntimeError("Wrong Python version used")
