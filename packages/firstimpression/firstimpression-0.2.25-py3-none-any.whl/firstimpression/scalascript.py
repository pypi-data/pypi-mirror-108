...
def _isActiveX():
    """Inspect the stack and look for signs of ActiveX"""
...
if not _isActiveX():
    raise ImportError(
        'The scalascript module may only be imported from within a Scala player.')
...
def find_content(scalapath):
    ...
    if platform_path:
        ...
        raise FileNotFoundError(f'The path "{scalapath}" does not exist.')
...
def install_content(abspath, subfolder=None):
    ...
    try:
        ...
        errstr = f'{str(err)}\n\nPlayer Transmission Client ({_prog_id_netic}) not available.'
...
def _lock_content(scalapath, report_err=True):
    ...
    class _StringAndLock(str):
        ...
        def __del__(self):
            if hasattr(self, 'lockObj'):
                log(logging.DEBUG, f'unlock_content: "{scalapath}"')
    ...
    try:
        ...
        log(logging.INFO, f'"{scalapath}" @ "{windows_path}"')
...
def quitplayer():
    ...
    try:
        ...
        errstr = f'{str(err)}\n\nScala Player ({_prog_id_infoplayer5}) not available.'
        log(logging.ERROR, errstr.replace('\n', '  '))
    except Exception as err:
