...
class MongoUtils:
    ...
    def dup_insert(self, table_name, filter_keys, data_list):
        data_list = self.transform_list(data_list)
        update_list = []
        for i in data_list:
            update_filter = {key:i[key] for key in filter_keys}
            update_list.append(pymongo.UpdateOne(update_filter, i, True, False))
        self.db[table_name].bulk_write(update_list)
    ...
    def insert_only(self, table_name, data_list):
        ...
        update_list = []
        for i in data_list:
            update_filter = {key:i[key] for key in primary_keys}
            update_list.append(pymongo.UpdateOne(update_filter, {"$setOnInsert": i}, True, False))
        self.db[table_name].bulk_write(update_list)

    def create_index(self, table_name, index_keys, is_unique=False):
        index_keys = self.transform_list(index_keys)
        index_catch_key = "".join(index_keys)
        if table_name not in self.index_map:
            self.index_map[table_name] = []
        if index_catch_key not in self.index_map[table_name]:
            self.db[table_name].create_index({key:1 for key in index_keys}, unique=is_unique)
            self.index_map[table_name].append(index_catch_key)
