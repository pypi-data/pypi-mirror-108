  ...
  def clip_parameters(self):
      ...
      for k in self.v:
          ...
          if key == 'n.u.':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.0000999,0.0002)
          elif key=='hbtol':
             self.p[k] = tf.clip_by_value(self.v[k],0.0001,0.0002)
          elif key=='cutoff':
             self.p[k] = tf.clip_by_value(self.v[k],0.009999,0.012)
          elif key=='ovun5':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,60.0*self.unit)
          elif key=='ovun1':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,2.0)
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.001*self.unit,0.333*self.unit)
          elif key == 'Dehb':
             self.p[k] = tf.clip_by_value(self.v[k],-3.0*self.unit,3.0*self.unit)
          elif key == 'Desi':
             self.p[k] = tf.clip_by_value(self.v[k],10.0*self.unit,1000.0*self.unit)
          elif key== 'Depi':
             self.p[k] = tf.clip_by_value(self.v[k],0.000*self.unit,299.0*self.unit)
          elif key== 'Depp':
             self.p[k] = tf.clip_by_value(self.v[k], 0.000*self.unit,299.0*self.unit)
          elif key=='tor1':
             self.p[k] = tf.clip_by_value(self.v[k],-99.9900,-0.0)
          elif key== 'ovun2':
             ...
             if self.nn:
                if key =='bo1':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.2,-0.15)
                elif key =='bo3':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.15,-0.1)
                elif key =='bo5':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.15,-0.1)
             else:
             ...
             if self.nn:
                if key =='bo2':
                   self.p[k] = tf.clip_by_value(self.v[k],6.0,7.0)
                elif key =='bo4':
                   self.p[k] = tf.clip_by_value(self.v[k],5.0,6.0)
                elif key =='bo6':
                   self.p[k] = tf.clip_by_value(self.v[k],4.0,5.0)
             else:
                self.p[k] = tf.clip_by_value(self.v[k],0.0,50.0)
          elif key == 'boc1':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.0,39.0*self.unit)
          elif key== 'lp3':
             self.p[k] = tf.clip_by_value(self.v[k],10.0,75.0001)
          elif key == 'be1':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.0,3.0)
          elif key=='val1':
