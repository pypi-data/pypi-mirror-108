...
class MPNN(ReaxFF):
  def __init__(self,libfile='ffield',direcs={},
      ...
      ReaxFF.__init__(self,libfile=libfile,direcs=direcs,
                      ...
                      VariablesToOpt=VariablesToOpt,optmol=optmol,mefac=mefac,
                      nanv=nanv,batch_size=batch_size,sample=sample,
                      hbshort=hbshort,hblong=hblong,vdwcut=vdwcut,
                      ro_scale=ro_scale,
                      clip_op=clip_op,InitCheck=InitCheck,resetDeadNeuron=resetDeadNeuron,nn=nn,
  ...
  def supervise(self):
      ...
      self.diffn,self.diffa,self.diffb,self.diffe,self.diffesi,self.diffe_= {},{},{},{},{},{}
      for bd in self.bonds: 
          ...
          if self.nbd[bd]>0:
             ...
             self.diffn[bd]  = tf.reduce_sum(input_tensor=tf.nn.relu(self.bo0[bd]*fao - self.botol*fao))
             l_atol = tf.add(self.diffn[bd]*self.lambda_bd,l_atol)

             if self.spv_bm:
                fao_= tf.where(tf.less_equal(self.rbd[bd],bm_*self.re[bd]),1.0,0.0) ##### r< 1.2*r_e that bo > botol
             ...
             if self.spv_be:
                fe  = tf.where(tf.less_equal(self.rbd[bd],self.re[bd]),1.0,0.0) ##### r< r_e that bo > bore_
             ...
             if self.EnergyFunction == 1:
                fe  = tf.where(tf.less(self.bo0[bd],self.botol),1.0,0.0) ##### bo <= 0.0 that e = 0.0
                self.diffesi[bd]  = tf.reduce_sum(input_tensor=tf.nn.relu((self.esi[bd])*fe))
  ...
  def set_m(self):
      ...
      for t in range(1,self.messages+1):
          ...
          self.set_message_wb(pref='f'+str(t),reuse_m=reuse_m,nin=3,nout=nout_,
                      vlist=bond,layer=self.mf_layer,nnopt=self.mpopt[t],bias=b) 
  ...
  def set_message_wb(self,pref='f',reuse_m=True,nnopt=True,nin=8,nout=3,layer=[8,9],vlist=None,bias=0.0):
      ''' set matix varibles '''
      if self.m_ is None:
         self.m_ = {}
      for sp in self.spec:
          self.m[pref+'w_'+sp] = []                                    
          self.m[pref+'b_'+sp] = []
          # self.m[pref+'w_'+sp+'-'+sp]  = []                                    
          # self.m[pref+'b_'+sp+'-'+sp]  = []
          if pref+'_'+sp in self.univeral_nn:
             ...
             self.m[pref+'wo_'+sp] = tf.Variable(tf.random.normal([nin,layer[0]],stddev=0.1),name=pref+'wo_'+sp)   
             self.m[pref+'bo_'+sp] = tf.Variable(tf.random.normal([layer[0]],stddev=0.1),name=pref+'bo_'+sp)  
             for i in range(layer[1]):   
          ...
          # self.m[pref+'wi_'+sp+'-'+sp] = self.m[pref+'wi_'+sp]
          # self.m[pref+'bi_'+sp+'-'+sp] = self.m[pref+'bi_'+sp]
          # self.m[pref+'wo_'+sp+'-'+sp] = self.m[pref+'wo_'+sp]
          # self.m[pref+'bo_'+sp+'-'+sp] = self.m[pref+'bo_'+sp]
          # self.m[pref+'w_'+sp+'-'+sp]  = self.m[pref+'w_'+sp]
          # self.m[pref+'b_'+sp+'-'+sp]  = self.m[pref+'b_'+sp]

      # for bd in vlist:
      #     b = bd.split('-')
      #     sp= b[0]
      #     if b[0]!=b[1]:                   # input layer
      #        self.m[pref+'wi_'+bd] = self.m[pref+'wi_'+sp]
      #        self.m[pref+'bi_'+bd] = self.m[pref+'bi_'+sp]
      #        self.m[pref+'wo_'+bd] = self.m[pref+'wo_'+sp]
      #        self.m[pref+'bo_'+bd] = self.m[pref+'bo_'+sp]
      #        self.m[pref+'w_'+bd]  = self.m[pref+'w_'+sp]
      #        self.m[pref+'b_'+bd]  = self.m[pref+'b_'+sp]
