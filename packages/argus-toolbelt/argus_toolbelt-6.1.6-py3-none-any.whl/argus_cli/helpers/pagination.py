...
class LimitOffsetPaginator:
    ...
    def __next__(self):
        ...
        if not current:
            raise StopIteration
        if self.has_next:
            if len(current.get("data", [])) == 0:
                # if there is no data and a next page is still expected,
                # something is off with the count.
                warnings.warn(
                    "expecting a next page but current page is empty - results may be incorrect."
                )
                raise StopIteration
