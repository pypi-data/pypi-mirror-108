...
def get_recursive_subclasses(cls: list) -> list:
    subclasses = list()
    for c in cls:
        subclasses.append(c)
        if c.__subclasses__():
            subclasses.append(get_recursive_subclasses(c.__subclasses__()))
...
def get_plugin_name():
    """
    Inspects frame stack to extract plugin_name. WARNING :
