...
def createiFDOReportSection(ifdo:dict):
	...
	svgs = ['image-set-acquisition','image-set-spectral-resolution','image-set-marine-zone','image-set-resolution','image-set-navigation','image-set-scale-reference','image-set-illumination','image-set-deployment','image-set-quality','image-set-license']

	# Which fields to turn into a plot
	plot = ['image-depth','image-meters-above-ground','image-entropy','image-area','image-average-color','image-particle-count']
	...
	html += '<div class="box" style="border-color:'+cols['mid_green']+';color:'+cols['green']+';background-color:'+cols['light_green']+'">'
	html += '<table>'
	for hed in miqtv.ifdo_header_fields:
	...
	for hed in ifdo['image-set-header']:
		if hed not in svgs and hed not in plot and hed not in miqtv.ifdo_header_fields:
			html += '<tr><td class="tab_key">'+hed.replace('image-set-','')+'</td><td class="tab_val">'+str(ifdo['image-set-header'][hed])+'</td></tr>'
	html += '</table>'
	html += '</div>'
	...
	pos = {'title':'Image capture positions of '+ifdo['image-set-header']['image-set-event']+" "+ifdo['image-set-header']['image-set-sensor'],'positions':[{'name':'Curated image coordinates','data':img_coordinates_cur,'color':miqtv.color_names['green']}]}
