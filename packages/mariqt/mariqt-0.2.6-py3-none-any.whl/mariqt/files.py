...
def tabFileColumnIndices(col_names,cols,optional=[]):
	...
	for name in col_names:
		if type(col_names) == list:
			col_indcs[name] = tabFileColumnIndex(name,cols)
		else:
			col_indcs[name] = tabFileColumnIndex(col_names[name],cols)
		if col_indcs[name] < 0:
			if name not in optional:
				raise ValueError("Could not find column " + name)
			else:
				del col_indcs[name]
	return col_indcs
...
def tabFileColumnValues(line,col_indcs:dict,col_separator="\t"):
	...
	if key_col != "" and col_indcs[key_col] >= 0:
		...
		for line in file:
			...
			if line[0] == "#":
				continue
			try:
				vals = tabFileColumnValues(line,col_indcs,col_separator)
				key = vals[key_col]
				del vals[key_col]
				data[key] = vals
		...
		for line in file:
			...
			if line[0] == "#":
				continue
			try:
				data.append(tabFileColumnValues(line,col_indcs,col_separator))
			except Exception as e:
				if graceful:
					continue
				else:
...
def positionFromTabFileLine(line:str,col_indcs:dict,date_format:str,add_utc:bool=False,col_separator="\t"):
	...
	if add_utc:
		...
		date_format += "%z"
	try:
		timestamp = int(datetime.datetime.strptime(time_str, date_format).timestamp())
	except Exception as e:
		raise Exception("Could not create datetime")
	...
	if "hgt" in col_indcs:
		...
		if hgt in not_allowed:
			return Exception("Height value has illegal value: "+dep)
	else:
		hgt = -1

	# Return Position object
	return miqtg.Position(timestamp,float(lat),float(lon),float(dep),float(hgt))


### TODO Implement a function that simply checks the validity of data values to be in
### appropriate (given) ranges
