...
def readAllPositionsFromFilePath(src_file_path:str,col_names:dict,date_format:str,optional=['hgt']):
	...
	max_col_idx,col_indcs = miqtf.tabFileColumnIndicesFromFile(navigation_file,col_names,optional=optional)
	return readAllPositionsFromFile(navigation_file,col_indcs,date_format)


def readAllPositionsFromFile(navigation_file,col_indcs:dict,date_format:str):
	""" Read all positions and store by timestamp"""
	nav_data = miqtg.Positions({})
	for line in navigation_file:
		try:
			position = miqtf.positionFromTabFileLine(line,col_indcs,date_format,add_utc=True)
...
def removeTemporalOutliers(positions:miqtg.Positions, minimum_data_values_per_hour:int=10):
	...
	time_hist = {}
	for utc in time_points:
		hour = datetime.datetime.fromtimestamp(utc,tz=datetime.timezone.utc).strftime("%Y%m%d%H")
...
def removeSpatialOutliers(positions:miqtg.Positions,
	...
	""" Removes positions that have few close neighbors (potential outliers)"""
	...
	for utc in time_points:
		...
		for i in range(-time_window_size,time_window_size):
			if utc+i in positions.positions and i != 0:

				t_lat = positions[utc+i].lat
				t_lon = positions[utc+i].lon
				t_dep = positions[utc+i].dep
		...
		if num_close > min_neighbors:
			tmp_positions[utc] = miqtg.Position(utc,lat,lon,dep)
		else:
			num_outliers += 1
...
def smoothPositionsSlidingGaussian(positions:miqtg.Positions,gauss_half_width:int=60):
	...
	gauss_half_width paramater. The whole gaussian window is of size 2*gauss_half_width+1
	"""
	...
	for utc in time_points:
		...
		for i in range(-gauss_half_width, gauss_half_width):
			if utc+i in positions.positions:
				new_lat += positions[utc+i].lat * gauss[i]
				new_lon += positions[utc+i].lon * gauss[i]
				new_dep += positions[utc+i].dep * gauss[i]
				new_hgt += positions[utc+i].hgt * gauss[i]
				g_sum += gauss[i]
		...
		for i in range(-gauss_half_width, gauss_half_width):
			if utc+i in positions.positions:
				dist = miqtg.distanceLatLon(new_lat,new_lon,positions[utc+i].lat,positions[utc+i].lon)
				dist += (positions[utc+i].dep - new_dep) * (positions[utc+i].dep - new_dep)
				var += math.sqrt(dist) * gauss[i]
		var /= g_sum

		# Store new positions
		smoothed_pos_tmp[utc] = miqtg.Position(utc,new_lat,new_lon,new_dep,new_hgt)
		uncertainty[utc] = var
...
def splineToOneSecondInterval(positions:miqtg.Positions,uncertainty:dict = {}):
	""" Fills gaps in the position list to adhere to the given data frequency"""
	...
	for utc in time_points:
		...
		if pre_utc > 0 and pre_utc+1 < utc:
			for tmp_utc in range(pre_utc+1,utc):
				...
				splined_positions[tmp_utc] = miqtg.Position(tmp_utc,new_lat,new_lon,new_dep,new_hgt)

				added_points += 1

		# Add current value to position list
		splined_positions[utc] = miqtg.Position(utc,lat,lon,dep,hgt)
...
def checkPositionContent(positions:miqtg.Positions):
	...
	Returns six booleans: lat_identical,lon_identical,dep_identical,hgt_identical,dep_not_zero,hgt_not_zero."""
	...
	return lat_identical,lon_identical,dep_identical,hgt_identical,dep_not_zero,hgt_not_zero
...
def writePositions(positions:miqtg.Positions,dst_path:str,output_type:str,overwrite:bool=False,with_depth:bool=True,with_height:bool=True):
	""" Write positions any kind of file, specified by the output_type"""
	...
	if output_type == "track":
		...
		writePositionsToTabFile(o,positions,output_type,with_depth,with_height)
...
def writePositionsToGxTrack(o,positions:miqtg.Positions,with_depth:bool = False):
	...
	position_str = ""
	for utc in positions.positions:
		o.write("<when>"+datetime.datetime.fromtimestamp(utc,tz=datetime.timezone.utc).strftime(miqtv.date_formats['gx_track'])+"</when>\n")
...
def writePositionsToTabFile(o,positions:miqtg.Positions,header_format:str,with_depth:bool = False,with_height:bool = False):
	...
	for utc in positions.positions:
		line = datetime.datetime.fromtimestamp(utc,tz=datetime.timezone.utc).strftime(date_format)+"\t"+str(round(positions[utc].lat,7))+"\t"+str(round(positions[utc].lon,7))
		if with_depth:
			line += "\t"+str(round(positions[utc].dep,1))
		if with_height:
			line += "\t"+str(round(positions[utc].hgt,1))
		line += "\n"
		o.write(line)
