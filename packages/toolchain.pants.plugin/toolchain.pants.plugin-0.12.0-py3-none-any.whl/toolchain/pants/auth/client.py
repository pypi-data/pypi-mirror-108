...
def _load_from_env(complete_env: Mapping[str, str], env_var_name: str) -> Optional[AuthToken]:
    ...
    if not token_str:
        _logger.warning(
            f"Failed to load Toolchain token from env var '{env_var_name}'. Please make sure the env var is set in your environment."
        )
        return None
    token = AuthToken.from_access_token_string(token_str)
    _logger.info(
        f"Successfully loaded Toolchain token from env var '{env_var_name}', expiration: {token.expires_at.isoformat()}."
    )
    return token
