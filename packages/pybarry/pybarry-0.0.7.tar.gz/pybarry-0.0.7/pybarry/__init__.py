...
class Barry:
    ...
    def update_co2_emission(self):
        current_time = self.hour_rounder(datetime.utcnow().replace(microsecond=0)).isoformat() + 'Z'
        last_hour_date_time = self.hour_rounder(
            (datetime.now() - timedelta(hours=1)).replace(microsecond=0)).isoformat() + 'Z'
        headers = {
            'Authorization': 'Bearer ' + self.access_token,
            'Content-Type': 'application/json',
        }
        data = '{ "jsonrpc": "2.0", "id": 0, "method": "co.getbarry.api.v1.OpenApiController.getHourlyCo2Emission", "params": [ "%s", "%s" ] }' % (
