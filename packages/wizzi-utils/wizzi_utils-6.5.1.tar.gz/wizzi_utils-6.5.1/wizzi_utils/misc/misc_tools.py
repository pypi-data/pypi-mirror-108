...
def exception_error(e: (Exception, str), tabs: int = 1, depth: int = 2):
    """
    Aux function - print exception error in red with function name
    :param e: error. e.g. <class 'ModuleNotFoundError'>
    :param tabs:
    :param depth: function name to print in respect to the call stack
    :return:
    """
    error_str = '{}{}: {}'.format(tabs * '\t', get_function_name_and_line(depth=depth), e)
...
def find_files_in_folder(dir_path: str, file_suffix: str, ack: bool = False, tabs: int = 1) -> list:
    ...
    if os.path.exists(dir_path):
        ...
        if ack:
            print('{}found {} files that ends with {} in folder {} - {}'.format(
                tabs * '\t', len(all_files_processed), file_suffix, dir_path, all_files_processed))
    else:
...
def move_file(file_src: str, file_dst: str, ack: bool = True, tabs: int = 1) -> None:
    ...
    if os.path.exists(file_src):
        ...
        if os.path.exists(os.path.dirname(file_dst)):
            ...
            exception_error(NOT_FOUND.format(os.path.dirname(file_dst)), tabs=tabs)
    else:
        exception_error(NOT_FOUND.format(file_src), tabs=tabs)
    return
...
def wizzi_utils_requirements():
    ...
    print('\t\t{} test'.format(sub_pack.format(CVT_PACK)))
...
def extract_file(
    ...
    if os.path.exists(src):
        ...
        exception_error(NOT_FOUND.format(src), tabs=tabs)
    return


def compress_file_or_folder(
        src: str,
        dst_path: str = './file.zip',
        file_type: str = 'zip',
        ack: bool = True,
        tabs: int = 1) -> None:
    """
...
class FPS:
    ...
    def start(self, ack_progress: bool = False, tabs: int = 1) -> None:
        if ack_progress:
            print('{}Round {} Started:'.format(tabs * '\t', len(self.times_elapsed) + 1))
    ...
    def update(self, ack_progress: bool = False, tabs: int = 1) -> None:
        if self.timer_begin is None:
            exception_error(e='Must call FPS::start before update', tabs=0)
        else:
    ...
    def so_far(self, tabs: int = 1) -> None:
        if len(self.times_elapsed) > 0:
            string = tabs * '\t'
            string += '{} Rounds Done: '.format(len(self.times_elapsed))

            # rt_str = self.convert()
            rt_str = '{:.3f}s'.format(self.times_elapsed[-1])

            string += 'roundTime={}, '.format(rt_str)
    ...
    def finalize(self, tabs: int = 1) -> None:
        tabs_s = tabs * '\t'
        if len(self.times_elapsed) <= 0:
            exception_error('Times_elapsed is empty', tabs=tabs)
        else:
            # title
            title = 'Rounds Summary '
            if self.summary_title is not None:
                title += 'of {}'.format(self.summary_title)
            # add rounds to title
            title += '({} Rounds)'.format(len(self.times_elapsed))
