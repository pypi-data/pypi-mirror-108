    ...
    if os.path.exists(file_src):  # origin file exists
        ...
        if os.path.exists(os.path.dirname(file_dst)):  # target file doesn't exists
            if not os.path.exists(file_dst):
                ...
                exception_error(EXISTS.format(file_dst), tabs=tabs)
        else:
            exception_error(NOT_FOUND.format(os.path.dirname(file_dst)), tabs=tabs)
    else:
        exception_error(NOT_FOUND.format(file_src), tabs=tabs)
...
def dict_as_table(table: dict, title: str = 'table', fp: int = 2, ack: bool = True, tabs: int = 1) -> str:
    ...
    if same_type and type_keys == 'str':
        ...
        for i, (k, v) in enumerate(table.items()):
            string += '\n'
            string += s_format_all.format(tabs * '\t', k, to_str(v, title='key_{}'.format(i), chars=200, fp=fp))
        if ack:
            print(string)
    else:
        exception_error('supports only dict with keys from type str', tabs=tabs)
