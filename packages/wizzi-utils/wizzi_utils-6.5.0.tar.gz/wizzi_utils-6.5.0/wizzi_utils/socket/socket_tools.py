...
def rounds_summary(times: list, title: str = '', tabs: int = 0) -> None:
    """
    :param times:
    :param title:
    :param tabs:
    ...
    if len(times) > 0:  # try print time avg before exit
        if title is None:
            print('{}Rounds Summary:'.format(tabs * '\t'))
        else:
            print('{}Rounds Summary of {}'.format(tabs * '\t', title))
        ...
        if np.mean(times) > 0.0001:
            ...
            print('{}\tFPS very high'.format(tabs * '\t'))
    return
...
def download_file(url: str, dst_path: str = './file', tabs: int = 1) -> bool:
    ...
    if not os.path.exists(dst_path):
        try:
            ...
            mt.exception_error('{} {}'.format(e, url), tabs=tabs + 1)
        except FileNotFoundError as e:
            mt.exception_error('{}'.format(e), tabs=tabs + 1)

    else:
        mt.exception_error(mt.EXISTS.format(dst_path), tabs=tabs)
    return ret
