...
def classify(m: (cvt.yolov3_coco, any), cv_img: np.array, img_title: str, cv_title: str, delay_ms: int,
    ...
    # msg = '\tclassify_cv_img time={}'.format(mt.get_timer_delta(classify_timer_begin, with_ms=True))
    # print(mt.add_color(string=msg, ops='r'))
    # classify_timer_end = mt.get_timer() - classify_timer_begin
...
def model_image_list_test(model: (cvt.yolov3_coco, any), images_list: list, delay_secs: (int, None)):
    ...
    for i, img_path in enumerate(images_list):
        ...
        # classify_time_list.append(classify_time)
        # round_time_end = mt.get_timer() - round_timer_begin
        # round_time_list.append(round_time_end)
        fps_rounds.update()
    ...
    for i in range(frames):
        ...
        for cam in cams:
            ...
            classify(
                ...
                delay_ms=delay_ms,
                loc=locations[cam.port % len(locations)],
                fps_classify=fps_classify
            )
        fps_rounds.update()
    cv2.destroyAllWindows()
