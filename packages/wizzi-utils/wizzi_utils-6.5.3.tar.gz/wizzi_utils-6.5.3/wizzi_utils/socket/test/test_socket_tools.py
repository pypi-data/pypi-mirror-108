...
def download_file_test():
    ...
    st.download_file(mtt.TEMP_IMAGE_LOGO_URL, dst_path=mtt.TEMP_IMAGE_PATH)
    print('\t{} exists ? {}'.format(mtt.TEMP_IMAGE_PATH, os.path.exists(mtt.TEMP_IMAGE_PATH)))

    # st.download_file(mtt.TEMP_IMAGE_URL, dst_path=mtt.TEMP_IMAGE_PATH)  # check no overwrite - will fail
    mt.delete_file(mtt.TEMP_IMAGE_PATH)

    mt.create_dir(mtt.TEMP_FOLDER_PATH)
    st.download_file(mtt.TEMP_IMAGE_LOGO_URL, dst_path='{}/{}'.format(mtt.TEMP_FOLDER_PATH, mtt.TEMP_IMAGE_NAME))
    mt.delete_dir_with_files(dir_path=mtt.TEMP_FOLDER_PATH)
    return
