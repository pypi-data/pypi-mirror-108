...
def to_str_test():
    ...
    print(tt.to_str(var=t_var, title='\tt_var', chars=25, fp=5, rec=True))
    t_var = t_var.double()
...
def save_load_tensor_test():
    ...
    path = '{}.pt'.format(mtt.TEMP_FILE_PATH)
...
def count_keys_test():
    mt.get_function_name(ack=True, tabs=0)
    data_root = mtt.TEMP_DS_FOLDER
    mt.create_dir(data_root, ack=True)
    dataset = datasets.CIFAR10(root=mtt.TEMP_DS_FOLDER, train=False, download=True)
    # dataset = datasets.MNIST(root=mtt.TEMP_DS_FOLDER, train=False, download=True)
    print('\tcifar10 test data {}:'.format(tt.size_s_ds(dataset)))
...
def size_s_ds_test():
    ...
    print('\tcifar10 test data size: {}'.format(tt.size_s_ds(dataset)))
    return
