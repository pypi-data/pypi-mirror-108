...
class google_handler:
    ...
    def __init__(self,
        ...
        if not os.path.exists(yaml_path):
            mt.exception_error('cant find {}'.format(yaml_path), tabs=0)
        ...
        try:
            ...
            if self.gauth.access_token_expired:
                cred_path = './GDrive2/credentials.json'
                mt.exception_error('./GDrive2/credentials.json expired'.format(cred_path))
            ...
            if len(duplicates_list) > 0:
                mt.exception_error('Found duplicated files or dirs:', tabs=tabs)
            ...
            mt.exception_error('client secrets file no found on path stated in settings.yaml')
    ...
    def __str__(self):
        ...
        string += '\n\t{}yaml_path={}'.format(self.tabs * '\t', self.yaml_path)
    ...
    def __get_file(self, full_path_on_drive: str, tabs: int = 1) -> GoogleDriveFile:
        ...
        if full_path_on_drive not in self.all_files_dict:
            mt.exception_error(mt.NOT_FOUND.format(full_path_on_drive), tabs=tabs, depth=3)
        else:
            file = self.drive.CreateFile({'id': self.all_files_dict[full_path_on_drive]['id']})
            if file['mimeType'].endswith('folder'):
                file = None
                mt.exception_error('{} is a dir'.format(full_path_on_drive), tabs=tabs, depth=3)
        return file
    ...
    def __get_dir(self, full_path_on_drive: str, tabs: int = 1) -> GoogleDriveFile:
        ...
        if full_path_on_drive not in self.all_files_dict:
            ...
            if not _dir['mimeType'].endswith('folder'):
                _dir = None
                mt.exception_error('{} is a file'.format(full_path_on_drive), tabs=tabs, depth=3)
        return _dir
    ...
    def __create_file_in_dir(self, full_drive_path: str, tabs: int = 1) -> GoogleDriveFile:
        ...
        if dir_path_on_drive not in self.all_files_dict:  # dir must exist
            mt.exception_error(mt.NOT_FOUND.format(dir_path_on_drive), tabs=tabs, depth=3)
        elif full_drive_path in self.all_files_dict:  # already a file with this name in dir_path_on_drive
            mt.exception_error(mt.EXISTS.format(full_drive_path), tabs=tabs, depth=3)
        else:
    ...
    def list_all_files(self, starting_dir: str = 'root', tabs: int = 1) -> None:
        ...
        if starting_dir not in self.all_files_dict:
            mt.exception_error(mt.NOT_FOUND.format(starting_dir), tabs=tabs + 1, depth=3)
        else:
            for full_path, file_d in self.all_files_dict.items():
    ...
    def rename_file(self, file_full_path_on_drive: str, new_name: str, tabs: int = 1) -> bool:
        ...
        if file is not None:
            ...
            if new_full_path_on_drive not in self.all_files_dict:
                ...
                mt.exception_error(mt.EXISTS.format(new_full_path_on_drive), tabs=tabs, depth=2)
        return success
    ...
    def upload_dir(self, dst_full_path_on_drive: str, local_dir_name: str, tabs: int = 1) -> bool:
        ...
        if not os.path.exists(local_dir_name):
            ...
            if success_dir:
                ...
                for _dir_path, dir_names, file_names in os.walk(local_dir_name):
                    for f in file_names:
                        fp_local = '{}/{}'.format(_dir_path.replace('\\', '/'), f)
                        fp_on_drive = '/'.join(fp_local.split('/')[1:])  # remove first dir
                        fp_on_drive = '{}/{}'.format(dst_full_path_on_drive, fp_on_drive)  # add 'root/.../...'
                        files_full_path[fp_local] = fp_on_drive
                for fp_local, fp_on_drive in files_full_path.items():
                    self.upload_file(dst_full_path_on_drive=fp_on_drive, local_file_path=fp_local, tabs=tabs + 1)
