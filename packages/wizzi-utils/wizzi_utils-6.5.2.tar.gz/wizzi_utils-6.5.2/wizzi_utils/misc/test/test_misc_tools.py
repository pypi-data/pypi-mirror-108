...
TEMP_IMAGE_LOGO_URL = 'https://cdn.sstatic.net/Sites/stackoverflow/img/logo.png'
TEMP_IMAGE_COCO_URLS = [  # https://github.com/pjreddie/darknet/blob/master/data/
    'https://github.com/pjreddie/darknet/blob/master/data/dog.jpg?raw=true',
    'https://github.com/pjreddie/darknet/blob/master/data/eagle.jpg?raw=true',
    'https://github.com/pjreddie/darknet/blob/master/data/person.jpg?raw=true'
]
TEMP_IMAGE_COCO_URLS2 = [  # https://github.com/pjreddie/darknet/blob/master/data/
...
TEMP_FAKE_NAME = 'fake{}'.format(SUFFIX)
TEMP_FAKE_PATH = './{}'.format(TEMP_FAKE_NAME)
TEMP_TXT_NAME = 'demo_file{}.txt'.format(SUFFIX)
TEMP_TXT_PATH = './{}'.format(TEMP_TXT_NAME)
TEMP_FILE_NAME = 'temp_file{}'.format(SUFFIX)
TEMP_FILE_PATH = './{}'.format(TEMP_FILE_NAME)
TEMP_IMAGE_NAME = 'logo{}.png'.format(SUFFIX)
TEMP_IMAGE_PATH = './{}'.format(TEMP_IMAGE_NAME)
TEMP_DS_FOLDER = './DATASET_TEMP_FOLDER{}'.format(SUFFIX)
TEMP_FOLDER_NAME = 'TEMP_FOLDER{}'.format(SUFFIX)
TEMP_FOLDER_PATH = './{}'.format(TEMP_FOLDER_NAME)
TEMP_MODEL_PATH = './MODELS_TEMP_FOLDER{}'.format(TEMP_FOLDER_NAME)
...
def create_demo_file(path: str, tabs: int = 1) -> None:
    ...
    f.close()
    print('{}{} size - {}'.format(tabs * '\t', mt.CREATED.format(path), mt.file_or_folder_size(path)))
    return
...
def set_env_variable_test():
    mt.get_function_name(ack=True, tabs=0)
    k = TEMP_FAKE_NAME
    mt.set_env_variable(key=k, val=TEMP_FAKE_NAME, ack=True, tabs=1)
    print('\tCheck:')
...
def get_env_variable_test():
    mt.get_function_name(ack=True, tabs=0)
    mt.get_env_variable(key=TEMP_FAKE_NAME, ack=True, tabs=1)
...
def del_env_variable_test():
    ...
    mt.del_env_variable(key=k, ack=True, tabs=1)
    mt.del_env_variable(key='v2_{}'.format(TEMP_FAKE_NAME), ack=True, tabs=1)
    return
...
def save_load_np_test():
    mt.get_function_name(ack=True, tabs=0)
    path = '{}.npy'.format(TEMP_FILE_PATH)
    ...
    _ = mt.load_np('{}.npy'.format(TEMP_FAKE_PATH), ack=True)
    return


def save_load_npz_test():
    mt.get_function_name(ack=True, tabs=0)
    path = '{}.npz'.format(TEMP_FILE_PATH)
...
def save_load_pkl_test():
    mt.get_function_name(ack=True, tabs=0)
    path = '{}.pkl'.format(TEMP_FILE_PATH)
...
def logger_test():
    mt.get_function_name(ack=True, tabs=0)
    path = './{}_log_{}.txt'.format(TEMP_FILE_NAME, mt.get_time_stamp(format_s='%Y_%m_%d_%H_%M_%S'))
...
def create_and_delete_empty_dir_test():
    mt.get_function_name(ack=True, tabs=0)
    mt.create_dir(dir_path=TEMP_FOLDER_PATH)
    mt.delete_empty_dir(dir_path=TEMP_FOLDER_PATH)

    mt.create_dir(dir_path='{}/{}'.format(TEMP_FOLDER_PATH, TEMP_FOLDER_NAME))
    mt.delete_empty_dir(dir_path=TEMP_FOLDER_PATH)  # will fail
    # remove empty dir 1 by 1
    mt.delete_empty_dir(dir_path='{}/{}'.format(TEMP_FOLDER_PATH, TEMP_FOLDER_NAME))  # will work
    mt.delete_empty_dir(dir_path=TEMP_FOLDER_PATH)  # will work
    return
...
def create_and_delete_dir_with_files_test():
    ...
    inner_dir = '{}/{}'.format(base_path, TEMP_FOLDER_NAME)
    mt.create_dir(dir_path=inner_dir)
    create_demo_file(path='{}/{}'.format(base_path, TEMP_TXT_NAME))
    create_demo_file(path='{}/v2_{}'.format(base_path, TEMP_TXT_NAME))
    create_demo_file(path='{}/{}'.format(inner_dir, TEMP_TXT_NAME))
    create_demo_file(path='{}/v2_{}'.format(inner_dir, TEMP_TXT_NAME))
...
def find_files_in_folder_test():
    test_f = TEMP_FOLDER_PATH
    mt.create_dir(test_f)
    txt1 = '{}/{}'.format(test_f, TEMP_TXT_NAME)
    txt2 = '{}/v2_{}'.format(test_f, TEMP_TXT_NAME)
    zip1 = '{}/{}.zip'.format(test_f, TEMP_FILE_PATH)
...
def move_file_test():
    mt.get_function_name(ack=True, tabs=0)
    file_src = './{}'.format(TEMP_TXT_NAME)
    file_dst = './v2_{}'.format(TEMP_TXT_NAME)
    create_demo_file(file_src)
    mt.move_file(file_src=file_src, file_dst=file_dst)
    mt.delete_file(file=file_dst)
    return


def delete_file_test():
    mt.get_function_name(ack=True, tabs=0)
    create_demo_file(TEMP_TXT_PATH)
    mt.delete_file(file=TEMP_TXT_PATH)
    return


def delete_files_test():
    mt.get_function_name(ack=True, tabs=0)
    path1 = './v1_{}'.format(TEMP_TXT_NAME)
    path2 = './v2_{}'.format(TEMP_TXT_NAME)
...
def file_or_folder_size_test():
    mt.get_function_name(ack=True, tabs=0)
    mt.file_or_folder_size(TEMP_FAKE_PATH, ack=True, tabs=1)

    create_demo_file(TEMP_TXT_PATH)
    mt.file_or_folder_size(TEMP_TXT_PATH, ack=True, tabs=1)
    mt.delete_file(TEMP_TXT_PATH)

    mt.create_dir('{}/{}'.format(TEMP_FOLDER_PATH, TEMP_FOLDER_NAME))
    create_demo_file('{}/{}'.format(TEMP_FOLDER_PATH, TEMP_TXT_NAME))
    create_demo_file('{}/v2_{}'.format(TEMP_FOLDER_PATH, TEMP_TXT_NAME))
    create_demo_file('{}/{}/{}'.format(TEMP_FOLDER_PATH, TEMP_FOLDER_NAME, TEMP_TXT_NAME))
    mt.file_or_folder_size(TEMP_FOLDER_PATH, ack=True, tabs=1)
    mt.delete_dir_with_files(TEMP_FOLDER_PATH)
    return
...
def compress_and_extract_test():
    ...
    f_name = TEMP_TXT_NAME
    path = './{}.txt'.format(f_name)
    create_demo_file(path=path)  # temp file

    for file_t in ['zip', '7z', 'tar']:
        print('\t{}:'.format(file_t))
        mt.compress_file_or_folder(src=path, dst_path=f_name, file_type=file_t, ack=True, tabs=2)
        mt.extract_file(src='./{}.{}'.format(f_name, file_t), dst_folder='./{}/'.format(f_name), file_type=file_t,
                        tabs=2)
        mt.delete_file('./{}.{}'.format(f_name, file_t), tabs=2)  # remove compressed
        mt.delete_dir_with_files('./{}/'.format(f_name), tabs=2)  # remove extracted

    mt.delete_file(path)  # remove temp file

    # folder
    print('\tFolder tests:')
    dir_name = TEMP_FOLDER_NAME
    dir_path = './{}'.format(dir_name)
    mt.create_dir(dir_path)  # temp folder
    path = '{}/{}'.format(dir_path, TEMP_TXT_NAME)
    create_demo_file(path=path)  # temp file 1
    path = '{}/v2_{}.txt'.format(dir_path, TEMP_TXT_NAME)
    create_demo_file(path=path)  # temp file 2
    for file_t in ['zip', '7z', 'tar']:
        print('\t{}:'.format(file_t))
        mt.compress_file_or_folder(src=dir_path, dst_path=dir_name, file_type=file_t, ack=True, tabs=2)
        mt.extract_file(src='./{}.{}'.format(dir_name, file_t), dst_folder='{}2/'.format(dir_path), file_type=file_t,
                        tabs=2)
        mt.delete_file('./{}.{}'.format(dir_name, file_t), tabs=2)  # remove compressed
        mt.delete_dir_with_files('{}2/'.format(dir_path), tabs=2)  # remove extracted

    mt.delete_dir_with_files(dir_path)  # remove temp folder with temp file 1 and 2
    return
