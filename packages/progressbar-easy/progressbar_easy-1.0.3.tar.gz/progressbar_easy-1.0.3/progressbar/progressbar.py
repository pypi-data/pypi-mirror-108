...
class ProgressBar(object):
    def __init__(self, items=None, completed=0, maxlen=25, char='█', show_on_update=False, ips=1, lsttime=None, learning_rate=.1):
...
[{(self.char*int(self.pos*self.maxlen)).ljust(self.maxlen)}] {st}\t\r')
    ...
    def update(self, completed=None):
        if isinstance(completed, NoneType):
...
if __name__ == '__main__':
    import random
    prog = ProgressBar(5, show_on_update=True, ips=1.5)
