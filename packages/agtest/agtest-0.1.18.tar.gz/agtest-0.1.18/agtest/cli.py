...
def create_model_report(obj, report=r'model_lookup.json'):
    with open(report, mode='wt') as f:
...
def get_release_list(base_url, mac_id, mac_token, host):
...
def get_list(ctx):
    ...
    mac_id = ctx.obj.get('MAC_ID')
    mac_token = ctx.obj.get('MAC_TOKEN')
    get_release_list(base_url, mac_id, mac_token, host)
