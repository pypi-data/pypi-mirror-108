...
def get_tag(tag_id: int) -> sa.sql.expression:
    return get_tags().where(tags.c.id == tag_id)
...
def get_contacts(tag_id: int):
    main_tag_hist = sa.select([
        history.c.start_ts,
        sa.func.coalesce(
            history.c.end_ts,
            sa.text('(now() at time zone \'utc\')')
        ).label('end_ts')
    ]).where(history.c.tag_id == tag_id).cte('main')
    ...
    ]).where(history.c.tag_id != tag_id).cte('other')
    sub = sa.select([
        ...
        sa.func.greatest(
            main_tag_hist.c.start_ts,
            other_tag_hist.c.start_ts).label('start_ts'),
        sa.func.least(
             main_tag_hist.c.end_ts, other_tag_hist.c.end_ts
        ).label('end_ts')
    ]).select_from(main_tag_hist, other_tag_hist).join(tags).where(
        sa.tuple_(main_tag_hist.c.start_ts,
                  main_tag_hist.c.end_ts).op('overlaps')(
            sa.tuple_(other_tag_hist.c.start_ts,
                      other_tag_hist.c.end_ts))).subquery()
    return sa.select(sub)


async def exists_or_404(db: AsyncConnection,
