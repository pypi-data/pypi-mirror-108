...
class ListenerSchema(Schema, AttrsMixin):
    ...
    last_seen = fields.DateTime('%Y-%m-%dT%H:%M:%S+00:00', dump_only=True)
...
class TagCreateBase(Schema, AttrsMixin):
    ...
    type = fields.Str(required=True, validate=lambda x: x in {
        'iBeacon', 'SmartRelay', 'LocationAnchor', 'SecureLocationAnchor',
        'SecureSmartRelay'})
    battery_pct = fields.Int(allow_none=True)
    alarm_active = fields.Boolean(allow_none=True)
    zone_start = fields.DateTime('%Y-%m-%dT%H:%M:%S+00:00', dump_only=True)
...
class ProxSchema(Schema):
    ...
    last_seen = fields.DateTime('%Y-%m-%dT%H:%M:%S+00:00', required=True)
...
class AlarmSchema(Schema):
    ...
    tag_name = fields.Str(required=True)
    zone_name = fields.Str(required=True)
    start_ts = fields.DateTime('%Y-%m-%dT%H:%M:%S+00:00', required=True)
    last_ts = fields.DateTime('%Y-%m-%dT%H:%M:%S+00:00', required=True)
...
class HistorySchema(Schema):
    start_ts = fields.DateTime('%Y-%m-%dT%H:%M:%S+00:00', required=True)
    end_ts = fields.DateTime('%Y-%m-%dT%H:%M:%S+00:00', required=True)
