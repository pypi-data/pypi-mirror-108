...
class TinyB(object):
    ...
    async def connection_create_kafka(self, bootstrap_server, username, password):
        return await self._req("/v0/connectors?service=kafka&kafka_security_protocol=SASL_SSL&kafka_sasl_mechanism=PLAIN&"
                               f"kafka_bootstrap_servers={bootstrap_server}&kafka_sasl_plain_username={username}&"
                               f"kafka_sasl_plain_password={password}",
                               method='POST')
