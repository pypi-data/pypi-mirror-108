...
@click.argument('datasource_name')
@click.option('--topic', help="Kafka topic")
...
async def datasource_connect(ctx, connection_id, datasource_name, topic, group, auto_offset_reset):
    ...
    # TODO See if we can preview the list of topics
    if not topic:
...
async def connection_create_kafka(ctx, bootstrap_server, username, password):
    ...
    client = ctx.obj['client']
    result = await client.connection_create_kafka(bootstrap_server, username, password)
    id = result['id']
