...
namespace qx
{
   ...
   class density_operator
   {
	...
	bool add_state(qx::qu_register * state, double probability)
	{
	   ...
	   {
	      println("[x] error : density_operator: cannot add the new state, mixed state dimensions are incompatible !");
	      return false;
	   }
	...
	{
	   linalg::matrix<complex_t> * dm  = new linalg::matrix<complex_t>(state_dim, state_dim); 
	   ...
	   for (size_t i=0; i<states.size(); ++i)
	   {
		 ...
		 {
		    // println("[" << j << "," << k << "] -> " << "[" << state[j] << " x " << std::conj(state[k]) << "]" << (p * state[j] * std::conj(state[k])));
