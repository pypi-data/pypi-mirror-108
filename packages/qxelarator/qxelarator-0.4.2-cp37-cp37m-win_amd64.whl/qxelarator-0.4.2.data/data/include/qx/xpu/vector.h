...
namespace xpu
{
   ...
   template <typename __T, size_t __A=16>
      class vector 
      {
	 public:
	     ...
	     * constructor
	     */
	    inline vector(size_t n) : __size(n)
	    {
	    ...
	    inline __T & operator[] (size_t i) 
	    {
	       ...
	       if (i >= __size) 
	       {
		  std::cerr << "[x] xpu::vector : fatal error : index out of bound !" << std::endl;
		  exit(-1);
	       }
#endif
	       return __data[i];
	    ...
	    inline vector<__T,__A> & operator = (vector<__T,__A> & v) 
	    {
		   ...
		   memcpy((void *)__data, (const void *)(v.__data), (v.__size*sizeof(__T)));
		   __size = v.__size;
		   return *this;
	    }
