    ...
    def write(self, f, compact=False):
        ...
        for key in self._params:
            ...
            if compact:
                ...
                print('{} = {}'.format(key, _value), file=f)
        print('', file=f)
...
class Config(object):
        ...
        for section in self._sections:
            for param in self._sections[section]:
                if not (param in self._direct_attr):
                    self._direct_attr[param] = self._sections[section][param]
                else:
    ...
    def get_template(template_dir):
        _internal_template = os.path.join(os.path.dirname(__file__), 'template.cfg')
        with open(template_dir, 'w') as f, open(_internal_template, 'r') as fr:
            line = fr.readline()
            while line:
                print(line.strip('\n'), file=f)
                line = fr.readline()
