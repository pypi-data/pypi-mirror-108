...
def FilterSignal(Signal, Rate, Frequency, Order=4, Coeff='butter', Type='bandpass'):
    """
...
def PSD(Signal, Rate, Scaling='density', Window='hann', WindowSize=None, NPerSeg=None, Overlap=None):
    ...
    if len(Signal.shape) == 2:
        for C in range(Signal.shape[1]):
            ...
            f, pxx = PSD(Signal[:,C], Rate, Scaling, WindowSize, NPerSeg, Overlap)
...
def QuantifyTTLs(Data=[], StdNo=2.5, Edge='rise', FixedThreshold=None):
    """
...
def Slice(Data, TTLs, SliceWindow):
    """
    ...
    if len(Data.shape) == 2:
        ...
        for C in range(Data.shape[1]):
            print(f'Processing {C+1} of {Data.shape[1]}...')
...
def Spectrogram(Signal, Rate, Window='hann', WindowSize=None, Overlap=None, NFFT=None):
    """
    Get the power spectrum density (PSD) of a signal. Built around
