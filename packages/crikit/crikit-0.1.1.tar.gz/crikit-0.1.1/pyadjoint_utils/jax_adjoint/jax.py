...
class JAXBlock(Block):
    def __init__(
        ...
        **kwargs,
        ...
        leaves, self._treedef = pytree.flatten(tuple(args))
    ...
    def _diff_func(self, *args):
        leaves, treedef = pytree.flatten(tuple(args))
    ...
    def prepare_evaluate_tlm_matrix(self, inputs, tlm_inputs, relevant_outputs):
        ...
        # Compute Jacobian df_dy.
        point_jacobians = tuple(
            self._tlm_matrix_func(*args) for args in zip(*standard_args)
        )
        ...
        for i, out_idx in enumerate(outnums):
            ...
            for j, in_idx in enumerate(argnums):
                ...
                if out_pw:
                    if self._single_output:
                        di_dj = _compiled_stack_j(point_jacobians, j)
                    else:
                        di_dj = _compiled_stack_ij(point_jacobians, i, j)
                    if in_pw:
                    ...
                    di_dj = _compiled_stack_ij_out(point_jacobians, i, j, out_rank)
...
def _compiled_stack_ij(point_jacs, i, j):
    return jnp.stack(tuple(p[i][j] for p in point_jacs))


@partial(jax.jit, static_argnums=(1, 2, 3))
def _compiled_stack_ij_out(point_jacs, i, j, out_ax):
    return jnp.stack(tuple(p[i][j] for p in point_jacs), axis=out_ax)
...
def is_tracer(x):
    tracers = {
        ...
        jax.abstract_arrays.ConcreteArray,
        jax.abstract_arrays.AbstractToken,
        jax.interpreters.partial_eval.DynamicJaxprTracer,
        jax.interpreters.batching.BatchTracer,
        jax.interpreters.ad.JVPTracer,
    }
    # concrete = {jax.interpreters.xla.DeviceArray,jnp.ndarray}

    xt = type(x)
    return xt in tracers  # and xt not in concrete
