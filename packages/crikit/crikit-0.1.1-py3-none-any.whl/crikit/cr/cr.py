...
class CR(PointMap):
    ...
    def __init__(
        ...
        **cr_jax_kwargs,
        ...
        self._pointwise = (True,) * self._num_inputs + (False,) * self._num_params
        if self._f is None:
            vmap = False
    ...
    def _check_inputs(self, inputs):
        ...
        for i, (inpt, expected) in enumerate(zip(inputs, self._in_types)):
            shape = inpt.shape[1:] if self._vmap else inpt.shape
...
class BlockCR(CR):
    ...
    def __init__(
        ...
        **cr_jax_kwargs,
        ...
        # undo the vmapping of self._f and re-build the overloaded function
        self._f = cr_function
        self._vmap = False  # so that the CR isn't marked as pointwise in
        # the call to overload_jax
        self._overloaded_call = self._make_overloaded_call()
