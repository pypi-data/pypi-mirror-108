...
<xsl:template match="note[starts-with(@class, 'amc_question')]">
  ...
  <xsl:attribute name="class"><xsl:value-of select="@class"/></xsl:attribute>
       ...
       <xsl:for-each select="text[@class='amc_bareme']">
        ...
        <xsl:for-each select=".//note[@class='amc_choices_options' or @class='amc_numeric_choices']"> <!-- FIXME mix both things, why for-each; pb with xpath-->
<!--        <xsl:for-each select=".//note[@class='amc_choices_options']">-->
<!--        <options><xsl:value-of select="text()"/>1111111111111111111111111111111111111111111111111111111111</options>-->
<!--			<xsl:apply-templates/>-->
			<xsl:copy-of select="."/>
        </xsl:for-each>
