...
class UMnetequip:
    ...
    def __init__(self, host='mallard.dsc.umich.edu', port=1521):
        ...
        creds = get_env_vars(['EQUIP_DB_USER','EQUIP_DB_PASSWORD'])

        self._url = f"oracle://{creds['EQUIP_DB_USER']}:{creds['EQUIP_DB_PASSWORD']}@(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST={host})(PORT={port}))(CONNECT_DATA=(SID=PC13)))"
    ...
    def get_devices_by_category(self, category, active_only=False):
        ...
        select = [ 'eq.monitored_device', 'eq.rancid', 'eq.off_line', 'eq.dns_name' ]
