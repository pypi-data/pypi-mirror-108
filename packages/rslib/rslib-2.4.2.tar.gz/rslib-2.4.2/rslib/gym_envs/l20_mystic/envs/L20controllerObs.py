...
class controllerObs(object):
    ...
    def __init__(self, config, model, modelfile, sess, src, batch_size=None, one_step=False, use_rule=False, test=False):
        ...
        self.item_emb3 = self.model.get_layer('embedding_3').get_weights()[0]
        self.item_emb4 = self.model.get_layer('embedding_4').get_weights()[0]
        self.item_emb5 = self.model.get_layer('embedding_5').get_weights()[0]
        self.item_embs = [self.item_emb2, self.item_emb3, self.item_emb4, self.item_emb5]
    ...
    def action_mask(self, step, selected_embs):
        x = True
        if not self.test:
            if_ssr_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.5 if x else 1),
                                   self.one_mask,
                                   self.not_l0_ssr_mask)
            if_taohua_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.8 if x else 1),
                                      self.one_mask,
                                      self.not_taohua_mask)
            if_l2_taohua_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.2 if x else 1),
                                         self.one_mask,
                                         self.not_taohua_mask)
            taohua_ajust_mask = np.less(np.random.random((self.batch_size,)), 0.5 if x else 1)
        else:
            if_ssr_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.1 if x else 1),
                                   self.one_mask,
                                   self.not_l0_ssr_mask)
            if_taohua_mask = np.where(np.less(np.random.random((self.batch_size,)), 1.0 if x else 1),
                                      self.one_mask,
                                      self.not_taohua_mask)
            if_l2_taohua_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.2 if x else 1),
                                         self.one_mask,
                                         self.not_taohua_mask)
            taohua_ajust_mask = np.less(np.random.random((self.batch_size,)), 0.0 if x else 1)

        if step < 3:
            if step == 0:
                l_mask = self.l1_mask
                t_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.5 if x else 1),
                                  self.t2_mask,
                                  self.t4_mask)
            elif step == 1:
                l_mask = self.l1_mask
                t_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.65 if x else 1),
                                  self.t2_mask,
                                  self.t4_mask)
            else:
                l_mask = self.l1_mask
            ...
            if step == 6:
                ...
                l_mask = self.l3_mask
                t_mask = np.where(np.less(np.random.random((self.batch_size,)), 0.8 if x else 1),
                                  self.t2_mask,
                                  self.t4_mask)
            else:
                l_mask = self.l3_mask
    ...
    def step_onehot(self, step):
        # onehot = np.array([[0] * (step) + [1] + [0] * (9 - step)] * self.batch_size)
        step += 1
    ...
    def rawstate2obs(self, rawstate, info, step):
        ...
        #           3633           500         500          10            256       256*9        3*9           4*9
