...
class L20RecSim(RecSim):
    ...
    def __init__(self, recEnvSrc, model, modelfile, sess, steps=9, batch_size=None, reward_sqr=False, use_rule=False, fenliu='1'):
        ...
        self.conf_paytype = {1, 2, 3, 4}
        self.num_paytype = len(self.conf_paytype)
    ...
    def _step(self, rawstate, action, info):
        ...
        paytypes = [[0] * 9 for _ in range(self.batch_size)]
        for batch_i in range(self.batch_size):
            ...
            # region 在最后一步获取已选物品的 price、paytype
            if step == self.steps:
                ...
                paytype = [self.recEnvSrc.get_item_paytype(i) if i in self.recEnvSrc.available_item else -1 for i in self.actions[0:9, batch_i]]
                paytypes[batch_i] = paytype
            # endregion
        # endregion
        a = 1
        ...
        rewards_paytype = collections.defaultdict(lambda: [[] for _ in range(self.batch_size)])
        # rewards_paytype = [[[] for _ in range(self.batch_size)] for _ in range(self.num_paytype)]
        reward = [0 for _ in range(self.batch_size)]
        reward_paytype = collections.defaultdict(lambda: [0 for _ in range(self.batch_size)])
        # reward_paytype = [[0 for _ in range(self.batch_size)] for _ in range(self.num_paytype)]
        ...
        if step < self.steps:
            ...
            for batch_i in range(self.batch_size):
                if self.fenliu == '1':
                    for i in range(0, 9):
                        rewards_paytype[paytypes[batch_i][i]][batch_i] += [ctr[i][batch_i] * prices[batch_i][i]]
                    rewards[batch_i] += [ctr[i][batch_i] * prices[batch_i][i] for i in range(0, 9)]
                    ...
                    for i in range(0, 9):
                        rewards_paytype[paytypes[batch_i][i]][batch_i] += [(ctr[i][batch_i] * prices[batch_i][i]) ** 0.5]
                    rewards[batch_i] += [(ctr[i][batch_i] * prices[batch_i][i]) ** 0.5 for i in range(0, 9)]
                ...
                if check:
                    ...
                    for paytype, _rewards in rewards_paytype.items():
                        reward_paytype[paytype][batch_i] = sum(_rewards[batch_i])
        ...
        info['reward_paytype'] = reward_paytype
        info['rewards_paytype'] = rewards_paytype
