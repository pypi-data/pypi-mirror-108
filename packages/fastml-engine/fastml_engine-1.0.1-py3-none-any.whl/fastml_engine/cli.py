...
def _build_gunicorn_command(service_path, host, port, workers, threads, timeout):
    ...
    return ["gunicorn"] + ["--pid", "%s/logs/gunicorn.pid" % service_path] \
           ...
           + ["--worker-class", "gevent"] \
           + ["-e", "SERVICE_PATH=%s" % service_path, "-b", bind_address, "-w", "%s" % workers, "--threads","%s" % threads, "-t", "%s" % timeout, "fastml_engine.server:app"]
...
@click.option(
    ...
    default=None,
    required=True,
    help="Path to the model service,the path is absolute path , example: /opt/inference-template",
)
@click.option(
    "--host",
    "-h",
