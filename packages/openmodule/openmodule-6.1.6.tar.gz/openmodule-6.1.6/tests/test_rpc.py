...
class RpcClientTest(RPCServerTestMixin, OpenModuleCoreTestMixin):
    ...
    def setUp(self):
        ...
        self.client = RPCClient(self.core, channels=[b"channel"])
        self.wait_for_dispatcher(self.core.messages)

    def test_blocking(self):
        result = self.client.rpc(b"channel", "test", {}, timeout=1)
        self.assertEqual("ok", result.status)

        with self.assertRaises(TimeoutError):
            result = self.client.rpc(b"channel", "timeout", {}, timeout=1)

    def test_non_blocking(self):
        result = self.client.rpc(b"channel", "test", {}, timeout=1, blocking=False)
        self.assertEqual("ok", result.status)
        self.assertEqual(None, result.response)
        time.sleep(1)
        result = self.client.check_result(result.rpc_id)
