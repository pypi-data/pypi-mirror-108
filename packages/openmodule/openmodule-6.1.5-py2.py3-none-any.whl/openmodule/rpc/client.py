...
class RPCClient:
    ...
    def __init__(self, core: OpenModuleCore, channels=None, timeout=3., cleanup_time=60):
        if channels is None:
            channels = []
        self.core = core
    ...
    def cleanup_old_results(self):
        ...
        with self.lock:
            ...
            for rpc_id, entry in self.results.items():
                if now - entry.timestamp > self.cleanup_time and now > entry.timestamp + entry.timeout:
