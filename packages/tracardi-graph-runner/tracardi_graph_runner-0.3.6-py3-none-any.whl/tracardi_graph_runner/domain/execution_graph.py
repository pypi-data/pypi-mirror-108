...
class ExecutionGraph(BaseModel):
    ...
    async def run_task(self, node: Node, payload, ready_actions_results: ActionsResults):
        ...
        if node.graph.in_edges:
            ...
            for start_port, edge, end_port in node.graph.in_edges:  # type: str, Edge, str
                ...
                if not ready_actions_results.has_edge_value(id):
    ...
    @staticmethod
    def _get_object(node: Node, debug, params=None) -> ActionRunner:
    ...
    def init(self, debug=False) -> InitResult:
        errors = []
        objects = []
        for node in self.graph:
            # Init object
            try:
                node.object = self._get_object(node, debug, node.init)
    ...
    def _post_process_result(self, input_port, input_params, input_edge_id, tasks_results, result, node) -> ActionsResults:
    ...
    async def run(self, payload) -> DebugInfo:
