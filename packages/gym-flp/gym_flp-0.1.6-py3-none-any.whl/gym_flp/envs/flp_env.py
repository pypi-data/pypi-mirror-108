...
class qapEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if MHC <= self.movingTargetReward:
            reward +=50
            self.movingTargetReward = MHC
            self.best_state = self.fromState.copy()
       ...
       # self.movingTargetReward = MHC if MHC < self.movingTargetReward else self.movingTargetReward
        ...
        return newState, reward, done, MHC
...
class ofpEnv(gym.Env):
    ...
    def ConvertCoordinatesToState(self, state_prelim):    
        ...
        # 05.06.21: Added single-colour representation per request
        R = np.array((p-np.min(p))/(np.max(p)-np.min(p))*0).astype(int)
        R[R == 0] = 40
        G = np.array((sources-np.min(sources))/(np.max(sources)-np.min(sources))*255).astype(int)
        G[G == 0] = 40
        B = np.array((sinks-np.min(sinks))/(np.max(sinks)-np.min(sinks))*0).astype(int)
        B[B == 0] = 40
