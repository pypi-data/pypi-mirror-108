...
class H3Accessor:
    ...
    # TODO: Test
    @doc_standard('h3_polyfill', 'containing a list H3 addresses whose centroid falls into the Polygon')
    def polyfill(self, resolution: int) -> AnyDataFrame:
        """
        Parameters
        ----------
        resolution : int
            H3 resolution
        """
        result = [list(polyfill(geometry, resolution, True)) for geometry in self._df.geometry]
        return self._df.assign(h3_polyfill=result)
    ...
    # TODO: Re-do properly
    def k_ring_smoothing(self, k: int) -> AnyDataFrame:
        return (self._df
