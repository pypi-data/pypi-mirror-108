...
#Import all required modules for the efficient MinHeap Tree...
#############################################################
#############################################################
#
...
def isPerfectTree(NUMNODES, LEVEL):
...
def ExtractMin(BT, INDEX, LEVEL):
    ...
    if((INDEX == 0 or INDEX == -1) and LEVEL != 0):
        ...
        #see if LEVEL needs to be changed...
        sum = 0
...
def MINBTAdd(BT, LEVEL, INDEX, NUMNODES, value):
...
def downwardT(value, INDEX, LEVEL, BT):
    ...
    while(True):
        ...
        if(temp1 <= INDEX and temp2 <= INDEX):
            ...
            if(BT[int(t)][int(q)][int(p)] < BT[int(a)][int(b)][int(c)]):
...
def AddValue(INFO,value):
    ...
    if(Secure(INFO) == 1 and (type(value) == type(9.3) or type(value) == type(7))):
        
        #retrieve level, index, and num-nodes...
        INFO[1], INFO[2], INFO[3] = MINBTAdd(INFO[0],INFO[1],INFO[2],INFO[3],value)
...
def ExtractMinValue(INFO):
    ...
    if(Secure(INFO) == 1 and INFO[2] != -1 and INFO[3] > 0):
        ...
        value, INFO[2], INFO[1], INFO[3] = ExtractMin(INFO[0], INFO[2], INFO[1])
...
def BreadthFirstOne(INFO, value):
    ...
    #see if the INFO structure appears legit...
    if(Secure(INFO) == 1 and INFO[2] != -1 and INFO[3] > 0 and (type(value) == type(9.2) or type(value) == type(9))):
...
def isPerfect(INFO):
    ...
    #tree code...
    return isPerfectTree(INFO[3], INFO[1])
...
def reCalibrateInfo():
    ...
    if(not(".BinaryT" in directory)):
        ...
        #check if L is full...
        if(len(L) == 0 or not("Data.Data" in L)):
...
def makeDir():
    ...
    #see how many binaryT files are in the current
    #directory...
    ...
    os.mkdir("Min_Heap_Tree_Files"+str(len(L)+1)+".BinaryT")

    #change operations to the new directory...
    os.chdir("Min_Heap_Tree_Files"+str(len(L)+1)+".BinaryT")
...
def getHeightThree(INFO, value):
    ...
    #security check...
    if((type(value) == type(9) or type(value) == type(7.3)) and Secure(INFO) == 1 and (INFO[3] > 0) and (INFO[2] != -1)):
...
#User functiton for seeing if a tree is full; each node contains
#0 or two children...
def isFullTree(INFO):

    #do a security check...
    if(Secure(INFO) == 1 and INFO[3] != 0 and INFO[1] != -1 and INFO[3] % 2 != 0):
