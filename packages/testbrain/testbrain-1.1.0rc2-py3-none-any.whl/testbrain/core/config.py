...
from ..helpers.fs import abspath, join
...
class ConfigHandler(ConfigInterface):
    ...
    def read_file(self, file_path=None):
        ...
        if not file_path:
            ...
            file_path = Path.joinpath(Path.cwd(), '.testbrain', 'config.{}'.format(self.extension))
    ...
    def write_file(self, file_path=None):
        ...
        if not Path(Path.joinpath(Path.cwd(), '.testbrain')).exists():
            # os.mkdir(join(os.getcwd(), '.testbrain'))
            Path(Path.joinpath(Path.cwd(), '.testbrain')).mkdir()
