...
def laplace_logpdf(op, value, mu, b):
    return -at.log(2 * b) - at.abs_(value - mu) / b


@_logpdf.register(arb.LogNormalRV)
def lognormal_logpdf(op, value, mu, sigma):
    res = (
...
def pareto_logpdf(op, value, alpha, m):
...
def cauchy_logpdf(op, value, alpha, beta):
...
def halfcauchy_logpdf(op, value, loc, beta):
    res = at.log(2) + cauchy_logpdf(op, value, loc, beta)
    res = at.switch(at.ge(value, loc), res, -np.inf)
    return res


@_logpdf.register(arb.GammaRV)
def gamma_logpdf(op, value, alpha, inv_beta):
...
def invgamma_logpdf(op, value, alpha, beta):
    res = (
        -at.gammaln(alpha)
        + logpow(beta, alpha)
        - beta / value
...
def weibull_logpdf(op, value, alpha, beta):
    res = (
        at.log(alpha)
        - at.log(beta)
...
def vonmises_logpdf(op, value, mu, kappa):
...
def triangular_logpdf(op, value, lower, c, upper):
    res = at.switch(
        at.lt(value, c),
...
def gumbel_logpdf(op, value, mu, beta):
    z = (value - mu) / beta
...
def logistic_logpdf(op, value, mu, s):
    z = (value - mu) / s
...
def binomial_logpdf(op, value, n, p):
    res = binomln(n, value) + logpow(p, value) + logpow(1.0 - p, n - value)
...
def betabinomial_logpdf(op, value, n, alpha, beta):
    res = (
        binomln(n, value)
...
def bernoulli_logpdf(op, value, *inputs, **kwargs):
    ...
    res = Assert("0 <= p <= 1")(res, at.all(at.le(0.0, p)), at.all(at.lt(p, 1.0)))
    return res
...
def poisson_logpdf(op, value, *inputs, **kwargs):
    (mu,) = inputs[3:]
    res = logpow(mu, value) - at.gammaln(value + 1) - mu
...
def nbinom_logpdf(op, value, *inputs, **kwargs):
    ...
    res = (
        ...
        + logpow(mu / (mu + n), value)
        + logpow(n / (mu + n), n)
    )
    res = at.switch(at.le(0, value), res, -np.inf)
    res = Assert("0 < mu, 0 < n")(res, at.all(at.lt(0.0, mu)), at.all(at.lt(0.0, n)))
    res = at.switch(at.gt(n, 1e10), poisson_logpdf(op, value, mu), res)
    return res
...
def geometric_logpdf(op, value, *inputs, **kwargs):
    (p,) = inputs[3:]
    res = at.log(p) + logpow(1 - p, value - 1)
    res = at.switch(at.le(1, value), res, -np.inf)
    res = Assert("0 <= p <= 1")(res, at.all(at.le(0.0, p)), at.all(at.le(1.0, p)))
    return res
...
def dirichlet_logpdf(op, value, *inputs, **kwargs):
    (a,) = inputs[3:]
    res = at.sum(logpow(value, a - 1) - at.gammaln(a), axis=-1) + at.gammaln(
        at.sum(a, axis=-1)
    )
    res = at.switch(at.bitwise_and(at.le(0.0, value), at.le(value, 1.0)), res, -np.inf)
    res = Assert("0 < a")(res, at.all(at.lt(0.0, a)))
    return res
...
def multinomial_logpdf(op, value, *inputs, **kwargs):
    n, p = inputs[3:]
    res = at.gammaln(n + 1) + at.sum(-at.gammaln(value + 1) + logpow(p, value), axis=-1)
    res = at.switch(
        at.bitwise_and(at.le(0.0, value), at.all(at.eq(at.sum(value, axis=-1), n))),
        res,
        -np.inf,
    )
