...
def config(loader):
    ...
    loader.add_attr('wsd_host', '', help='When this value is set, ws_host and api_host is overritten by this value.')
    loader.add_attr_boolean('wsd_secure', False, help='When this value is True, wss and https is used for wsd')
...
class HttpdServer(object):
    ...
    def setup_route(self):
        ...
        #router_methods = {kv[0]:kv[1] for kv in inspect.getmembers(self.app.router, inspect.ismethod)}
        ducts_app = web.Application()
        for route_add_type, path, methods, handler in self.context.event_handler_manager.web_handlers():
            if route_add_type == 'add_route':
                methods = '*' if methods == '' else methods
                methods = [methods] if type(methods) is not list else methods
                self.logger.debug('WEBAPI|PATH=%s|METHOD=%s', self._root_path_for_api()+path, methods)
                self.logger.debug('WEBAPI|PATH=%s|METHOD=%s', path, methods)
                for method in methods:
                    #self.app.router.add_route(method, self._root_path_for_api()+path, handler)
                    ducts_app.router.add_route(method, path, handler)
        self.app.add_subapp(self._root_path_for_api(), ducts_app)
                    
        subapp = web.Application()
        subapp.router.add_route('*', '/headers', self.print_headers)
        subapp.router.add_route('*', '/requests', self.print_requests)
        self.app.add_subapp('/ducts_test', subapp)

    async def print_headers(self, request):
        return web.Response(text='{}'.format(request.raw_headers), content_type="text/plain")
        return web.json_response()
        
    async def print_requests(self, request):
    ...
    async def _create_wsd_dict(self, request):
        ...
        params['ws_host'] = self.conf.wsd_host if self.conf.wsd_host else request.host
        params['ws_scheme'] = 'wss' if request.secure or self.conf.wsd_secure else 'ws'
        params['root_path_api'] = self._root_path_for_api()
        params['api_host'] = self.conf.wsd_host if self.conf.wsd_host else request.host
        params['api_scheme'] = 'https' if request.secure or self.conf.wsd_secure else 'http'
    ...
    async def _msgpack_msg_handler(self, socket_session, msg, received):
        ...
        try:
            ...
            if not await session.is_closed():
                event_id = -1 * event_id
                await session.send_bytes_if_available(request_id, event_id, e.__repr__())
