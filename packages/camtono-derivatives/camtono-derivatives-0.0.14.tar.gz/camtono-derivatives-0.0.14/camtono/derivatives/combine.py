def generate_derivative(definition: dict, feature_map: dict, single_query: bool = False,
    ...
    definition_outputs = definition.get('outputs', [dict(column_name=definition['grain'])])
    ...
    selects, group_by = generate_select_mapping(
        ...
        default_inputs=default_inputs, output_queries=output_queries,
        definition_outputs=definition_outputs,
        feature_map=feature_map, grain=definition['grain']
    )
    queries = generate_multi_query_skeleton(
        ...
        table_prefix=new_table_prefix, group_by=group_by
    )
    return queries
...
def standardize_input_attributes(definition):
    ...
    feature_inputs = {i['feature_id']: i.get('inputs', {}) for i in definition['features']}
