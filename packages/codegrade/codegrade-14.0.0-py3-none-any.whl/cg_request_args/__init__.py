...
def swaggerize(
    operation_name: t.Union[str, t.Mapping[str, str]],
    *,
    no_data: bool = False,
        ...
        if isinstance(operation_name, str):
            ...
            for method, name in operation_name.items():
                func_dict[method] = _SwaggerFunc(
                    name, no_data, func, query_parser=query_parser
                )
...
class RichValue:
    ...
    class TimeDeltaFactory(Transform[datetime.timedelta, t.Union[str, float]]):
    ...
    TimeDelta = TimeDeltaFactory(SimpleValue.str | SimpleValue.float)

    class _StringWithoutNullByte(_SimpleValue[str]):
        def __init__(self) -> None:
            super().__init__(str)
