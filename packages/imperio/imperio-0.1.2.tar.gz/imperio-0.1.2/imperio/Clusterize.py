...
class ClusterizeTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X : 'np.array', y : 'np.array' = None, **fit_params : dict):
        ...
        self.shape = X.shape
        if self.index:
            self.clustering_alg.fit(X[:, self.index])
        else:
            self.clustering_alg.fit(X)
        return self

    def transform(self, X: 'np.array', y=None, **fit_params) -> 'np.array':
        '''
        ...
        if self.shape[1] == X.shape[1]:
            self.X_copy = X.copy()
            if self.index:
                cluster = self.clustering_alg.predict(self.X_copy[:,self.index])
            else:
                cluster = self.clustering_alg.predict(self.X_copy)
