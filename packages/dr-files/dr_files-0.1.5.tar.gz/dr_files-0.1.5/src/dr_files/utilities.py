...
def dr_to_int_stream(
    file_path: os.PathLike,
) -> (MeasurementHeader, typing.List[int]):
    """
...
def dr_to_raw_values(
    file_path: str,
) -> (MeasurementHeader, typing.List[np.array]):
    """
...
def dr_to_csv(file_path: os.PathLike, out_file_path: os.PathLike):
    ...
    with open(out_file_path, "w+") as fout:
        ...
        time = 0
        fs = 1 / header.sample_rate
        for signal_values in zip(*signals):
            row = [time]
            row.extend(signal_values)
            time += fs

            rows.append(list(map(lambda value: format(value, ".10f"), row)))
...
def float_value_converter(header: MeasurementHeader.ChannelHeader):
    """
    Returns a generator function, which converts a stream of integer values to actual measurement
    values using the given header.
    """
    ...
    def fun(value):
        ...
        if header.db_reference > 0.0:
            ...
            if header.pregain > 0.0:
                value -= header.pregain
            value = pow(10, (value / 20)) * header.db_reference
        if not offset:
            offset = 0.0
        value = ((header.sensitivity * value) / 1000.0 - offset) / multiplier
        return value
...
def write_int_data(stream, data):
    stream.write(int_to_bytes(data))
...
class DrWriter:
    ...
    def write_header(self, header: dict) -> None:
        ...
        self._channels_count = len(header["channels"])
        measurement = MeasurementHeader()
        measurement.sample_rate = header["sample_rate"]
        for channel in header["channels"]:
            measurement.channels.append(
                MeasurementHeader.ChannelHeader(
                    reference=channel["reference"],
                    sensitivity=channel["sensitivity"],
                    db_reference=channel["db_reference"],
                    pregain=channel["pregain"],
                    offset=channel["pregain"],
                )
            )
        size = measurement.ByteSize()
        _proto_write(self.stream, measurement)

    def write_row(self, row) -> None:
        if len(row) != self._channels_count:
            raise ValueError("Length of row must match the length of channels")
        for item in row:
            write_int_data(self.stream, item)
