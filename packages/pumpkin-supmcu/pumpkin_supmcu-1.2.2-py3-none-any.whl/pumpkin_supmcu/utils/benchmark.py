...
from pumpkin_supmcu.supmcu.i2c import SupMCUMaster
...
BUS_FILE = Path(__file__).parent.parent / 'bus.json'
...
class Benchmark:
    ...
    def __init__(self, supmcu_master: SupMCUMaster, driver_master: I2CDriverMaster,
                 bus_definition: List[ModuleDefinition]):
        self.supmcu_master = supmcu_master
        self.driver_master = driver_master
    ...
    def measure_telem_read(self, module: SupMCUModuleDefinition, telem: SupMCUTelemetryDefinition,
        ...
        for i in self.delays():
            self.supmcu_master.request_delay = i
            try:
                response = self.supmcu_master.request_telemetry(module.name, telem_type, telem.idx)
                if response.header.ready:
                    return i
            except ValueError as e:
                logger.warning(e)
                return 0    # couldn't parse telem
            if i >= self.delay_max:
...
class BenchmarkApp(cli.Application):
    ...
    def main(self, *modules: str):
        ...
        bus: List[ModuleDefinition] = import_bus_telemetry_definition(Path(BUS_FILE))
        ...
        sup_master = SupMCUMaster(i2c_master=i2c_master, supmcu_modules=(list(mod.definition for mod in bus)))

        benchmark = Benchmark(sup_master, i2c_master, bus)
