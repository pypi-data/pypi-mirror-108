...
class MVPR_forward():
    ...
    def TSVD_error(self,X_val,V,S,U,ind):
        ...
        if self.target_standardisation:
            training_targets = self.training_targets - self.mean_targets
            validation_targets = self.validation_targets - self.mean_targets

            if 0 not in self.std_targets :
                training_targets /= self.std_targets
                validation_targets /= self.std_targets
        else:
            training_targets = np.copy(self.training_targets)
            validation_targets = np.copy(self.validation_targets)

        SIGMA = np.diag(S)

        T_INV =np.matmul( np.matmul(V[:, 0: ind],np.linalg.inv(SIGMA[0:ind, 0:ind] )),np.transpose(U[:, 0: ind]))
        B = np.matmul(T_INV , training_targets)

        predicted_validation_profiles = np.matmul(X_val , B)

        error = (np.linalg.norm(validation_targets - predicted_validation_profiles, axis=1) / np.linalg.norm(
            validation_targets, axis=1)) * 100
    ...
    def Tik_error(self, X_val, X_train, lambda_param):
        ...
        XTA = np.matmul(np.transpose(X_train), training_targets)
    ...
    def find_order(self):
        ...
        if self.training_standardisation:
            training_data = self.training_data - self.mean_data
            validation_data = self.validation_data - self.mean_data

            if 0 not in self.std_data :
                training_data /= self.std_data
                validation_data /= self.std_data
        else:
        ...
        for i in range(0,len(order),1):
            ...
            X_train=poly.fit_transform(training_data)
            X_val = poly.fit_transform(validation_data)
