...
class K8sIntegration(Worker):
    ...
    def run_one_task(self, queue: Text, task_id: Text, worker_args=None, **_):
        ...
        container['container'] = container.get('container') or str(
            ENV_DOCKER_IMAGE.get() or self._session.config.get("agent.default_docker.image", "nvidia/cuda"))
        container['arguments'] = container.get('arguments') or \
            self._session.config.get("agent.default_docker.arguments", "nvidia/cuda")
