...
class Session(object):
    ...
    def connect(self,
        ...
        self.repository.push_cmd(self.remote.command, auth_request)
    ...
    def remove(self, uuid, remove_dependencies=True):
        ...
        if self.repository.get_node(uuid):
            nodes_to_delete = []
            ...
            for node in nodes_to_delete:
                ...
                self.repository.push_cmd(self.remote.command, delete_command)
        else:
    ...
    def kick(self, user):
        ...
        if user == self._id:
            ...
            return
        if self.is_admin():
            kick_order = Kick(owner=self._id,
                                        data={'user': user})
            self.repository.push_cmd(self.remote.command, kick_order)
        else:
    ...
    def change_owner(self,
        ...
        for n in affected_nodes:
            try:
                ...
                self.repository.push_cmd(self.remote.command, right_command)
    ...
    def update_user_metadata(self, dikt):
        ...
        self.repository.push_cmd(self.remote.command, user_state_update)

    # TODO: remove
    def list(self, filter=None, filter_owner=None):
        """List all graph nodes keys
        :param filter: node type
        :type filter: Node class (or child class)
        """
    ...
    def request_snapshot_init(self):
        ...
        self.repository.push_cmd(self.remote.command, snapshot_request)
    ...
    def request_server_repository_init(self):
        ...
        snapshot_cmd = ServerSnapshot(
            ...
            data={'STATE': 'INIT',
                  ...
                  'NODES': keys})
        self.repository.push_cmd(self.remote.command, snapshot_cmd)
    ...
    def listen(self, timeout: int = 0):
        ...
        if self.remote.command in sockets:
            try:
                command = self.repository.fetch_cmd(self.remote.command)
                ...
                if isinstance(command, UpdateUserMetadata):
                    user = self._online_users.get(command.owner)
        ...
        if self.remote.data in sockets:
            try:
                datablock = self.repository.fetch_data(self.remote.data)
            except Exception as e:
                logging.error(
                    f"Corrupted frame received, skipping it. Cause:{e}")
                traceback.print_exc()
