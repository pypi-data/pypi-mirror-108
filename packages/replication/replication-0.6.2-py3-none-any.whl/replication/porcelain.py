...
def apply(repository,
        ...
        """
        node = repository.nodes.get(node_id)
        ...
        if node and (node.state in [FETCHED, UP] or force):
            ...
            apply_queue = []
            for dep in deps:
                dep_node = repository.nodes.get(dep)
...
def commit(repository: Repository, node_id: str):
        ...
        for dep_uuid in repository.get_dependencies_ordered(node=node_id):
            dep = repository.nodes.get(dep_uuid)
...
def push(repository: Repository, remote: str, node_id, force=False):
    ...
    for node_id in push_queue:
        repository.push_data(remote.data, node_id)
...
def lock(repository: Repository,
    ...
    if affect_dependencies:
        affected_nodes.extend(
            repository.get_dependencies_ordered(node=node_id))
    ...
    for n in affected_nodes:
        try:
            ...
            if ignore_warnings:
                node_ref = repository.nodes.get(n)
                logging.debug(
            ...
            repository.push_cmd(repository.remote.command, right_command)
...
def kick(repository: Repository, username: str):
    ...
    if repository.remote.is_admin():
        kick_cmd = Kick(owner=repository.username,
                        data={'user': username})
        repository.push_cmd(repository.remote.command, kick_cmd)
    else:
...
def rm(repository: Repository, node_id: str, remove_dependencies: bool=True):
    ...
    if repository.nodes.get(node_id):
        nodes_to_delete = []
        ...
        for node in nodes_to_delete:
            ...
            repository.push_cmd(repository.remote.command, delete_command)
    else:
...
def update_user_metadata(repository: Repository, metadata_fieds: dict):
    ...
    repository.push_cmd(repository.remote.command, user_state_update)
