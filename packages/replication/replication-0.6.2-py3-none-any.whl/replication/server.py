...
from replication.objects import Node, ReplicationObject, Auth, Delete, Disconnect, Kick,Right, ServerSnapshot, Snapshot,UpdateClientsState, UpdateUserMetadata
...
class ServerNetService(threading.Thread):
            ...
            for rr_cmd in cleanup_commands:
                for cli in self.clients.keys():
                    if cli != client and not self._command._closed:
                        ...
                        self._repository.push_cmd(self._command, rr_cmd)
    ...
    def _update_clients_states(self):
        ...
        for client_uid in self.clients:
            ...
            self._repository.push_cmd(self._command, clients_states)
    ...
    def kick(self, user):
        ...
        for k, v in self.clients.items():
            if v['id'] == user:
                ...
                self._repository.push_cmd(self._command, disconnect)
    ...
    def send_client_snapshot_init(self, client):
        ...
        self._repository.push_cmd(self._command, snapshot_state)
    ...
    def handle_client_snapshot(self, command):
        ...
        if snapshot_state == 'REQUEST_INIT':
            ...
            node = self._repository.nodes.get(command.data['ID'])
            ...
            self._repository.push_cmd(self._command, snapshot_cmd)
    ...
    def run(self):
        ...
        while not self._exit_event.is_set():
            ...
            if self._command in socks:
                try:
                    command = self._repository.fetch_cmd(self._command)
                    ...
                    if isinstance(command, Auth):
                        ...
                        self._command.send(command.sender, zmq.SNDMORE)
                        self._repository.push_cmd(self._command, auth_response)
                            ...
                            try:
                                ...
                                for client_uid in self.clients:
                                    if client_uid != command.sender:
                                        self._command.send(client_uid, zmq.SNDMORE)
                                        self._repository.push_cmd(self._command, command)
    ...
    def run(self):
        ...
        while not self._exit_event.is_set():
            ...
            if self._data in socks:
                ...
                try:
                    datablock = self._repository.fetch_data(self._data)
                    ...
                    for client_uid, client_data in self.clients.items():
                        if client_uid != datablock.sender:
                            self._repository.push_data(self._data,
                                                datablock.uuid,
                                                identity=client_uid,
                                                force=True)
