...
class Repository():
    ...
    def get_orphans_nodes(self):
        return list(set(self.object_store.keys()).difference(self.list_ordered()))
    ...
    def list_ordered(self):
        return reversed(self.object_store.bfs(self.heads))
    ...
    def assert_modification_rights(self, node_id):
        if self.nodes.get(node_id).owner not in [self.username, RP_COMMON]:
