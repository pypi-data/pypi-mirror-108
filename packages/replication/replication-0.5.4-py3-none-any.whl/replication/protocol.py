...
class DataTranslationProtocol(object):
    ...
    def register_implementation(
        ...
        if type(dcc_types) is list:
            for dcc_type in dcc_types:
                ...
                logging.info(f"Registering DCC type {dcc_type.__name__}")
        else:
            self._supported_types[dcc_types.__name__] = implementation
            logging.info(f"Registering DCC type {dcc_types.__name__}")
    ...
    def get_implementation(self, datablock):
        return self._supported_types.get(type(datablock).__name__)
...
class ReplicatedDatablock(object):
    ...
    def compute_delta(last_data:dict, current_data: dict)-> Delta:
        ...
        :return: deepdiff.Delta
        """
        return Delta(DeepDiff(last_data, current_data, cache_size=5000), mutate=True)
