...
from ..modules.modules import TransformerEncoder, EmbeddingsWithTokenTypes
...
class ElectraForSequenceClassification(Module):
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor, token_type_ids: Optional[Tensor] = None):
        sequence_output = self.transformer(input_ids, attention_mask, token_type_ids)
        return self.classifier(sequence_output)

    @classmethod
    def from_pretrained(cls, model_path: str, num_labels: int) -> 'ElectraForSequenceClassification':
        model = cls(
            vocab_size=30522, token_types=2, dim=768, pad_token_id=0, max_position_embeddings=512,
            ffn_size=3072, n_heads=12, n_layers=12, num_labels=num_labels
        )
