...
def get_query_url_for_author_without_gender(info, mode="basic"):
    query = (
        """
# title: Authors on Wikidata without a gender property
SELECT
...
def get_query_url_for_articles(info, mode="basic"):
  ...
  (URI(CONCAT('https://doi.org/', ?DOI_)) AS ?DOI)
  ?work 
...
"""
  ...
  GROUP BY ?work ?workLabel ?venueLabel ?DOI_
  ORDER BY DESC(?date)  
...
def get_topics_as_table(info, mode="basic"):
  ...
  ?example_work 
  WITH {
    SELECT (COUNT(?work) AS ?n) ?theme (SAMPLE(?work) AS ?example_work)
    WHERE {
    """
        + get_selector(info, mode)
        + """
...
def get_query_url_for_venues(info, mode="basic"):
  ...
  GROUP BY ?n ?venue ?venueLabel ?topics ?topicsUrl
  ORDER BY DESC(?n)
...
def get_query_url_for_citing_authors(info, mode="basic"):
  ...
  WITH {
    SELECT (COUNT(?citing_work) AS ?n) ?citing_author WHERE {
    """
        + get_selector(info, mode)
        + """
      ?citing_work wdt:P2860 ?work . 
