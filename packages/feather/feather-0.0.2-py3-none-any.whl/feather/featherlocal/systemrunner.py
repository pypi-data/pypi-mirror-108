...
class StepInfo:
    ...
    def _serialize_inputs(self, outputSchema):
        ...
        if len(argNames) != len(self.inputs):
            raise RuntimeError("Step '{0}' expects {1} argument but {2} were found during serialization".format(self.name, len(argNames), len(self.inputs)))
        ...
        for idx in range(len(self.inputs)):
            ...
            if isinstance(stepInput, components.Component):
                stepInput._serialize_schema(inputSchema)
                payloadSchema  = stepInput._get_payload_schema()
                inputSchema.schema = json.loads(payloadSchema)
            elif isinstance(stepInput, components.FeatherComponent):
                stepInput.component._serialize_schema(inputSchema)
                payloadSchema = stepInput.component._get_payload_schema()
                inputSchema.schema = json.loads(payloadSchema)
            else:
                inputSchema.type = type(stepInput).__qualname__ # :TODO: Check for specific types (ie. we can't support everything...)
    ...
    def _serialize_ouptuts(self, outputSchema):
        for idx in range(len(self.inputs)):
            ...
            if isinstance(stepInput, components.Component):
                stepInput._serialize_schema(info)
            elif isinstance(stepInput, components.FeatherComponent):
                stepInput.component._serialize_schema(info)
            else:
                info.type = type(stepInput).__qualname__ # :TODO: Check for specific types (ie. we can't support everything...)
...
class SystemRunner:
    ...
    def get_system_info(self) -> str:
        ...
        ret.context_id = self.context_id
    ...
    def publish(self, api_key: str):
        if self.file_bundle == None or type(self.file_bundle) != publisher.Bundle:
            raise ValueError("A file_bundle needs to be provided in order to publish. The bundle should contain all your code and models needed to run your system")

        for step in self.steps:
            if step.evaluated == False and step.isEnd == False:
                raise RuntimeError("Error during publish - not all steps were evaluated! You can only publish once all steps have evaluated successfully")
        ...
        self.file_bundle.do_publish(name=self.name, version="0.0.0", 
    ...
    def run_step(self, stepIndex: int, inputPayloads: str):
        ...
        if stepIndex < self.currentStep:
            ...
            raise ValueError("change_step: Attempt to jump >1 step at once")
    ...
    def _run_func(self, name, func, inputs):
        try:  
            ...
            if not isinstance(outputs, list):
                raise TypeError("Step '{0}' needs to return a List".format(name))
            return outputs, components.step_output_adapter(outputs)
        except Exception as e:
            raise e
            #raise RuntimeError("Step Execution failed for:", name, "with error=", e)
    ...
    def _next_step(self, input_payloads):
        ...
        if self.currentStep < len(self.steps):
            ...
            self.steps[self.currentStep].inputs = step.outputs if step.outputs != None else []
