...
import flask
from feather.helpers import dump
...
class GetSystemInfoHandler(Resource):
    ...
    # The return value includes a "context_id" which is unique for this system.
...
class GetStepInputsHandler(Resource):
    ...
    def get(self, contextId, stepIndex):
        global _runner
        if _runner == None:
            return None, 404
        if _runner.context_id != contextId:
...
class RunStepHandler(Resource):
    ...
    #   ]
    # }
    def put(self, contextId, stepIndex): 
        if _runner.context_id != contextId:
            return None, 410 # GONE
...
_api.add_resource(GetStepInputsHandler, "/v1/system/<string:contextId>/step/<int:stepIndex>/info")
_api.add_resource(RunStepHandler, "/v1/system/<string:contextId>/step/<int:stepIndex>")
_api.add_resource(DoPublish, "/v1/system/<string:contextId>/publish")
...
# context_id in the running system. The browser should detect a change of ID and reset
# the experience
#----------------------------------------------------------------
