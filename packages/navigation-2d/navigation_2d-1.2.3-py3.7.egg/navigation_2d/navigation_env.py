...
class NavigationEnvDefault(gym.Env, EzPickle):
    ...
    def randomize_goal(self):
        ...
        self.task_args['Goal'] = goal_set[goal]
        return
...
class NavigationEnvAcc(NavigationEnvDefault):
    ...
    def step(self, action: np.iterable):
        ...
        v = np.clip(v, a_min=[-1, -1], a_max=[1, 1])
...
class RandomInitNavigationEnvAccLidarObs(NavigationEnvAccLidarObs):
    @property
    def drone_start_pos(self):
        pos = np.zeros(2)
        a = np.random.random()
        ...
        if a < 0.5:
            ...
            if a < 0.75:
                ...
                pos[1] = 1
            pos[0] = b
        return denormalize_position(pos, W ,H)

class NonStationaryNavigation(NavigationEnvAccLidarObs):
    def __init__(self, task_args, max_obs_range=3, max_speed=5, initial_speed=2, **kwargs):
        super().__init__(task_args, max_obs_range, max_speed, initial_speed, **kwargs)
        self.theta = 0
        self.speed = task_args['speed']
        self.task = 'clockwise'
    ...
    def step(self, action):
        ...
        if self.timesteps % (40 // self.speed) == 0:
            self.moving_goal()
        return state, reward, done, info

    def moving_goal(self):
        # if self.task == 'clockwise':
        #     self.theta += np.pi / 180 * self.speed
        # elif self.task == 'counterclockwise':
        #     self.theta += np.pi / 180 * self.speed
        self.theta += np.pi * 2 * np.random.random()
        goal = denormalize_position(np.array([np.cos(self.theta), np.sin(self.theta)]) * 0.424 + np.array([0.5, 0.5]), W, H)
        self.task_args['Goal'] = goal
