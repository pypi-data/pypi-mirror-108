...
def rasterAggregated(mapId, timestamp, bounds, approximate = True, token = None):
    sh = gpd.GeoDataFrame({'geometry':[bounds]})
...
def seriesGet(shapeId, layerId, geometryId, propertyName = None, dateFrom = None, dateTo = None, userId=None, limit = 5000, token  = None):
    mapId = shapeId
    body = {'mapId':mapId, 'geometryId':geometryId, 'layerId':layerId}
    if str(type(userId)) != str(type(None)):
        body['userId'] = userId
...
def removeShapeLayer(shapeId, layerName, token):
    mapId = shapeId
...
def mapVisualisationAdd(mapId, name, method, bands, parameters, token):
    ...
    body = {'mapId': mapId, 'name':name, 'method':method, 'bands':list(bands), 'overwrite':True, 'parameter':parameter, 'dryRun':False}
...
def mapVisualisationRemove(mapId, name, token):
    body = {'mapId': mapId, 'name':name}
