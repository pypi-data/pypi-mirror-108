...
def _calc_extinct(wavelength, n_env, alpha_tensor, E0, E, 
                  with_radiation_correction=False, normalization_E0=False):
...
def extinct(sim, field_index, with_radiation_correction=False, normalization_E0=False):
...
def multipole_decomp_extinct(sim, field_index, r0=None, eps_dd=0.1, quadrupoles=False):
    ...
    ** ------- FUNCTION STILL UNDER TESTING ------- **
    ...
    geo = sim.struct.geometry
    n_env = sim.dyads.getEnvironmentIndices(wavelength, geo[:1])[0]  # assume structure is fully in one environment
    E = sim.E[field_index][1]
    ex, ey, ez = E.T
...
# =============================================================================
    ...
    sigma_ext_p = (4 * np.pi * k0 / n_env * 1/E2in * np.sum(np.conjugate(E0)*p)).imag
    sigma_ext_m = (4 * np.pi * k0 / n_env * 1/E2in * np.sum(np.conjugate(H0)*m)).imag
...
# =============================================================================
    if quadrupoles:
        ...
        sigma_ext_q = (np.pi * k0 / n_env * 1/(3. * E2in) * 
        ...
        sigma_ext_mq = (2 * np.pi * k0 / n_env * 1/E2in * 
