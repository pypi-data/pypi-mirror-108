...
def receive_from_can_isotp_socket(interface, rx_addr, tx_addr, bufsize, use_padding, q):
    """
    ...
    :return: None.
    """
    s = CanIsoTpSocket(interface=interface, rx_addr=rx_addr, tx_addr=tx_addr, use_padding=use_padding)
...
class TestCanIsoTpSocket:
    ...
    def test_can_isotp_socket(self, can_interface):
        ...
        p = Thread(target=receive_from_can_isotp_socket,
                   args=(can_interface, tx_addr, rx_addr, bufsize, use_padding, q,))
        p.setDaemon(True)
        p.start()
        time.sleep(1)
        s.send(data)
...
#                     LOGGER.error(
#                         "Re-synced queue after {0} dropped frames (overall {1})".format(cnt, dropped_frames_on_interface))
#                 framecount = framecount + 1
#
