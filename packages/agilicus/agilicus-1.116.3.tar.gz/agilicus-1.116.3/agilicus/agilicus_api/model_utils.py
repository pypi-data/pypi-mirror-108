...
class ModelComposed(OpenApiModel):
    ...
    def __setitem__(self, name, value):
        ...
        for model_instance in self._composed_instances:
            intersection = find_args_intersection(model_instance.__class__, {name: value})
            if len(intersection):
                setattr(model_instance, name, value)
                found_intersection = True
        if not found_intersection:
            # we didn't find an intersection for the property, it could be an
            # additional type, so pass it through
            for model_instance in self._composed_instances:
                setattr(model_instance, name, value)
...
def find_args_intersection(cls, potential_args):
    intersected_args = {}

    for key, value in potential_args.items():
        if key in cls.openapi_types:
