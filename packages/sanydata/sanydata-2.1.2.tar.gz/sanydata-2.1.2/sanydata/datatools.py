...
class DataTools(object):
    ...
    @stub_channel
    def get_data(self, stub, file_list, columns=None, map_fc=None, map_v=None):
        """
        获取文件数据
        :param file_list：所要获取文件列表
        ...
        for file_name, f in zip(files_name, unzip_files):
            if file_list[0].split('.')[-1] == 'parquet':
                ...
                try:
                    ...
                    df = pd.read_csv(StringIO(pure_data.decode('gbk')), header=header, names=names)
                df['turbine_num'] = turbine_num
                df = df[columns + ['turbine_num']] if columns else df
                if check_result == 1:
                    df = map_fc(data=df)
                elif check_result == 2:
                    df = map_fc(data=df, map_v=map_v)
