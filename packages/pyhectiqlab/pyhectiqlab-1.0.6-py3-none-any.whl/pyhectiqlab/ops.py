...
def refresh_token(refresh_token):
    res = lab_request(path="/update-token", 
                request_type="POST", 
                body_args=dict(refresh_token=refresh_token))
    return res

def login(username, password):
    res = lab_request(path="/login", 
                request_type="POST", 
...
def log_mlmodel(run_id, mlmodel_name, version, project_id, token):
    res = lab_request(path=f"/runs/{run_id}/log_mlmodel",
                     request_type="POST",
                     token=token,
                     query_args=dict(mlmodel_name=mlmodel_name, version=version, project_id=12))
    return res
