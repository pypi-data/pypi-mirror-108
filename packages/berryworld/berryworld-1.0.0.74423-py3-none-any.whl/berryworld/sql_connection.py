...
class SQLConnection:
    ...
    def insert(self, data, schema, table, truncate=False, delete=False, identity=False, chunk=1000, print_sql=False,
               with_brackets=False, commit_all_together=False, output=None):
        ...
        try:
            ...
            if with_brackets:
                ...
                insert_ = insert_ + str(tuple(data.columns.values)).replace("\'", "")
            if len(output) > 0:
                insert_ = insert_ + " OUTPUT Inserted.[" + "], Inserted.[".join(output) + "] "
            insert_ = insert_ + """ VALUES """
    ...
    def insert2(self, data, schema, table, truncate=False, delete=False, identity=False, print_sql=False,
                fast_insert=False, output=None):
        """ Insert data in a table in SQL truncating the table if needed
        -----------------------------
        df = pd.DataFrame({'col1': ['a', 'b'], 'col2': [1, 2]})
        con_.insert(df, table_schema, table_name)
        ...
        :param identity: Indicate whether the identity columns will be inserted or not
        :param print_sql: boolean to indicate that you want the sql_statement to be printed on the console
        :param fast_insert: Try a 100x times faster update. You have to make sure the decimal values correctly align
        the values python is providing. For instance, having [Value] DECIMAL (5,2) will mean you have to provide 2
        exact decimals when uploading the data for column Value
        :param output: Outputs the columns indicated in this list
        :return: None
        """
        if output is None:
        ...
        try:
            ...
            # Extract the columns to parse
            col_insert = data.columns.values.tolist()

            sql_statement = 'INSERT INTO [%s].[%s] (' % (schema, table) + '[' + '], ['.join(col_insert) + ']' + ') '

            if len(output) > 0:
                sql_statement += ' OUTPUT Inserted.[' + '], Inserted.['.join(output) + '] '

            sql_statement += ' VALUES (' + ', '.join(['?'] * len(col_insert)) + ')'
            ...
            list_of_variables = data.values.tolist()
            ...
            if len(sql_statement) > 0:
                if len(output) > 0:
                    cursor.executemany(sql_statement, list_of_variables)
    ...
    def update2(self, data, update_list, on_list, schema, table, bool_cols=None, print_sql=False, fast_update=False,
                output=None):
        """ This method updates a table in batches in sql server.
        -----------------------------
        UPDATE [SCHEMA].[TABLE]
        ...
        :param print_sql: boolean to indicate that you want the sql_statement to be printed on the console
        :param bool_cols: columns to include as booleans
        :param fast_update: Try a 100x times faster update. You have to make sure the decimal values correctly align
        the values python is providing. For instance, having [Value] DECIMAL (5,2) will mean you have to provide 2
        exact decimals when uploading the data for column Value
        :param output: Outputs the columns indicated in this list as a DataFrame. It should indicate if the column to
        retrieve is the inserted one or the deleted one (If nothing is indicated, then the Deleted one will be
        ...
        try:
            ...
            # Extract the columns to parse
            total_updates = [x for x in update_list if x not in on_list] + on_list
            data_update = data[total_updates]
            ...
            sql_statement = sql_statement[:-1] + ' WHERE '
            for col in on_list:
                sql_statement += " [%s] = ? AND " % col
            sql_statement = sql_statement[:-4]

            # Handle null values and apostrophes (')
            data_update = data_update.replace("'NULL'", np.nan)
            data_update = data_update.replace("'null'", np.nan)
            data_update = data_update.replace("NULL", np.nan)
            data_update = data_update.replace("null", np.nan)
            data_update = data_update.replace("'", "~~", regex=True)
            data_update = data_update.where(pd.notnull(data_update), None)

            list_of_variables = data_update[total_updates].values.tolist()
