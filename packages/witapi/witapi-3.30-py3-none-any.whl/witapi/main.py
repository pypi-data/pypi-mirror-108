...
from witapi.auth import auth_module as am
from witapi.errors import NoSQLshellSupplied
import struct
from witapi import methods


class WApi:
    """Super class for API (Server&Client)"""

    def __init__(self, debug=False, assync_operating=True):
    ...
    def send_data(self, sock, data):
        ...
        print('\tОтправка длины', data_length)
    ...
    def unpickle_data(self, sock, data_size):
        ...
        print('data length', data_length_unpacked)
        data = b''
...
class WITClient(WApi):
    ...
    def subscribe(self, command=None):
        ...
        if not command:
            command = {methods.methods_dict['subscribe']['method']: methods.methods_dict['subscribe']['args']}
    ...
    def get_data(self, *args, **kwargs):
        ...
        функция собирает объект по частям, пока не соберет его весь и возвращает результат"""
        self.show_print('\nОжидаем данные')
        data = self.unpickle_data(self.sock, 4096)
        return data
...
class WITServer(WApi):
    ...
    def __init__(self, myip, myport, without_auth=False, mark_disconnect=True, debug=False, sqlshell = None,
        ...
        super().__init__(debug=debug, assync_operating=assync_operating)
    ...
    def dispatcher(self, conn, conn_dict):
        ...
        while True:
            ...
            try:
                ...
                sleep(1)
                threading.Thread(target=self.command_execute_queue, args=(conn_dict, conn, command)).start()
    ...
    def check_auth(self, conn_dict, command, connection):
        ...
        for comm, values in command.items():
            if conn_dict[connection]['auth']:
                response = self.execute_command(comm, values, connection)          # Выполнить пользовательские команды
            else:
                if comm == 'authMe':
                    response, auth = am.auth_user(self.sqlshell, values['login'], values['password'], connection)
                    if auth:
                        conn_dict[connection]['auth'] = True
                        self.after_auth_execute(values['login'])
                else:
    ...
    def execute_command(self, comm, values, conn, *args, **kwargs):
        if comm == 'subscribe':
            for connection in self.connections:
                try:
                    connection[conn]['subscriber'] = True
                except KeyError:
                    pass
            return {'status': 'success', 'info': 'Subscribed'}
        elif comm == 'unsibscribe':
            for connection in self.connections:
                try:
                    connection[conn]['subscriber'] = False
                except KeyError:
                    pass
            return {'status': 'success', 'info': 'Unsubscribed'}
        else:
            return {'status': 'failed', 'info': 'Для комманды {} не прописана логика.'.format(comm)}
