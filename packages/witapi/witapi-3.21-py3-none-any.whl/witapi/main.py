...
class WITServer(WApi):
    ...
    def check_auth(self, conn_dict, command, connection):
        ...
        for comm, values in command.items():
            if conn_dict[connection]['auth']:
                self.execute_additional(comm, values, connection)
                response = self.execute_command(comm, values)               # Выполнить пользовательские команды
            else:
                if comm == 'authMe':
    ...
    def execute_command(self, comm, values):
        self.show_print('Got command: {} with values {}. Working....'.format(comm, values), debug=True)
        sleep(10)
        return 'WITApi server execution is not realised yet!'

    def execute_additional(self, comm, values, conn):
        """ Выполняет заданную команду не возвращая ответ """
        if comm == 'subscribe':
            for connection in self.connections:
