...
from witapi.auth import auth_settings as s
...
def auth_user(sqlshell, login, password, connection):
    ...
    if response:
        ...
        user_id = save_conn_ip(sqlshell, s.users_tablename, s.login_name, login, ip)['info'][0][0]
...
def check_if_auth_succes(response):
    ...
    if len(response) == 0 or not response[0][0]:
        return False
    else:
