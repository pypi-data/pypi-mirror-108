...
class CreateObjectRelationshipCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = """
    Create an object replication relationship that initiates a copy of file data to S3.
    """
    ...
    def options(parser):
        ...
        parser.add_argument(
            '--object-folder',
            required=True,
            help="""Replication destination folder in the bucket. A slash separator is
                automatically used to create a folder. For example, a folder "example"
                and a file path (relative to source directory) "dir/file" results in key
                "example/dir/file". A value of "" or "/" replicates to the root of the
                bucket.""",
        )
        parser.add_argument(
            '--use-port',
