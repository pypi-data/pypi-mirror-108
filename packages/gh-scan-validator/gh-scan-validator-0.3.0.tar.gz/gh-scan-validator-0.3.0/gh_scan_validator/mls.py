...
from tse_location.las_reader import get_container_client, download_blob
...
def load_camera_orbit_csv(csv_path: Path) -> pd.DataFrame:
    if csv_path.is_file():
        ...
        df = pd.read_csv(csv_path, header=0, names=header,)
        ...
        header, float_fields = get_header(csv_path)
        df = pd.read_csv(common_camera_csv_path, header=0, names=header,)
...
class Validator(object):
    ...
    def validate_image_folder(self):
        ...
        cameras = []
        topodot_cal_files = self.get_topdot_files(images_path)
    ...
    def get_topdot_files(self, images_path: Union[str, Path]):
        raise NotImplementedError("This is an abstract class, please not use directly")
...
class LocalValidator(Validator):
    ...
    def get_topdot_files(self, images_path: List):
...
class BlobValidator(Validator):
    def __init__(self, scan_path: Path, scan_name: str = None, cache_folder=Path("/cache/scan_input_validator")):
        ...
        self.topdof_files_cached = False
        ...
        for recording in recordings:
            ...
            for blob_csv in blob_csvs:
                ...
                if camera_name not in cameras:
                    error(f"Cannot find {camera_name} from topdot files")
    ...
    def get_topdot_files(self, images_path: Union[str, Path], download=False):
        ...
        for cal_file in cal_files:
            ...
            if download and not self.topdof_files_cached:
                download_blob(cal_file["name"], self.cache_folder / Path(cal_file["name"]).name)
        if download:
            self.topdof_files_cached = True
