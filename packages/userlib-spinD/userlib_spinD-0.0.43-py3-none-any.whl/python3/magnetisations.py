...
class MultiLayer:
    r"""
    class for multilayer spin lattices. Consists out of instances of monolayer lattices
    """

    def __init__(self, size=None, key=None, number_layers=None, path=None):
        r"""
        Initializes multilayer
        """
        ...
        if path != None:
            ...
            for n in range(len(data)):
                ...
                self.Layer[index_layer].X[n // number_layers], self.Layer[index_layer].Y[n // number_layers], self.Layer[index_layer].Z[
                    n // number_layers] = data[n][0], data[n][1], data[n][2]
                self.Layer[index_layer].mx[n // number_layers], self.Layer[index_layer].my[n // number_layers], self.Layer[index_layer].mz[
                    n // number_layers] = data[n][3], data[n][4], data[n][5]
                self.Layer[index_layer].magmom[n // number_layers] = data[n][6]
...
class SpinLattice():
    ...
    # ==============================================
    def addSkyrmion(self, origin: Union[List, np.ndarray, None] = None, vorticity: int = 1, helicity: float = np.pi,
                    c: float = 2.5, w: float = 3.0, AFM=False, uplo=1, elongation=False, angl_elongation=0.,
                    ab_elongation=[1., 1.]) -> None:
        r"""
        Adds a skyrmion to the spin lattice

        Args:
            origin(List, np.ndarray or None): Position of the center of the skyrmion. If None the skyrmion will be
            placed in the center of the lattice

            vorticity(int): Vorticity of a skyrmion (topological charge). Default is 1 (Skyrmion)

            helicity(float): a float from [0, 2pi]. The default is a Néel Skyrmion (pi)

            c(float): related to size of skyrmion

            w(float): width of the border region of the skyrmion corresponds to width of domain wall.
        ...
        """
        if origin is None:
            origin = [round(float(np.mean(self.X)), 4), round(float(np.mean(self.Y)), 4)]
        mag = Magnetisation_Skyrmion(self.X, self.Y, origin, vorticity, helicity, c, w, AFM, uplo, elongation,
                                     angl_elongation, ab_elongation)
    ...
    def add_chimera(self, ursprung, vorticity, helicity, c, w, AFM=False, uplo=1, sym_chimera=True, angl_chimera=0.,
        ...
        mag = Magnetisation_Chimera(self.X, self.Y, ursprung, vorticity, helicity, c, w, AFM, uplo, sym_chimera,
            ...
            for m in range(self.size + 1):
                ...
                if n % index_dist == 0 and m % index_dist == 0:
                    if n >= self.size and m >= self.size:
                        ...
                        pos = [self.X[skyrmion_index] + self.X[self.size] + self.X[1] + delta_X,
                               self.Y[skyrmion_index] + self.Y[self.size] + self.Y[1] + delta_Y]
                    elif n >= self.size:
                        skyrmion_index = (n - 1) * self.size + m
                        pos = [self.X[skyrmion_index] + self.X[self.size] + delta_X,
                               self.Y[skyrmion_index] + self.Y[self.size] + delta_Y]
                    elif m >= self.size:
                        skyrmion_index = n * self.size + m - 1
                        pos = [self.X[skyrmion_index] + self.X[1] + delta_X,
                               self.Y[skyrmion_index] + self.Y[1] + delta_Y]
...
class Magnetisation_Skyrmion():
    def __init__(self, X, Y, pos0, m, g, c, w, AFM, uplo, elongation, angl_elongation, ab_elongation):
        self.mx, self.my, self.mz = self.build_skyrmion(X, Y, pos0, m, g, c, w, uplo, AFM, elongation, angl_elongation,
                                                        ab_elongation)
    ...
    def build_skyrmion(self, X, Y, pos0, m, g, c, w, uplo, AFM, elongation, angl_elongation, ab_elongation):
        ...
        for n in range(len(X)):
            ...
            if elongation:
                ...
                r = r * (2. - np.sqrt((ab_elongation[0] * np.cos(p - angl_elongation)) ** 2 + (
                        ab_elongation[1] * np.sin(p - angl_elongation)) ** 2))
...
class Magnetisation_Chimera():
    def __init__(self, X, Y, pos0, m, g, c, w, AFM, uplo, sym_chimera, angl_chimera, elongation, angl_elongation,
                 ab_elongation):
        self.mx, self.my, self.mz = self.build_chimera(X, Y, pos0, m, g, c, w, uplo, AFM, sym_chimera, angl_chimera,
                                                       elongation, angl_elongation, ab_elongation)
    ...
    # respect to the profile parameters, helicity and vorticity
    # ==========================================================
    def build_chimera(self, X, Y, pos0, m, g, c, w, uplo, AFM, sym_chimera, angl_chimera, elongation, angl_elongation,
                      ab_elongation):
        assert len(X) == len(Y)
        mx, my, mz = [], [], []
        for n in range(len(X)):
