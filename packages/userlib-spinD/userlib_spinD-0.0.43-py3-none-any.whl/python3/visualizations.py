...
# ============================================
def write_STM(sp, name: Union[str, Path] = 'SpinSTMi.dat'):
...
def show_lattice(spinlattice, details=None, filename='spinorientation.png', zoom=None):
    ...
    def DWdetails(ax):
        ...
        ax.arrow(0, 0, direction[0] * size, direction[1] * size, head_width=1, head_length=1, color='k', ls='-',
                 width=0.00002)
        ax.arrow(R0[0], R0[1], -direction[1] * size / 2.0, direction[0] * size / 2.0, head_width=0, head_length=0,
                 color='k', ls='-', width=0.00002)
        ax.arrow(R0[0], R0[1], direction[1] * size / 2.0, -direction[0] * size / 2.0, head_width=0, head_length=0,
                 color='k', ls='-', width=0.00002)
...
def show_eigenvector_translation(spinlattice, vectorlattice, amplitude):
    ...
    for m in range(len(spinlattice.mx)):
        axis, ang = rotate_to_axis([vectorlattice.mx[m], vectorlattice.my[m], vectorlattice.mz[m]],
                                   [spinlattice.mx[m], spinlattice.my[m], spinlattice.mz[m]])
...
def show_topodensity(spinlattice, filename='topolodensity.png', zoom=None):
    ...
    for p in range(len(qs)):
        if zoom != None:
            if any([xs[p][n] > xc + dist for n in range(3)]) or any([xs[p][n] < xc - dist for n in range(3)]) or any(
                    [ys[p][n] > yc + dist for n in range(3)]) or any([ys[p][n] < yc - dist for n in range(3)]):
                continue
            else:
    ...
    def NNangles_hex(splat):
        ...
        for n in range(size):
            ...
            for m in range(n + 1, size):
                ...
                if d < 4:
                    if d == 1.0:
                        bin1.append(angle(Mn, Mm))
                    elif d == 1.73205:
                        bin2.append(angle(Mn, Mm))
                    elif d == 2.0:
                        bin3.append(angle(Mn, Mm))
                    elif d == 2.64575:
                        bin4.append(angle(Mn, Mm))
                    elif d == 3.0:
                        bin5.append(angle(Mn, Mm))
                    elif d == 3.46410:
                        bin6.append(angle(Mn, Mm))
        return [np.asarray(bin1), np.asarray(bin2), np.asarray(bin3), np.asarray(bin4), np.asarray(bin5),
                np.asarray(bin6)]
...
def spinpov(sp, image_name):
    ...
    objects.append(
        LightSource([sp.size / 2.0, 0, 30], 0.7, 'area_light', (5, 0, 0), (0, 5, 0), 5, 5, 'adaptive', 1, 'jitter'))
...
def show_fft(sp, complex=False):
    ...
    def reciproce_lattice(sp):
        ...
        recvec = np.asarray([np.cross(latvec[1], latvec[2]), np.cross(latvec[2], latvec[0]),
                             np.cross(latvec[0], latvec[1])]) * 2.0 * np.pi / spat
        recX, recY, recZ = [], [], []
