...
class SeriesName:
    ...
    def gen_name(
        ...
        if not series_number:
            series_number = Counter(
                dcm.bulk_get("SeriesNumber")
            ).most_common()[0][0]
    ...
    def __repr__(self) -> str:
        ...
        descr = (
            f"_{self.series_description}" if self.series_description else ""
        )
...
def populate_tags(
    ...
    context.update_file_metadata(
        context.get_input_filename("dicom"), tags=input_tags
    )
