...
def run_job_mt(
    ...
    # if engine ="all", run against every engine in config_file
    if engine == "all":
        dx_logging.print_debug(f"Executing on all configured Delphix Engines.")
        try:
            for delphix_ddp in dx_obj.dlpx_ddps:
...
def track_running_jobs(
    ...
    if not engine_running_jobs:
        ...
        dx_logging.print_info(f'Tracking running jobs on {engine["hostname"]}')
    while engine_running_jobs:
        for j in engine_running_jobs:
            ...
            if job_obj.job_state in ["COMPLETED"]:
                ...
                engine_running_jobs.remove(j)
                dx_logging.print_exception(f"{engine['hostname']}:{job_obj.reference} FAILED or was CANCELLED "
                      f"due to an error.", stdout= False)
                dx_logging.print_exception(f"{err_msg}")
