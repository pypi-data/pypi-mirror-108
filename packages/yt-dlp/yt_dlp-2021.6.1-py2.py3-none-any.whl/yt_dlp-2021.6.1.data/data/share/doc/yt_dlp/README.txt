...
-   MERGED WITH YOUTUBE-DL COMMIT/D495292: (v2021.05.16) You get all the
...
Python versions 3.6+ (CPython and PyPy) are officially supported. Other
versions and implementations may or maynot work correctly.

On windows, Microsoft Visual C++ 2010 Redistributable Package (x86) is
also necessary to run yt-dlp. You probably already have this, but if the
executable throws an error due to missing MSVCR100.dll you need to
install it.
...
AtomicParsley, mutagen, pycryptodome, phantomjs and any of the supported
external downloaders. Note that the windows releases are already built
with the python interpreter, mutagen and pycryptodome included.
...
-   proto: Protocol used for download (https/ftps > http/ftp >
    m3u8_native > m3u8 > http_dash_segments > other > mms/rtsp >
    unknown > f4f/f4m)
