...
class SQLAlchemyPostgresql(SQLAlchemy):
    ...
    def _create_engine(self, options: Dict = {}) -> Tuple[str, object]:
        ...
        connection_uri = f"{scheme_driver}://{connection_uri_no_scheme_no_db_name}/{options.get('db_name', None)}"
        ...
        if not database_exists(connection_uri):
            self._create_database(
                connection_uri=f"{scheme_driver}://{connection_uri_no_scheme_no_db_name}",
