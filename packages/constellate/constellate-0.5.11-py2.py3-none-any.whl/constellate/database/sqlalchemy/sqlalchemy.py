...
class SQLAlchemy:
    ...
    @contextmanager
    def session_scope(self, auto_flush=True, expire_on_commit=True) -> Generator:
        """Provide a transactional scope around a series of operations."""
        session = self.session_scope2(auto_flush=auto_flush, expire_on_commit=expire_on_commit)
        yield session

    def session_scope2(self, auto_flush=True, expire_on_commit=True):
    ...
    async def async_session_scope(self, auto_flush=True, expire_on_commit=True):
        """Provide a transactional scope around a series of operations."""
        session = await self.async_session_scope2(
            auto_flush=auto_flush, expire_on_commit=expire_on_commit
        )
        yield session

    async def async_session_scope2(self, auto_flush=True, expire_on_commit=True):
        """Provide a transactional scope around a series of operations."""
        session_maker = self._setup_multi_engine_session_maker(
            autoflush=auto_flush, expire_on_commit=expire_on_commit
        )
