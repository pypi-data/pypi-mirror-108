...
class Evaluate():
    ...
    def feature_importance(self, df: pd.DataFrame, 
        ...
        with tqdm(desc='Calculating Feature Importance', total=8, unit='step', file=sys.stdout, leave=False) as progress:
            ...
            ### For Later Use
            df_perm = pd.DataFrame({'feature_importance': model.get_feature_importance(train_dataset), 
            ...
            for column in df_shap:
                try:
                    b = min(10, fi_df[column].nunique())
                    H, xedges, yedges = np.histogram2d(x=fi_df[column], y=df_shap[column], bins=b)
