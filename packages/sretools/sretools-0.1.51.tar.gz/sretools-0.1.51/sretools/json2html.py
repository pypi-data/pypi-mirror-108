...
class JsonConverter:
    ...
    def json2html_helper(self, obj, lvl=0):
        ...
        inlist_border = 1
        ctoggle = '+/-'
        jqyhdr = """<header><script src="https://code.jquery.com/jquery-3.6.0.min.js"></script> <script type="text/javascript"> $(document).ready(function () { $('tr.parent') .css("cursor", "pointer") .attr("title", "Click to expand/collapse") .click(function () { $(this).siblings('.child-' + this.id).toggle(); }); $('tr[@class^=child-]').hide().children('td'); }); </script></header>"""
        if self.__collapseandexpand and lvl == 0 :
            out += jqyhdr
        ...
        if type(obj) is list:
            ...
            if self.__is_final_list(obj):
                ...
                if self.__collapseandexpand and lvl != 0 :
                    out += """ <tr class="parent" id="{}" title="Click to expand/collapse" style="cursor: pointer;"> <td>{}</td> </tr> """.format(tid,ctoggle)
                ...
                for r in obj:
                    ...
                    for k, v in r.items():
                        if not v:
                            ...
                            v = str(v).replace("\n", "<br>")
                        if need_switch:
                            out += '<tr class="child-{}" style="background-color:{}">\n'.format(tid,c)
                        else:
                            out += "<tr>\n"
                        out += "<td><b>" + str(k) + "</b></td>\n"
                        out += "<td>" + str(v) + "</td>\n"
        ...
        if type(obj) is dict:
            ...
            for k, v in sorted(obj.items(), key=fs):
                ...
                out += "<td><b>" + str(k) + "</b></td>\n"
                out += "<td>\n"
                out += self.json2html_helper(v,lvl+1)
                out += "</td>\n"
                out += "</tr>\n"
            out += "</table>\n"
            return out
