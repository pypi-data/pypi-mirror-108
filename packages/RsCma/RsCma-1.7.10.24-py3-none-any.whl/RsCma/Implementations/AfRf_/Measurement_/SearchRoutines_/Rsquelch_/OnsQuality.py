...
class OnsQuality:
	"""OnsQuality commands group definition. 2 total commands, 0 Sub-groups, 2 group commands"""
	...
	def read(self) -> float:
		...
		Snippet: value: float = driver.afRf.measurement.searchRoutines.rsquelch.onsQuality.read() \n
		No command help available \n
		Use RsCma.reliability.last_value to read the updated reliability indicator. \n
			:return: on_sig_quality: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:AFRF:MEASurement<Instance>:SROutines:RSQuelch:ONSQuality?', suppressed)
		return Conversions.str_to_float(response)
