from ....Internal.Core import Core
from ....Internal.CommandsGroup import CommandsGroup


# noinspection PyPep8Naming,PyAttributeOutsideInit,SpellCheckingInspection
class Measurement:
	"""Measurement commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("measurement", core, parent)

	def set(self) -> None:
		"""SCPI: INIT:VSE:MEASurement<Instance> \n
		Snippet: driver.init.vse.measurement.set() \n
		Starts or continues the measurement. \n
		"""
		self._core.io.write(f'INIT:VSE:MEASurement<Instance>')

	def set_with_opc(self) -> None:
		"""SCPI: INIT:VSE:MEASurement<Instance> \n
		Snippet: driver.init.vse.measurement.set_with_opc() \n
		Starts or continues the measurement. \n
		Same as set, but waits for the operation to complete before continuing further. Use the RsCma.utilities.opc_timeout_set() to set the timeout value. \n
		"""
		self._core.io.write_with_opc(f'INIT:VSE:MEASurement<Instance>')
