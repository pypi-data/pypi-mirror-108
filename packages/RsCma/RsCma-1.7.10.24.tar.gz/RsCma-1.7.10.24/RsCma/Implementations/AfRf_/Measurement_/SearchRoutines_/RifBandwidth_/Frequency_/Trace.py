...
class Trace:
	...
	def read(self) -> List[float]:
		...
		Snippet: value: List[float] = driver.afRf.measurement.searchRoutines.rifBandwidth.frequency.trace.read() \n
		No command help available \n
		Use RsCma.reliability.last_value to read the updated reliability indicator. \n
			:return: rf_level: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'READ:AFRF:MEASurement<Instance>:SROutines:RIFBandwidth:FREQuency:TRACe?', suppressed)
		return response
