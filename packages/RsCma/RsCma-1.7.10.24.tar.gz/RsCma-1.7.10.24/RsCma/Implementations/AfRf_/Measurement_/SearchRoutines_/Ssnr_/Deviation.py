...
class Deviation:
	...
	def read(self) -> float:
		...
		Snippet: value: float = driver.afRf.measurement.searchRoutines.ssnr.deviation.read() \n
		No command help available \n
		Use RsCma.reliability.last_value to read the updated reliability indicator. \n
			:return: ssnr: No help available"""
		suppressed = ArgSingleSuppressed(0, DataType.Integer, False, 1, 'Reliability')
		response = self._core.io.query_str_suppressed(f'READ:AFRF:MEASurement<Instance>:SROutines:SSNR:DEViation?', suppressed)
		return Conversions.str_to_float(response)
