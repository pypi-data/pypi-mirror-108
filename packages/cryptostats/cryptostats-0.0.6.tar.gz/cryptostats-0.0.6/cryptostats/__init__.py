...
class CryptoStats:
    ...
    def get_price(api_key, currency, id):
        r = requests.get(
            f"https://api.nomics.com/v1/currencies/ticker?key={api_key}&ids={id}&interval=1d,30d&convert={currency}&per-page=100&page=1")
        crypto = r.json()
    ...
    def get_name(api_key, id):
        r = requests.get(
            f"https://api.nomics.com/v1/currencies/ticker?key={api_key}&ids={id}&interval=1d,30d&convert=USD&per-page=100&page=1")
        crypto = r.json()
        name = crypto[0]['name']
        return name
