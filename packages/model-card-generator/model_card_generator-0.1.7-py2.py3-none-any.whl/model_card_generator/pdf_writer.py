...
cne_logo_path='./cne_logo.png'
request.urlretrieve(cne_logo_url, cne_logo_path)
...
first_page_elements = [
    ...
    { 
        ...
        'align': 'R', 'text': cne_logo_path, 'priority': 2, 
    },
    { 
...
def write_first_page(model_data: ModelData, path: str):
    ...
    t.render(path)
    remove(cne_logo_path)

def merge_pdfs(path: str, *filenames: List[str]): 
...
def write_pdf(model_data: ModelData): 
    ...
    write_first_page(model_data, first_page_path)
    merge_pdfs(model_card_path, first_page_path)