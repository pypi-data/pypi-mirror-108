    ...
    try:
        if "." in field_name:  # 这里只支持两层嵌套  不断循环
            ...
            if "." in field_name1:
                ...
                zh_name = "%s-%s" % (ser1.Meta.model._meta.verbose_name, zh_name)
    ...
    try:
        ...
        if change:
            for i in change:
                ...
                if "last_modified" in i["field"] or "最近更新时间" in i["field"] or isinstance(i["field"], list) or   "创建时间" in i["field"] :
                    continue
                changestr = "由%s更新为%s" % (i["old"], i["new"])
