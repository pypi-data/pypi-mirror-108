...
class ParserGenerator(object):
    ...
    def compute_grammar_hash(self, g):
        ...
        hasher.update(json.dumps(sorted(g.各词)).encode())
    ...
    def serialize_table(self, table):
        return {
            ...
            "terminals": sorted(table.grammar.各词),
            "precedence": table.grammar.优先级,
            "productions": [
                (p.name, p.prod, p.prec) for p in table.grammar.规则
            ],
        }
    ...
    def data_is_valid(self, g, data):
        ...
        for key, (assoc, level) in iteritems(g.优先级):
            if data["precedence"][key] != [assoc, level]:
                return False
        if len(g.规则) != len(data["productions"]):
            return False
        for p, (name, prod, (assoc, level)) in zip(g.规则, data["productions"]):
            if p.name != name:
                return False
            if p.prod != prod:
                return False
...
class LRTable(object):
                    ...
                    if p.name == "S'":
                        ...
                        for a in laheads:
                            if a in st_action:
                                r = st_action[a]
                                if r > 0:
                                    sprec, slevel = grammar.规则[st_actionp[a].number].prec
                        ...
                        if j >= 0:
                            if a in st_action:
                                ...
                                if r > 0:
                                    ...
                                    rprec, rlevel = grammar.规则[st_actionp[a].number].prec
                                    sprec, slevel = grammar.优先级.get(a, ("right", 0))
                                    if (slevel > rlevel) or (slevel == rlevel and rprec == "right"):
                                        grammar.规则[st_actionp[a].number].reduced -= 1
                                        st_action[a] = j
                                        st_actionp[a] = p
                                        if not rlevel:
    ...
    def lr0_items(cls, grammar, add_count, cidhash, goto_cache):
        C = [cls.lr0_closure([grammar.规则[0].lr_next], add_count)]
    ...
    def dr_relation(cls, grammar, C, trans, nullable, add_count, goto_cache):
        ...
        for p in g:
            if p.lr_index < p.getlength() - 1:
                ...
                if a in grammar.各词 and a not in terms:
                    terms.append(a)
        if state == 0 and N == grammar.规则[0].prod[0]:
            terms.append("$end")
        return terms
