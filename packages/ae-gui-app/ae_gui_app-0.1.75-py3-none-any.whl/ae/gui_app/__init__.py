...
from ae.base import (                                                                                   # type: ignore
    ...
    duplicates, instantiate_config_parser, norm_name, now_str, os_platform, snake_to_camel)
...
def font_min_max_size_from_win(win_width: float, win_height: float) -> Tuple[float, float]:
    """ calculate optimal range of font size depending on the window/screen resolution.

    :param win_width:       window/screen width in pixels.
    :param win_height:      window/screen height in pixels.
    :return:                minimum and maximum font size in pixels.
    """
    min_len = min(win_width, win_height)
    return (min(max(MIN_FONT_SIZE, round(min_len / 48.9)), MAX_FONT_SIZE),
            min(max(MIN_FONT_SIZE, round(min_len / 24.6)), MAX_FONT_SIZE))
...
class MainAppBase(ConsoleApp, ABC):
    ...
    def change_flow(self, new_flow_id: str, **event_kwargs) -> bool:
        ...
        :param event_kwargs:    optional args to pass additional data or info onto and from the flow change confirmation
                                ...
                                * `count`: optional number used to render a pluralized help text for this flow change
                                  (for this gets also passed to the help text renderer by/in
    ...
    def global_variables(self, **patches) -> Dict[str, Any]:
        ...
        :return:                dict of global variables patched with :paramref:`.patches`.
        """
    ...
    def setup_app_states(self, app_states: AppStateType):
        ...
        win_width, win_height = app_states.get('win_rectangle', (0, 0, 1920, 1080))[2:]
        if not MIN_FONT_SIZE <= font_size <= MAX_FONT_SIZE:   # adopt device scaling on very first app start
            if font_size == -1.0:
                min_f, max_f = font_min_max_size_from_win(win_width, win_height)
                font_size = min_f + (max_f - min_f) / 3.0
            elif font_size < 0.0:
    ...
    def win_pos_size_change(self, *win_pos_size):
        ...
        font_size = self.font_size
        min_f, max_f = font_min_max_size_from_win(win_width, win_height)
        if not min_f <= font_size <= max_f:
            self.change_app_state('font_size', min(max(min_f, font_size), max_f))
        app.max_font_size = max_f                               # update dynamic min/max font size
        app.min_font_size = min_f

        self.vpo(f"MainAppBase.win_pos_size_change {win_pos_size}: landscape={app.landscape} font size={min_f}-{max_f}")
