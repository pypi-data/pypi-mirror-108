...
def generate_payload_table_ddl(
    ...
    if create_index and (record_timestamp is not None):
        result += "\n"
        result += "CREATE INDEX {1}_index ON TABLE {0}.{1} ({2}) AS 'COMPACT' WITH DEFERRED REBUILD;\n".format(
...
def generate_explanations_table_ddl(
    ...
    return __generate_table_ddl(ddl_fields, table_name=table_name, database_name=database_name, path_to_hdfs_directory=path_to_hdfs_directory, stored_as="orc")
