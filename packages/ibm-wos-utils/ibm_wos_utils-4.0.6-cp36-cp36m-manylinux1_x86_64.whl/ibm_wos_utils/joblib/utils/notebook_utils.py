...
def get_max_length_categories(spark_df):
    ...
    for columns in ichunked(string_columns, 100):
        ...
        max_df = spark_df.select([F.max(F.length(F.col(column))).alias(column) for column in columns]).toPandas()
        # Converting to pandas series
        max_df = max_df.transpose().squeeze()
        result.update(max_df.to_dict())
