...
class NotificationManager:
    ...
    def stage(self, event_type: str, message, group_id: str, event_cause: str = None):
        self._to_publish.append((event_type, message, group_id, event_cause))

    def flush(self):
        with concurrent.futures.ThreadPoolExecutor(max_workers=16) as executor:
            for to_publish in self._to_publish:
                executor.submit(self.publish, *to_publish)

    def publish(self, event_type: str, message, group_id: str, event_cause: str = None):
        if type(message) is not str:
            message = message.json()
        try:
            message_attributes = {"eventType": event_type}
            if event_cause:
                message_attributes["eventCause"] = event_cause
            self._publish(
                message=message,
...
class NotificationManagerMock:
    ...
    def stage(self, event_type: str, message, group_id: str):
        pass
