...
# sys.path.insert(0, ".")
import xt, excel
# from xt import engfmt, openplot
...
def opendatafiles(tags=["bearing", "mats", "sgear", "shaft", "vbelt"], makezip=False):
    ...
    for s in tags:
        ...
        x.append(excel.Xcel(datafilename))
        if makezip:
            xt.mdxziplist([datafilename])
    return x
...
def steelprop(x, alloyname, props):
    ...
    """
    return alloyprop(x, "STEELS", alloyname, props)
...
# def plotIJ():
#     N1 = np.arange(17, 30)
#     N2 = np.array([17, 18, 25, 35, 50, 85, 170])
#     N2 = np.array([17, 18])
#     ax = plt.gca()
...
#             x[k] = N1[k]
#             y[k] = J2(x[k], n2)[0]
#             print("%d: J(%.0f, %.0f)=%.3f" % (k, x[k], n2, y[k]))
...
\n" % (xt.engfmt(p[0], p=9, d=4, latex=True), uts, xt.engfmt(-p[1], p=6, d=4, latex=True), uts,
            xt.engfmt(p[2], p=3, d=4, latex=True), uts, p[3], kt)
    return kt
...
def shrinkscf(uts, record=False):
    ...
    if record:
        ...
        CALCRECORD += "$-%s\\times%.0f^3+%s\\times%.0f^2-%s\\times%.0f+%.4f=%.3f$\
\n" % (xt.engfmt(-p[0], p=9, d=4, latex=True), uts, xt.engfmt(p[1], p=6, d=4, latex=True), uts,
            xt.engfmt(-p[2], p=3, d=4, latex=True), uts, p[3], scf)
    return scf
    ...
    """
        ...
        **Examples**
        ...
        The above will create the following two image files:
            ...
            .. image:: ../../../pics/examplebmdiagram_xy.png

            .. image:: ../../../pics/examplebmdiagram_xz.png
...
def scf_roundbartension(D, d=10, r=1):  # used to be scf_shaft_with_shoulder
    """
        ...
        .. image:: ../../../pics/scf_roundbartension.png
...
filename='yourchoice.png', title='your choice')"
    ...
    if draw_circle:
        (fig, ax) = xt.openplot(figno, aspect=1.0)
