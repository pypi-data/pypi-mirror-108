...
class ListRouter(Lister):
    """List Routers."""
    schema = schemas.Router(many=True)
...
class ShowRouter(ShowOne):
    """Show Router details."""
    schema = schemas.Router(many=False)
...
class ListRouterInterface(Lister):
    ...
    schema = schemas.RouterInterface(many=True)
...
class ShowRouterInterface(ShowOne):
    ...
    schema = schemas.RouterInterface(many=False)
        ...
        if len(payload) == 0:
            raise CLIError([dict(
                msg="You need to at least provide one of "
                    "--name, --enable-gateway or --disable-gateway"
            )])
        uri = '/routers/%s' % parsed_args.router
        data = self.app.session.patch(uri, payload)
        return data
