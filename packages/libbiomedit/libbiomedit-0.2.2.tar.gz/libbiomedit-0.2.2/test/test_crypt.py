...
DCC_UID = gpg.Uid(
    full_name="DCC Test (Test key for transferprotocol)", email="dcc@sib.swiss"
)
...
test_key_data = dict(
    ...
    signatures=(
        gpg.Signature(
            issuer_uid=CN_UID,
            issuer_key_id="FDC66B7A5F6FFD4B",
            creation_date="1550241679",
            signature_class="13x",
        ),
        gpg.Signature(
            issuer_uid=DCC_UID,
            issuer_key_id="3A6500D5C1DE39AC",
...
class TestCrypt(unittest.TestCase):
        ...
        for key in keys:
            for keyserver in (
                ...
                with self.subTest(key=keys.index(key), keyserver=keyserver):
                    crypt.assert_keyserver_origin_valid(key, keyserver=keyserver)
        with self.assertRaises(RuntimeError):
            crypt.assert_keyserver_origin_valid(
                self.key1, keyserver="fakekeyserver.dcc.sib.swiss"
            )
        with self.assertRaises(RuntimeError):
            crypt.assert_keyserver_origin_valid(
                self.key1, keyserver="keyserver.dcc.sib.swiss:11111"
            )
    ...
    def test_assert_key_is_signed(self):
        ...
        with self.assertRaises(RuntimeError):
            crypt.assert_key_is_signed(
                fake_key(signatures=(rev_sig,)), sig_keyid="3A6500D5C1DE39AC"
            )
    ...
    def test_validate_pub_key(self):
        ...
        with self.assertRaises(RuntimeError):
            crypt.validate_pub_key(
                fake_key(validity=gpg.Validity.revoked),
                validation_keyid="3A6500D5C1DE39AC",
                keyserver_url="http://keyserver.dcc.sib.swiss:11371",
            )
    ...
    def test_verify_metadata_signature(self):
        class MockArchiveReader:
            ...
            def __repr__(self):
                return f"MockArchiveReader({self.metadata}, {self.signature})"
        ...
        for mock_archive in (
            ...
            with self.subTest(tar=repr(mock_archive)):
                ...
                @contextmanager
                def mock_archive_reader_init(f):
                ...
                with mock.patch(
                    ...
                    "libbiomedit.crypt.validated_keys_by_ids", mock_validate_keys
                ), mock.patch(
                    "libbiomedit.crypt.extract_key_id_from_sig", mock_gpg_key_id
                ):
    ...
    def test_validated_keys_by_ids(self):
        ...
        class MockGPGStore:
            def __init__(self, keys, keyserver_keys):
                ...
                self.keyserver_keys = [(key.key_id, key) for key in keyserver_keys]
            ...
            def recv_keys(self, *key_ids, keyserver=None, url_opener=None):
                ...
                self.keys += [
                    (key_id, key)
                    for key_id, key in self.keyserver_keys
        ...
        for gpg_store in (
            ...
            with self.subTest(store=[key_id for key_id, _key in gpg_store.keys]):
                available_keys = frozenset(key_id for key_id, key in gpg_store.keys)
                should_refresh = [
                    key for key in [key_A, key_B] if key.key_id in available_keys
                ]
                refresh_keys = mock.Mock(side_effect=lambda keys, **_: keys)
                validate_pub_key = mock.Mock()
                with mock.patch(
                    "libbiomedit.crypt.refresh_keys", refresh_keys
    ...
    def test_refresh_keys(self):
        ...
        with self.assertWarns(UserWarning):
            new_keys = crypt.refresh_keys(
                keys, gpg_store, sigs=..., keyserver_url=None, url_opener=urlopen
            )
        ...
        gpg_store.recv_keys.assert_has_calls(
            (
                mock.call(
                    40 * "A",
                    keyserver="http://keyserver.dcc.sib.swiss:11371",
                    url_opener=urlopen,
                ),
        ...
        # to be refreshed.
        new_keys = crypt.refresh_keys(
            keys, gpg_store, sigs=..., keyserver_url=self.keyserver, url_opener=urlopen
        )
        ...
        gpg_store = new_gpg_store((fake_key(fingerprint="0" + 39 * "A"),) + keys[1:])
        with self.assertRaises(RuntimeError), self.assertWarns(UserWarning):
            crypt.refresh_keys(keys, gpg_store, sigs=...)
...
def assert_conditional_raise(test: unittest.TestCase, error_type: Optional[Exception]):
    if error_type:
        with test.assertRaises(error_type):  # type: ignore
            yield None
    else:
        yield None
