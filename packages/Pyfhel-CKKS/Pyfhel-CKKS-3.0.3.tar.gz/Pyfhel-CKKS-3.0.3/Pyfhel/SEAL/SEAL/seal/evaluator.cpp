...
namespace seal
{
    ...
    void Evaluator::multiply_plain_inplace(Ciphertext &encrypted, const Plaintext &plain, MemoryPoolHandle pool) const
    {
        ...
        {
            throw invalid_argument("encrypted metadata is not valid for encryption parameters");
        }
        if (!is_buffer_valid(encrypted))
        {
          throw invalid_argument("encrypted buffer is not valid for encryption parameters");
        }
        if (!is_metadata_valid_for(plain, context_) || !is_buffer_valid(plain))
        {
            throw invalid_argument("plain is not valid for encryption parameters");
