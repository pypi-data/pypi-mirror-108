...
typelib = {
    ...
    ContextType.EXECUTE_ADDING_FILES: store.get_context_type(ContextType.EXECUTE_ADDING_FILES),
    ContextType.EXECUTE_REMOVING_FILES: store.get_context_type(ContextType.EXECUTE_REMOVING_FILES),
...
def create_artifacts(type, obj_array: Array):
    artifacts = []
...
def create_association_attribution_multi_artifacts(context_id, execution_id, artifact_ids:Array):
    association_arr = []
    attribution_arr = []
    if execution_id is not None:
        association = metadata_store_pb2.Association()
