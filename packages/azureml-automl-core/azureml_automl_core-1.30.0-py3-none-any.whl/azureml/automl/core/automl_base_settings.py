...
class AutoMLBaseSettings:
    ...
    def _validate_model_filter_lists(self):
        ...
        forecasting_algos = set()

        # If lag or rolling_window is enabled, we need to throw warnings for some forecasting models:
        if self.is_timeseries and (self.lags is not None or self.window_size is not None):
            forecasting_algos = {
                constants.SupportedModels.Forecasting.Average,
                constants.SupportedModels.Forecasting.AutoArima,
                constants.SupportedModels.Forecasting.Arimax,
        ...
        if self.blacklist_algos is None and self.whitelist_models is None:
            ...
            # the JOS create parent run endpoint. Since we removed the blocking logic for step
            # runs in here, a warning needs to be thrown for step run users.
            if forecasting_algos:
                logging.warning(
                    "The following algorithms are not compatibile with lags and rolling windows"
                    ": {} and have been added to the blocked model list.".format(", ".join(forecasting_algos)))
        ...
        if self.blacklist_algos is not None:
            ...
            # If everything but classical forecasting learners are blocked (equivalent to allowing only
            # classical forecasting learners) with lags/rolling windows enabled, we should throw a warning:
            enabled_algos -= forecasting_algos
            if len(enabled_algos) == 0:
                logging.warning(
                    "The following algorithms are not compatibile with lags and rolling windows"
                    ": {} and have been added to the blocked model list. Please block fewer algorithms "
                    "to enable training.".format(", ".join(forecasting_algos)))
        ...
        if self.whitelist_models is not None:
            ...
            # If only forecasting algorithms are excluded (equivalent to exclude all but forecasting algos) but
            # lags/rolling windows enabled, we should throw a warning.
            actual_wlm -= forecasting_algos
            if len(actual_wlm) == 0:
                logging.warning(
                    "The following algorithms are not compatibile with lags and rolling windows"
                    ": {} and have been added to the blocked model list. Please allow more algorithms "
                    "to enable training.".format(", ".join(forecasting_algos)))
