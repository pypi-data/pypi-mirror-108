...
class Reservation:
    ...
    def __init__(self, node: Node, provision: str = None, loop=None, monitor: Monitor = None, ignore_node_exceptions=False, enter_on=[ReserveState.ACTIVE], exit_on=[ReserveState.ERROR, ReserveState.CANCELLED], context=None, **params) -> None:
    ...
    async def assign_async(self, *args, bypass_shrink=False, bypass_expand=False, persist=True, **kwargs):
        ...
        try:
            ...
            return_message = await self._postman.assign(self.reservation, shrinked_args, shrinked_kwargs=shrinked_kwargs, on_progress=self.on_progress, persist=persist, context=self.context)
    ...
    async def stream(self, *args, bypass_shrink=False, bypass_expand=False, persist=True, **kwargs):
        ...
        try:
            shrinked_args, shrinked_kwargs = await shrinkInputs(self.node, args, kwargs) if not bypass_shrink else (args, kwargs) 
            async for message in self._postman.assign_stream(self.reservation, shrinked_args, serialized_kwargs=shrinked_kwargs, with_progress=True, persist=persist, context=self.context):
    ...
    async def contract_worker(self):
        self.running = True
        try:
            async for message in self._postman.reserve_stream(node_id=self.node.id, provision=self.provision, params_dict=self.params.dict(), with_progress=True, context=self.context):
