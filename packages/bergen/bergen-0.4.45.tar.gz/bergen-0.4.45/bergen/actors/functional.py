...
class FunctionalThreadedGenActor(FunctionalActor):
    ...
    def _assign_threaded(self, args, kwargs, queue):
    ...
    async def iterate_queue(self, async_q, assign_handler: AssignHandler, provide_handler):
        while True:
            ...
            action = val[0]
            if action == "log":
                await assign_handler.pass_log(*val[1])
                async_q.task_done()
            if action == "yield":
    ...
    async def _assign(self, assign_handler: AssignHandler, args, kwargs):
        ...
        print(queue)
        try:
            fut = self.loop.run_in_executor(self.threadpool, self._assign_threaded, args, kwargs, queue.sync_q)
