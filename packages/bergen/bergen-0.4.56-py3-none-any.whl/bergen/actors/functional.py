...
class FunctionalThreadedFuncActor(FunctionalActor):
    ...
    async def _assign(self, assign_handler: AssignHandler, args, kwargs):
        queue = janus.Queue()
        try:
            fut = self.loop.run_in_executor(self.threadpool, self._assign_threaded, args, kwargs, queue.sync_q, assign_handler.message.meta.context)
            await self.iterate_queue(queue.async_q, assign_handler, self.provide_handler)
            await fut
            queue.close()
...
class FunctionalThreadedGenActor(FunctionalActor):
    ...
    async def _assign(self, assign_handler: AssignHandler, args, kwargs):
        ...
        try:
            ...
            fut = self.loop.run_in_executor(self.threadpool, self._assign_threaded, args, kwargs, queue.sync_q, event, assign_handler.message.meta.context)
            await self.iterate_queue(queue.async_q, assign_handler, self.provide_handler)
            await fut
            queue.close()
            await queue.wait_closed()
