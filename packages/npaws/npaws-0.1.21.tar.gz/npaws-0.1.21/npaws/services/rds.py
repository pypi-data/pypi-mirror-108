...
class RDSClient():
    ...
    def get_project_version_documents(self,
        ...
        """
        """
        query = f"SELECT d.id, d.uri FROM neuralplatform_document d, neuralplatform_version v WHERE (d.tagged = 1 AND d.project_id = {project_id} AND v.id = {version_id} AND (d.uploadDate BETWEEN v.startDate AND v.endDate));"
        return self.select_query(query, fetchone=False)
    ...
    def update_page_img_status(self,
        ...
        """
        """
        query = f'UPDATEneuralplatform_page SET imgStatus = "{status}" WHERE id = {page_id};'
        return self.insert_query(query)
    ...
    def update_page_ocr_status(self,
        ...
        """
        """
        query = f'UPDATEneuralplatform_page SET ocrStatus = "{status}" WHERE id = {page_id};'
        return self.insert_query(query)
