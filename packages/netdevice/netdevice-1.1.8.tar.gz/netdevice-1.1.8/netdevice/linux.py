...
class LinuxDevice(object):
    ...
    def __init__(self, server = None, **kwargs):
        ...
        if self.attributes.get("autologin", True):
            ...
            LinuxDevice.cmd(self,
                'echo "set enable-bracketed-paste off" > .netdevice_workaround_for_pexpect_issue669',
                log_level = self.LOG_WARNING)
            LinuxDevice.cmd(self,
                    'export INPUTRC=$PWD/.netdevice_workaround_for_pexpect_issue669',
                    log_level = self.LOG_WARNING)
