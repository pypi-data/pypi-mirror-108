...
def preprocess(d, gl, cl):
	# extract common genes
	com_genes=set(d.data['gene'].index.tolist()).intersection(set(gl.index.tolist()))
	com_genes=sorted(list(com_genes))
	d.genelist=gl.loc[com_genes]

	# get common samples
	com_samps=set()
	for oidx, o in d.data.items():
		if not len(com_samps):
			com_samps=set(o.columns)
		else:
			com_samps=com_samps.intersection(o.columns)

	com_samps=list(com_samps)

	cl=cl[cl['Sampid'].isin(com_samps)].sort_values('Class')
	d.samples=cl.reset_index(drop=True)
	d.groups=cl['Class'].value_counts().sort_values()

	# extract common samples
	for omics in d.data.keys():
		d.data[omics]=d.data[omics].loc[:,cl.Sampid]
...
def split_data(mob, odir, cv_n, tratio):
	modata=mob.gldata
	genelist=mob.genelist
	samp_info=mob.samples
	...
	for i in range(cv_n):
		...
		test_idx=sorted(np.random.randint(mob.samples.shape[0], size=int(mob.samples.shape[0]*0.1)))
		test=mob.samples.iloc[test_idx,:].sort_values('Class')
		train=mob.samples.iloc[~mob.samples.index.isin(test.index),:].sort_values('Class')
