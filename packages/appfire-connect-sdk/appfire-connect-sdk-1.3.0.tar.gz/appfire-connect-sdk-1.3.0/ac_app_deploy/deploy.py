...
def process(verbose, command, profile, env, stage, stack, app_suffix, build, api_domain, api_stage, app_api_domain, require_approval, tag, dts_ecr_profile):
    ...
    if command.lower() == 'bootstrap':
        ...
        if stack == 'core':
            ...
            deploy_automation(logger,app_name,stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag)
        else:
...
def deploy_automation(logger,app_name,stage,app_domain,brand,env,app_suffix,aws_profile,registry_url,tag):
    logger.info('Deploying ac-automation')

    # Pull the ac-automation:tag, default is latest
    service_module_image = generate_image_url("ac-automation",registry_url,tag)
    returnCode = shell(logger,"docker pull " + service_module_image,raise_error=False)

    if returnCode == 1:
        return False

    returnCode = shell(logger,"docker run -v ~/.aws:/appfire/.aws -e STAGE=" + stage + " -e CDK_DEPLOY_ACCOUNT=" + os.environ.get("CDK_DEPLOY_ACCOUNT") +
                 " -e CDK_DEPLOY_REGION=" + os.environ.get("CDK_DEPLOY_REGION") + " -e AWS_PROFILE=" + aws_profile +
                 " " + service_module_image,raise_error=False)
