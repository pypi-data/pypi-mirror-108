...
class Exceptions:
    ...
    class FetchError(BaseException):
        """Raised when a web fetch request fails (qualified when status code \
            is not in 200 range and is not 301 or 302)."""

    class PatchError(BaseException):
        """Raised for errors involving patch files, caused by them being \
            missing, invalid..."""

    class ReleaseError(BaseException):
        """Raised for errors involving release archive files, caused by them \
            being missing, invalid..."""

    class TargetError(BaseException):
        """Raised for errors involving specified bandage.Patcher target for \
            upgrade."""

    class RemoteError(BaseException):
        """Raised for errors involving specified bandage.Patcher remotes for \
            fetching."""

    class VersionError(BaseException):
        """Raised for errors involving VERSION files, caused by them being \
            missing, invalid, mismatching, being the same..."""

    class UnableToParseError(BaseException):
        """Raised when Bandage is unable to interpret a string, usually \
...
class Patcher:
    ...
    def __init__(self, patch: str, target: str,
...
class Weave:
    ...
    def __init__(self, release_old: str, release_new: str, output_path: str,
...
class Supply:
    """Main class for bandage.Supply instances, which checks for new patches \
        on remotes."""
