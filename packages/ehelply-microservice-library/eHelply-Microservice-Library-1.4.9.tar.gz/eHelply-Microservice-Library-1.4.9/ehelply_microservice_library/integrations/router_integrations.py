...
async def get_auth(
    ...
    :return:
    """
    with sentry_sdk.start_span(op="header_authentication", description="Header Authentication") as span:
        import time
        ...
        # print("AUTH_STAGE1 --- %s seconds ---" % (time.time() - start_time))

        if x_secret_token and x_access_token:
            my_auth.access_token = x_access_token
            my_auth.secret_token = x_secret_token

            is_m2m_speedy: bool = False
            if m2m.speedy_m2m_auth(access_token=my_auth.access_token, secret_token=my_auth.secret_token):
                is_m2m_speedy = True
                my_auth.active_participant_uuid = m2m.get_speed_m2m_auth_participant(access_token=my_auth.access_token)

            if not is_m2m_speedy or not my_auth.active_participant_uuid:
                security = get_security(request=request)
                access = get_access(request=request)

                key = await security.sdk.verify_key(access=x_access_token, secret=x_secret_token)
                if is_response_error(response=key):
                    raise HTTPException(status_code=401, detail="Invalid key - Denied by eHelply")

                # print("AUTH_STAGE2 --- %s seconds ---" % (time.time() - start_time))

                ehelply_cloud_entity = asyncio.create_task(access.sdk.get_entity_for_key(key_uuid=key.uuid, partition="ehelply-cloud"))
                ehelply_resources_entity = asyncio.create_task(access.sdk.get_entity_for_key(key_uuid=key.uuid, partition="ehelply-resources"))

                # print("AUTH_STAGE3 --- %s seconds ---" % (time.time() - start_time))

                result = await asyncio.gather(ehelply_cloud_entity, ehelply_resources_entity)
                ehelply_cloud_entity = result[0]
                ehelply_resources_entity = result[1]

                # print("AUTH_STAGE4 --- %s seconds ---" % (time.time() - start_time))

                # Checks for project in ehelply-cloud key
                # STEP 1
                if is_response_error(response=ehelply_cloud_entity):

                    # If that fails, check to see if it's an administrative or M2M key
                    # STEP 2
                    if is_response_error(response=ehelply_resources_entity):
                        raise HTTPException(status_code=401, detail="Invalid participant for key - Denied by eHelply")
                    else:
                        my_auth.active_participant_uuid = ehelply_resources_entity.entity_identifier
                        if is_m2m_speedy and my_auth.access_token in m2m.m2m_keys:
                            m2m.m2m_keys[my_auth.access_token].participant_uuid = my_auth.active_participant_uuid
                else:
                    my_auth.active_participant_uuid = ehelply_cloud_entity.entity_identifier
        ...
        # print("AUTH_TOTAL --- %s seconds ---" % (time.time() - start_time))
