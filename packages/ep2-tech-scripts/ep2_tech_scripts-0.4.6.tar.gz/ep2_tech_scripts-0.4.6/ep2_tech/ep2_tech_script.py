...
@cli.command()
@click.option("--date", help='date of the test', type=click.DateTime())
@click.option("--testdata", required=True, help='file with start and end times for the groups',
              type=click.Path(exists=True, file_okay=True, allow_dash=False))
@click.option("--tutordata", required=False, help='file with firstname, lastname and email of tutors',
              type=click.Path(exists=True, file_okay=True, allow_dash=False))
@click.option("--zip/--nozip", default=False, help='generate zip files')
def prepare_protocols(date: datetime, testdata, tutordata, zip: bool):
    ep2 = Ep2()

    base_path = "{}".format(date.date())
    if not os.path.exists(base_path):
        os.mkdir(base_path)

    id_dir = os.path.join(ep2.config.get('Local', 'GitHome'), 'org', 'leitung', 'stud-list', 'student-ids')

    tutor_index: {str: [Tutor]} = {}

    if tutordata is not None:
        with open(tutordata) as file:
            ...
            for row in reader:
                ...
                tutor = Tutor(row[1], row[2])

                if group in tutor_index:
                    tutor_index[group] += [tutor]
                else:
                    tutor_index[group] = [tutor]
    ...
    with open(testdata) as file:
        ...
        for row in reader:
            ...
            outfile = open(os.path.join(base_path, "{}.md".format(group)), 'w')

            grp_data = ep2.group(group)
            stud_data = grp_data.student_info()

            if zip:
                zipf = zipfile.ZipFile(os.path.join(base_path, '{}.zip'.format(group)), mode='w',
                                       compression=zipfile.ZIP_DEFLATED)
                for student in stud_data.values():
                    stud_id = student['id']
                    stud_first_name = student['first_name']
                    stud_last_name = student['last_name']

                    original_file = os.path.join(id_dir, '{}.pdf'.format(stud_id))
                    if not os.path.exists(original_file):
                        click.secho('WARN could not add student with student id {} no id file found!'.format(stud_id),
                                    fg='yellow', bold=True)
                        continue

                    zipf.write(original_file, arcname='{}_{}_{}.pdf'.format(stud_last_name, stud_first_name, stud_id))

            print("# Prüfungsprotokoll EP2 Test {}, Gruppe {}".format(date.date(), group), file=outfile)
            print(file=outfile)
            print("Chat der Aufsicht (RocketChat): [https://chat.tuwien.ac.at/group/ep2-exam-intern]"
                  "(https://chat.tuwien.ac.at/group/ep2-exam-intern)", file=outfile)
            print(file=outfile)
            print("Die Protokolle nach der Prüfung sobald wie möglich jedoch spätestens um 16:00 auf TUWEL im Ordner "
                  "Protokolle/{} hochladen. Wenn das Protokoll hochgeladen wurde bitte im Chat Bescheid "
                  "geben.".format(date.date()), file=outfile)
            print(file=outfile)
            print("## Allgemeine Informationen", file=outfile)
            print(file=outfile)
            print("Geplanter Beginn der Arbeitszeit: **{}**".format(start), file=outfile)
            print(file=outfile)
            print("Geplantes Ende der Arbeitszeit: **{}**".format(end), file=outfile)
            print(file=outfile)
            print("> Anm: Beginn- und Endzeiten sind für alle Gruppen verschieden", file=outfile)
            print(file=outfile)
            print("Tatsächlicher Beginn der Arbeitszeit: ****", file=outfile)
            print(file=outfile)
            print("Tatsächliches Ende der Arbeitszeit: ****", file=outfile)
            print(file=outfile)
            print("In den Zoom Chat kopieren:", file=outfile)
            print("```", file=outfile)
            print("Daten zum Clonen des Projekts:", file=outfile)
            print("Version Control: Git", file=outfile)
            print("URL: git@b3.complang.tuwien.ac.at:ep2/2021s/uebung/XXXXXXXXt1a.git (mit public-Key) oder "
                  "https://b3.complang.tuwien.ac.at/ep2/2021s/uebung/XXXXXXXXt1a.git (mit Passwort)", file=outfile)
            print("Directory: beliebig wählbar", file=outfile)
            print("Ersetzen Sie XXXXXXXX durch Ihre Matrikelnummer", file=outfile)
            print("```", file=outfile)
            print(file=outfile)
            print("## Teilnehmer_innen", file=outfile)
            print(file=outfile)
            data = [
                ["Nr.", "Matrikelnummer", "Vorname", "Nachname", "anwesend (J/N)", "auffällig (J/N)", "Extra Minuten"],
            ]
            data += [[str(i + 1), student['id'], student['first_name'], student['last_name'], "?", "N", "0"]
                     for (i, student) in enumerate(stud_data.values())]
            table = terminaltables.github_table.GithubFlavoredMarkdownTable(data)
            print(table.table, file=outfile)
            print(file=outfile)
            print("## Aufsicht", file=outfile)
            print(file=outfile)
            print("Aufsichtspersonen:", file=outfile)

            if group in tutor_index:
                tutors = tutor_index[group]
                for tutor in tutors:
                    print("* {} {}".format(tutor.firstname, tutor.lastname), file=outfile)
                if len(tutors) == 1:
                    print("* ...", file=outfile)
            else:
                print("* Vorname Nachname", file=outfile)
                print("* ...", file=outfile)

            print(file=outfile)
            print("### Vorkommnisse", file=outfile)
            print(file=outfile)


@cli.command()
@click.option("--tag", required=True, prompt=True, type=click.STRING)
@click.option("--group", required=True, prompt=True, type=click.STRING)
@click.pass_context
def delete_protected_tag(ctx, tag: str, group: str):
    ep2 = Ep2(verbose=ctx.obj["VERBOSE"])

    group = ep2.group(group)
    student_list = group.student_list()

    print("deleting branch {}".format(tag))

    for student in student_list:
        repo_path = ep2.ue_repo(student).replace("/", "%2F")
        repo = ep2.gitlab.projects.get(repo_path)

        try:
            repo.protectedtags.delete('*')
            print("lifted protection off {}".format(student))
        except Exception:
            pass
        ...
        for repo_tag in tags:
            if repo_tag.name == tag:
                print("deleting tag {} from {}".format(tag, student))
                repo_tag.delete()

        repo.protectedtags.create({
            'name': '*'
        })

        print("created wildcard protection for {}".format(student))
...
@click.option("--rewrite-yml", required=False, default=False, help='rewrite YAML files while parsing them',
              type=click.BOOL)
@click.pass_context
def collect(ctx, ue: int, rewrite_yml: bool):
    ...
    fail_regex = re.compile(
        "org.opentest4j.AssertionFailedError:\\s*(typesys\\s+([\\w(),.\\s]*)|"
        "assert\\s+(\\d+\\.\\d+\\.\\d+\\.\\d+(\\.\\d+)?)):([\\s\\w:()]*)\\s*(==>.*)?")
                ...
                with open(eval_file_tmp, 'w') as outfile:
                    ...
                    for row in reader:
                        ...
                        if not os.path.exists(data_file):
                            print("no data file found for {}/{}".format(group.name, student_id), file=sys.stderr)
                            continue
                        ...
                        with open(data_file) as data_file:
                            ...
                            if struct['build']:
                                ...
                                for test_result in struct['results']:
                                    ...
                                    for test in test_result['tests']:
                                        try:
                                            ...
                                            print(
                                                "failed to create test {}.{}.{} for {}/{} with error: {}"
                                                .format(ue, suite, test_cnt, group.name, student_id, e))
                                            continue
                                        ...
                                        if 'fail_msg' in test:  # failed test, analyze message
                                            ...
                                            if match is not None:
                                                if match.group(1).startswith("assert"):
                                                    fail_index.inc_assert(match.group(3), match.group(5).strip(),
                                                                          test['name'], group.name)
                                                elif match.group(1).startswith("typesys"):
                                                    fail_index.inc_typesys(match.group(2).strip(),
                                                                           match.group(5).strip(), student_id,
                                                                           group.name)
                                ...
                                if not struct['changes']:
                                    ...
                                    for match in re.findall('src/(test|main)/java/(.*)\\.java', build_log,
                                                            re.DOTALL):
                                        dir = match[0]
    ...
    stat_file = click.open_file(os.path.join(ep2.config['Local']['GitHome'], 'org', 'leitung', 'unittests',
                                             'statistics_{}.txt'.format(ue)), mode='w')
    print_results(stat_file, test_index, fail_index, groups_dirs, group_results)
...
def print_results(file: any, test_index: TestIndex, fail_index: FailIndex, groups: [str], group_results: {TestGroup}):
    ...
    for group in groups:
        ...
        table_data += [[group,
                        "%3d (%6.2f %%)" % (
                            group_result.build_failed, (group_result.build_failed / group_result.total * 100) if group_result.total != 0 else 0),
                        "%3d (%6.2f %%)" % (
                            group_result.no_changes, (group_result.no_changes / group_result.total * 100) if group_result.total != 0 else 0),
                        "%3d (%6.2f %%)" % (
                            group_result.build_success, (group_result.build_success / group_result.total * 100) if group_result.total != 0 else 0),
                        group_result.total] + test_results]

    if len(groups) > 1:
        global_test_results = ["%6.02f %%" % test.global_success(groups) for test in flat_tests]
        table_data += [["TOTAL",
                        "%3d (%6.2f %%)" % (global_build_failed, (global_build_failed / global_build_total * 100) if global_build_total != 0 else 0),
                        "%3d (%6.2f %%)" % (global_no_changes, (global_no_changes / global_build_total * 100) if global_build_total != 0 else 0),
                        "%3d (%6.2f %%)" % (global_build_success, (global_build_success / global_build_total * 100) if global_build_total != 0 else 0),
    ...
    for index in sorted(fail_index.asserts):
        ...
        if test.fail_count(groups) == 0:
            click.echo("\t{} ({}): {} ({:3.2f}% of test failures)".format(index, assert_obj.message, count, 100),
                       file=file)
        else:
            click.echo("\t{} ({}): {} ({:3.2f}% of test failures)".format(index, assert_obj.message, count,
                                                                          count / test.fail_count(groups) * 100),
                       file=file)
