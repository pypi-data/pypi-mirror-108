...
class DottedDict(UserDict):
    ...
    def from_dict(cls, dictionary: dict):
        d = DottedDict._dict_loader(dictionary)
        return cls(data=d)
    ...
    def _dict_loader(dictionary: dict):
        ...
        for key, value in dictionary.items():
            if isinstance(value, dict):
                nested = DottedDict._dict_loader(value)
                d = DottedDict._setter(d, key.split("."), nested)
            else:
                d = DottedDict._setter(d, key.split("."), value)
        return d
    ...
    def _setter(dictionary: dict, path: list, value) -> dict:
        ...
        if len(path) > 1:
            if key in copy and isinstance(copy[key], dict):
                copy[key] = DottedDict._setter(copy[key], path[1:], value)
            else:
                copy[key] = DottedDict._setter({}, path[1:], value)
        else:
            copy[key] = value
        return copy
        ...
        for index, key in enumerate(path):
            if key in dictionary:
                if index == len(path) - 1:
                    ...
                    return DottedDict._getter(dictionary[key], path[index + 1 :])
