...
class Fpm:
  ...
  def __init__(self):
    ...
    self.skipDeps = ['openmp', 'VecGeom', 'O2-customization', 're2']  # as sometimes there's mismatch between "requires" deps and modulefile
  ...
  def run(self, name, version, deps, devel_deps, extra_deps, RPMS_GENERATED = []):
    ...
    subdirs = ['bin', 'lib', 'lib64', 'etc', 'include', 'bin-safe', 'libexec', 'WebDID', 'share', 'plugins']
    paths = []
