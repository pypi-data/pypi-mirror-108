...
class kraken_client():
    ...
    def _error_in_response(self,response):
        if len(response['error']) > 0:
            print("Error present in request : ",response['error'])
            return True
    ...
    def requestTickerInfo(self,pair,proxies=False):
        tickers_request = requests.get('https://api.kraken.com/0/public/Ticker?pair='+ pair, proxies=proxies)
        if self._error_in_response(tickers_request.json()):return False
        return tickers_request.json()['result'] 

    def AddOrder(self,type,pair,volume):
        orderRequest = self.kraken_request('/0/private/AddOrder', {
            "nonce": str(int(1000*time.time())),
            "pair" : pair,
            "type" : type,
            "ordertype" : "market",
        ...
        if self._error_in_response(orderRequest.json()):return False
        return orderRequest.json()['result'] 
