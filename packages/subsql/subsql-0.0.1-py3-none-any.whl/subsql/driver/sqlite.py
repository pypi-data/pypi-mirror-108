...
from subsql.parser import parser, util;
from subsql.parser.types import ParserSnippet, ParserError
...
def execute(self, command, args, cursor=None, format=None, modifier=None):
    cursor = cursor if cursor else self.cursor
    query = format(query) if format else query
    modifier = modifier if modifier else command.modifier

    if command.execute == "many":
        cursor.executemany(query, args)
        psycopg2.extras.executemany(query, args)
    elif command.execute == "script":
        psycopg2.extras.executescript(query)
    else:
