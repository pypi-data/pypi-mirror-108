 ...
 */
static bool belongs_in_bie(ast_node *bie_node, ast_node *expr_node) {
    if (!bie_node || !expr_node) {
        return false;
    }

    switch(expr_node->type) {
    case type_unary_expr:
        return belongs_in_bie(bie_node, expr_node->unary_expr.operand);
        break;
    case type_binary_expr:
        ...
        if (expr_node->binary_expr.operator == AND || expr_node->binary_expr.operator == OR) {
            return belongs_in_bie(bie_node, expr_node->binary_expr.left) ||
                    belongs_in_bie(bie_node, expr_node->binary_expr.right);
        } else {
            /* otherwise, just check if the belongs_in interval expression matches */
            return expr_node->binary_expr.belongs_in == bie_node;
        }

        break;
    case type_map_field:
        return expr_node->map_field.interval_expression == bie_node;
        break;
    case type_time_field:
        return expr_node->time_field.interval_expression == bie_node;
        break;
    default:
        return true;
    }
...
static unsigned int generate_eval_from_expr(ast_node *bie_node, ast_node *node, expression_input *input, unsigned int input_position) {
    ...
    case type_boolean_literal:
        ...
        break;
    case type_unary_expr:
        added = generate_eval_from_expr(bie_node, node->unary_expr.operand, input, input_position);
    ...
    case type_binary_expr:
        ...
        if (node->binary_expr.operator == AND || node->binary_expr.operator == OR) {
            ...
            if (!belongs_in_bie(bie_node, node->binary_expr.left)) {
                ...
                added = generate_eval_from_expr(bie_node, node->binary_expr.left, input, input_position);
            }

            // if the right side doesn't reference the BIE
            if (!belongs_in_bie(bie_node, node->binary_expr.right)) {
                ...
                added += generate_eval_from_expr(bie_node, node->binary_expr.right, input, input_position + added);
            }
        } else {
    ...
    case type_map_field:
        ...
        break;
    case type_time_field:
        if (node->time_field.is_time || node->time_field.interval_expression == bie_node) {
...
static unsigned int get_eval_size(ast_node *bie_node, ast_node *node) {
    unsigned int size = 0;
    if (!node) {
        return 0;
    }
    ...
    case type_boolean_literal:
        ...
        break;
    case type_unary_expr:
        size = 1 + get_eval_size(bie_node, node->unary_expr.operand);
        break;
    case type_binary_expr:
        if (node->binary_expr.operator == AND || node->binary_expr.operator == OR) {
            // if the left side doesn't reference the BIE
            if (!belongs_in_bie(bie_node, node->binary_expr.left)) {
                ...
                size = get_eval_size(bie_node, node->binary_expr.left);
            }
            ...
            if (!belongs_in_bie(bie_node, node->binary_expr.right)) {
                ...
                size += get_eval_size(bie_node, node->binary_expr.right);
            }
        } else {
            // not a Boolean binary expression
            size = get_eval_size(bie_node, node->binary_expr.left);
...
static expression_input * generate_eval_from_map_field(ast_node *bie_node, map_value *value, side_enum side) {
    ...
    if (value->type == string_type) {
        ...
        expr[2].string_value = value->string_value;
    } else if (value->type == pointer_type) {
        node = (ast_node *)value->pointer_value;
        size = 1 + get_eval_size(bie_node, node);
        initialize_expression_input(&expr, size);
        expr[0].length = size;
        generate_eval_from_expr(bie_node, node, expr, 1);
    } else if (value->type == boolean_type) {
        initialize_expression_input(&expr, 3);
        expr[0].length = 3;
        expr[1].action = param_boollit;
        expr[2].boolean_value = value->boolean_value;
    }
...
static void generate_evals_from_time_maps(data_map *map, word_id left_begin, word_id left_end, word_id right_begin, word_id right_end) {
    ...
    if (left_begin != WORD_NOT_FOUND) {
        ...
        value.pointer_value = expression;
...
static nfer_rule * generate_each_rule(ast_node *node, nfer_specification *spec, word_id result, ast_node *where_node) {
    ...
    case type_binary_interval_expr:
        ...
        if (where_node) {
            ...
            if (belongs_in_bie(node, where_node)) {
        ...
        while (has_next_map_key(&mit)) {
            ...
            expression.pointer_value = generate_eval_from_map_field(node, &value, left_side);
        ...
        while (has_next_map_key(&mit)) {
            ...
            expression.type = pointer_type;
            expression.pointer_value = generate_eval_from_map_field(node, &value, right_side);
            map_set(&rule->map_expressions, key, &expression);
        }
...
static void generate_eval_from_map_expr_list(ast_node *bie_node, ast_node *map_expr_list, data_map *map) {
    expression_input *expression;
    map_value value;
    int size;
    if (!map_expr_list) {
    ...
    size = 1 + get_eval_size(bie_node, map_expr_list->map_expr_list.map_expr);
    initialize_expression_input(&expression, size);
    expression[0].length = size;
    generate_eval_from_expr(bie_node, map_expr_list->map_expr_list.map_expr, expression, 1);
    value.type = pointer_type;
    value.pointer_value = expression;
    map_set(map, map_expr_list->map_expr_list.resulting_map_key, &value);

    generate_eval_from_map_expr_list(bie_node, map_expr_list->map_expr_list.tail, map);
}
