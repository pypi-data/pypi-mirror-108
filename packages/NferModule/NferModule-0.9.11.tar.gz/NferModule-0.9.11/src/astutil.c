...
static word_id write_nodes(FILE *dotfile, ast_node *node, dictionary *parser_dict, dictionary *dot_dict) {
    ...
    case type_binary_expr:
        ...
        if (node->binary_expr.belongs_in != NULL) {
            fprintf(dotfile, "%s -> %s%p [style=dotted];\n", get_word(dot_dict, node_id), "BinaryIntervalExpr", node->binary_expr.belongs_in);
        }
        return node_id;
        break;
    case type_map_field:
        ...
        if (node->map_field.interval_expression != NULL) {
            ...
            fprintf(dotfile, "%s -> %s%p", get_word(dot_dict, node_id), is_bie ? "BinaryIntervalExpr" : "AtomicIntervalExpr", node->map_field.interval_expression);
            if (is_bie) {
    ...
    case type_time_field:
        ...
        if (node->time_field.interval_expression != NULL) {
            ...
            fprintf(dotfile, "%s -> %s%p", get_word(dot_dict, node_id), is_bie ? "BinaryIntervalExpr" : "AtomicIntervalExpr", node->time_field.interval_expression);
            if (is_bie) {
