...
void evaluate_expression(expression_input *input, typed_value *result, data_stack *stack,
    ...
    map_key key;
    stack_value value, left_value, right_value;
    ...
    while (position < length) {
        ...
        case action_add:
            ...
            if (left_value.type == real_type) {
                ...
                if (right_value.type == real_type) {
                    value.real_value = left_value.real_value + right_value.real_value;
                } else if (right_value.type == integer_type) {
                    value.real_value = left_value.real_value + right_value.integer_value;
                } else {
                ...
                if (right_value.type == real_type) {
                    value.type = real_type;
                    value.real_value = left_value.integer_value + right_value.real_value;
                } else if (right_value.type == integer_type) {
                    value.type = integer_type;
                    value.integer_value = left_value.integer_value + right_value.integer_value;
                } else {
                    filter_log_msg(LOG_LEVEL_ERROR, "Type error in expression evaluation: non-numeric value on right side of add\n");
                }
            } else {
        ...
        case action_div:
            ...
            if (left_value.type == real_type) {
                ...
                if (right_value.type == real_type) {
                    ...
                    if (right_value.integer_value != 0) {
                        value.real_value = left_value.real_value / right_value.integer_value;
                    } else {
                        filter_log_msg(LOG_LEVEL_WARN, "Divide by zero error in expression evaluation\n");
                        value.real_value = 0.0;
        ...
        case action_lt:
            ...
            if (left_value.type == real_type) {
                if (right_value.type == real_type) {
                    value.boolean_value = left_value.real_value < right_value.real_value;
                } else if (right_value.type == integer_type) {
                    value.boolean_value = left_value.real_value < right_value.integer_value;
                } else {
                    filter_log_msg(LOG_LEVEL_ERROR, "Type error in expression evaluation: non-numeric value on right side of <\n");
                }
            } else if (left_value.type == integer_type) {
                if (right_value.type == real_type) {
                    value.boolean_value = left_value.integer_value < right_value.real_value;
                } else if (right_value.type == integer_type) {
                    value.boolean_value = left_value.integer_value < right_value.integer_value;
                } else {
                    filter_log_msg(LOG_LEVEL_ERROR, "Type error in expression evaluation: non-numeric value on right side of <\n");
                }
            } else {
        ...
        case action_eq:
            ...
            if (left_value.type == real_type) {
                ...
                if (right_value.type == string_type) {
                    value.boolean_value = left_value.string_value == right_value.string_value;
                } else {
        ...
        case action_neg:
            ...
            if (left_value.type == integer_type) {
                value.type = integer_type;
                value.integer_value = -left_value.integer_value;
            } else if (left_value.type == real_type) {
                value.type = real_type;
                value.real_value = -left_value.real_value;
            } else {
        ...
        case action_not:
            ...
            if (left_value.type == boolean_type) {
                value.boolean_value = !left_value.boolean_value;
            } else {
        ...
        case param_boollit:
            value.type = boolean_type;
            value.boolean_value = input[position++].boolean_value;
            push(stack, &value);
            break;
        case param_intlit:
            value.type = integer_type;
            value.integer_value = input[position++].integer_value;
            push(stack, &value);
            break;
        case param_reallit:
            value.type = real_type;
            value.real_value = input[position++].real_value;
            push(stack, &value);
            break;
        case param_strlit:
            value.type = string_type;
            value.string_value = input[position++].string_value;
            push(stack, &value);
            break;
        case param_left_field:
            key = input[position++].field_name;
            map_get(left_map, key, &value);
        ...
        case param_left_begin:
            value.type = integer_type;
            value.integer_value = left_begin;
            push(stack, &value);
            break;
        case param_left_end:
            value.type = integer_type;
            value.integer_value = left_end;
            push(stack, &value);
            break;
        case param_right_begin:
            value.type = integer_type;
            value.integer_value = right_begin;
            push(stack, &value);
            break;
        case param_right_end:
            value.type = integer_type;
            value.integer_value = right_end;
            push(stack, &value);
            break;
        }
