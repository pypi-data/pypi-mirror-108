...
class BaseAgent:
    ...
    @classmethod
    def __get_console_handler(cls, logging_formatter=None):
        """
...
class BaseCliAgent(BaseAgent):
    def __init__(
        ...
        self._argument_parser = self.__build_argument_parser(description=description)
    ...
    def _init(self):
        ...
        self._arguments = self._argument_parser.parse_args()
    ...
    def start(
        ...
        super().start(
            ...
            logging_level=logging_level or cast.string_to_enum(
                self._arguments.logging_level_literal,
                LoggingLevelLiteral))


class BaseCliRdbmsAgent(BaseCliAgent):
    ...
    def _init(self):
        ...
        if self.__rdbms_hostname is None:
            self.__rdbms_hostname = self._arguments.rdbms_hostname

        if self.__rdbms_port is None:
            self.__rdbms_port = self._arguments.rdbms_port

        if self.__rdbms_database_name is None:
            self.__rdbms_database_name = self._arguments.rdbms_database_name

        if self.__rdbms_username is None:
            self.__rdbms_username = self._arguments.rdbms_username
