...
def make_roc_figure(signal, background, **kwargs):
    ...
    for sig, bkg in zip(signal, background):
        roc = deepcopy(sig)
        roc.binedges = np.append(sig.heights[::-1], np.min(sig.heights-1.0)) # the uppermost bin edge doesn't matter at all
        roc.set_heights(bkg.heights[::-1])
        # print(roc.plot_attributes)
        rocs.append(roc)
    panel = heppy.panel(scatters=rocs, xlabel='Signal efficiency', ylabel='Background efficiency')
    return make_figure(panel, **kwargs)
