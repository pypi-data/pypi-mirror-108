...
# %% [markdown]
# # Applied: phaseMS(70.5,  -1569.8, 16004.4, 0.00)
#
...
# %%
D1ph.axis1.mztoi(800), D1md.axis1.highmass
...
# %%
# find peaks in high mass zone (easier)
D1md.pp(autothresh=100,  zoom=(D1md.axis1.highmass, D1md.axis1.highmass/3))
pks = D1md.peaks
# the largest one
pks.largest()
print(pks[0].report())
...
extend = D1.size1//30
center = pks[0]
zone = [int(center.pos-extend), int(center.pos+extend)]
zone
#D1.get_buffer()[zone[0]:zone[1]]
...
xp = autopoints(D1md, 32000)
D1.set_unit("points").display()
plt.scatter(xp, D1.buffer[xp], marker='o', c='r')

# %%
D1bc = D1.copy().bcorr(method='spline', xpoints=xp, nsmooth=0) #.display(label='BC', new_fig=False)

# %%
D1bc.set_unit('m/z').display(title=FC.selected_path, label='phase-sensitive')
D1md.set_unit('m/z').display(new_fig=False, label='modulus')
#plt.scatter(D1bc.axis1.itomz(xp), D1bc.buffer[xp], marker='.', c='r')

# %%
D1bc.axis1.mztoi(664.5)

# %%
D1bc.set_unit('points').display(title=FC.selected_path, label='phase-sensitive')
#D1md.set_unit('points').display(new_fig=False, label='modulus')
plt.scatter(xp, D1bc.buffer[xp], marker='.', c='r')

# %%
import numpy as np
b = D1.get_buffer()[int(round(D1.axis1.mztoi(693))):int(round(D1.axis1.mztoi(657)))]
dd = spike.FTICR.FTICRData(buffer=b)
dd.display()

# %%
dd.bcorr(xpoints=200)
dd.display()
...
for i in range(1):
    ...
    bb = b-mbb
    sbb = b.std()
    bo = np.where(abs(bb) < tresh*sbb, b, mbb )
ddo = spike.FTICR.FTICRData(buffer=bo).display(new_fig=False)
ddsg = ddo.sg(51,5).display(new_fig=False)
#dd -= ddsg
#dd.display()
