...
class SpanContext:
    ...
    def is_span_finished(self):
        if self.span.status == SpanStatus.ABORTED.name or self.span.status == SpanStatus.FINISHED.name or self.span.status == SpanStatus.FAILED.name:
    ...
    def failed(self, context_data: dict = None):
        ...
        if self.status == SpanContextStatus.FAILED:
            logger.info('Can not abort span that is aborted failed.')
            return
        span_event = SpanEvent()
        event_res = span_event.convert(
