...
def _get_tqsim_stat(trade_log={}, quotes={}):
    ...
    account_df['daily_yield'] = account_df['balance'] / account_df['pre_balance'] - 1  # 每日收益
    mean = account_df['daily_yield'].mean()
    stddev = _tq_pstdev(account_df['daily_yield'], mu=mean)
    tqsdk_stat["sharpe_ratio"] = TRADING_DAYS_OF_YEAR ** (1 / 2) * (mean - RISK_FREE) / stddev if stddev else float("inf")  # 年化夏普率
    left_daily_yield = account_df.loc[account_df['daily_yield'] < RISK_FREE, ['daily_yield']]
    stddev = _tq_pstdev(left_daily_yield['daily_yield'], mu=RISK_FREE) if left_daily_yield.shape[0] > 0 else 0
    tqsdk_stat['sortino_ratio'] = (TRADING_DAYS_OF_YEAR * left_daily_yield.shape[0] / account_df.shape[0]) ** (1 / 2) \
                                  * (mean - RISK_FREE) / stddev if stddev else float("inf")  # 年化索提诺比率
    tqsdk_punchlines = [
        '幸好是模拟账户，不然你就亏完啦',
