...
def vwap_table(api: TqApi, symbol: str, target_pos: int, duration: float,
    ...
    Example1::
        ...
        # 设置 vwap 任务参数
        time_table = vwap_table(api, "CZCE.MA105", -100, 600)  # 目标持仓 -100 手，600s 内完成
        print(time_table.to_string())
...
def _gen_random_list(sum_val: int, length: int, min_val: int, max_val: int):
    ...
    assert result_rest_value <= result_rest_position
    result_rest_list = [1 for _ in range(result_rest_value)] + [0 for _ in range(result_rest_position - result_rest_value)]
    utils.RD.shuffle(result_rest_list)
    for i in range(length):
        start = (max_val - min_val) * i
        end = (max_val - min_val) * (i + 1)
