...
def download_funremindxls_form_drive():
    ...
    fr = pyocs_confluence.PyocsConfluence()
    map_list = fr.get_osm_distribute_mapping_info_by_user(funremindxls_name)
...
def get_function_remind_info():
    ...
    for i in range(len(function_remind_list)):
        ...
        if (function_remind_list[i]['审核人'] != ''):
            auditor_name = function_remind_list[i]['审核人'].strip()
        if (function_remind_list[i]['审核结果'] != ''):
