...
from ..base.uadevice import (UaDevice, GROUP, StatInterface, CfgInterface, RpcInterface, 
     CtrlConfig, DeviceConfig, Recorder, BaseStatModel)
...
class ShutterCtrlConfig(CtrlConfig):
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
class ShutterConfig(DeviceConfig):
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Data Structure (redefine the ctrl_config)
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ctrl_config : ShutterCtrlConfig = ShutterCtrlConfig() 
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
class ShutterStatInterface(StatInterface):
    ...
    def is_closed(self, substate):
        return substate in [self.SUBSTATE.OP_CLOSED, self.NOT_OP_READY_CLOSED]
    
@buildproperty(UaNode.prop, 'vtype') 
class ShutterCfgInterface(CfgInterface):
    # we can define the type to parse value directly on the class by annotation
    timeout: UInt32  


class ShutterRpcInterface(RpcInterface):
    RPC_ERROR = RPC_ERROR
...
class Shutter(UaDevice):
    ...
    stat = ShutterStatInterface.prop('stat')    
    cfg  = ShutterCfgInterface.prop('cfg')
    rpc  = ShutterRpcInterface.prop('rpc')
...
class ShutterCfgModel(BaseModel):
...
class ShutterStatModel(BaseStatModel):    
...
class ShutterModel(BaseModel):        
    StatModel = ShutterStatModel
    CfgModel = ShutterCfgModel
    
    cfg: ShutterCfgModel = ShutterCfgModel()
    stat: ShutterStatModel = ShutterStatModel() 
