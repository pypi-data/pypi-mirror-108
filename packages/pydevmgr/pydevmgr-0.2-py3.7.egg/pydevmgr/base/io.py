...
def find_map(dev_type, ioconfig=ioconfig):
    ...
    if not pkg_resources.resource_exists(pkg_name, os.path.join(ioconfig.resources, resource)):
...
def explore_config(what=None, ioconfig=ioconfig):
    """ Iterator on all config files found inside the $RESPATH environment variable 
