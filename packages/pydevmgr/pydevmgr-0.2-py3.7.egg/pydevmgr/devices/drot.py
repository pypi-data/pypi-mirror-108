from pydevmgr_core import buildproperty, NodeVar
...
from ..base.uadevice import UaDevice, GROUP, StatInterface, CfgInterface, RpcInterface, Recorder, BaseStatModel

from enum import Enum

from . import trk
from .motor import Motor, MotorCfgInterface, MotorCfgModel, MotorStatModel
...
class DrotCfgInterface(MotorCfgInterface):
    focus_sign : Int32
    dir_sign   : Int32
    trk_period : Int32

@buildproperty(UaRpc.prop, 'atypes')
class DrotRpcInterface(RpcInterface):    
    RPC_ERROR = RPC_ERROR
    ##
...
class Drot(Motor,trk.Trk):
    ...
    stat = DrotStatInterface.prop('stat')    
    cfg  = DrotCfgInterface.prop('cfg')
    rpc  = DrotRpcInterface.prop('rpc')
    ...
    def start_track(self, mode, angle=0.0) -> UaNode:
        """ Start drot tracking 
    ...
    def move_abs(self, pos, vel=None) -> UaNode:
    ...
    def move_rel(self, pos, vel=None) -> UaNode:
...
Recorder.new_device_type('Drot', Drot)
...
class DrotCfgModel(MotorCfgModel):
...
class DrotStatModel(MotorStatModel, trk.TrkStatModel):
...
class DrotModel(BaseModel):
    StatModel = DrotStatModel
    CfgModel = DrotCfgModel
        
    cfg: DrotCfgModel = DrotCfgModel()
    stat: DrotStatModel = DrotStatModel()
