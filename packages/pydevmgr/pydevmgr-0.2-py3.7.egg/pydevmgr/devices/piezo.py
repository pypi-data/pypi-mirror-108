...
class PiezoStatInterface(StatInterface):
...
class PiezoCfgInterface(CfgInterface):
...
class PiezoRpcInterface(RpcInterface):    
    RPC_ERROR = RPC_ERROR
    ##
...
#
class Piezo(UaDevice):
    SUBSTATE = SUBSTATE
    ERROR = ERROR
    
    stat = PiezoStatInterface.prop('stat')    
    cfg  = PiezoCfgInterface.prop('cfg')
    rpc  = PiezoRpcInterface.prop('rpc')
...
Recorder.new_device_type('Piezo', Piezo)
...
class PiezoStatModel(BaseStatModel):    
...
class PiezoModel(BaseModel):
    StatModel = PiezoStatModel
    CfgModel = PiezoCfgModel
        
    cfg: PiezoCfgModel = PiezoCfgModel()
    stat: PiezoStatModel = PiezoStatModel() 
