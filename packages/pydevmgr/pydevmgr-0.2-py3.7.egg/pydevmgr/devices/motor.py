...
from ..base.uadevice import (UaDevice, GROUP, StatInterface, CfgInterface, RpcInterface, 
    DeviceConfig, CtrlConfig, Recorder, BaseStatModel)
from ..base.tools import _inc, enum_group, enum_txt, EnumTool
from ..base.uacom import Int32
...
class InitialisationConfig(BaseModel):
    """ Data Model for the initialisation sequence """
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Data Structure 
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    sequence : List[str] = []
...
class MotorCtrlConfig(CtrlConfig):
    """ Data Model for the Motor Ctrl Configuration"""
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Data Structure 
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    def validate_axis_type(cls, ax):
        ...
        if isinstance(ax, int):
            if ax<1 or ax>3:
                raise ValueError('axis type number must be >=1 <=3')
        return ax
# ################################
...
class MotorStatInterface(StatInterface): 
    ERROR = ERROR
    SUBSTATE = SUBSTATE
    ...
    @NodeAlias.prop("pos_name", ["pos_actual"])
...
# the decorator is converting annotations to UaNode.property, only vtype is defined in annotation
@buildproperty(UaNode.prop, 'vtype')    
class MotorCfgInterface(CfgInterface):
...
# the decorator is converting annotations to UaRpc property, only atypes are defined 
@buildproperty(UaRpc.prop, 'atypes')   
class MotorRpcInterface(RpcInterface):
    RPC_ERROR = RPC_ERROR
    ##
...
class Motor(UaDevice):
    ...
    def get_configuration(self, **kwargs) -> Dict[UaNode,Any]:
        ...
        cfg_dict = {self.cfg.get_node(k):v for k,v in  ctrl_config.dict().items() }
    ...
    def move_abs(self, absPos, vel=None) -> UaNode:
    ...
    def move_name(self, name, vel=None) -> UaNode:
    ...
    def move_rel(self, relPos, vel=None) -> UaNode:
    ...
    def is_near(self, pos: float, tol: float, data: Dict[str,Any] =None) -> bool:
        """ -> True when abs(pos_actual-pos)<tol """
        apos = self.stat.pos_actual.get(data) 
        return abs(apos-pos)<tol
    
Recorder.new_device_type('Motor', Motor)
...
class MotorStatModel(BaseStatModel):
...
class MotorModel(BaseModel):
    StatModel = MotorStatModel
    CfgModel  = MotorCfgModel
            
    stat: MotorStatModel = MotorStatModel()
    cfg: MotorCfgModel = MotorCfgModel()    
