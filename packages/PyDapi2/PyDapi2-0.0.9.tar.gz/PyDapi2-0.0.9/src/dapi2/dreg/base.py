...
from dapi2.common import DApiException
...
class BaseRegElement(object):
    ...
    :param addr: Element address in registers array.
    :type addr: int

    :param size: Element size, number of registers slot in group (optional, default: 1).
    :type size: int

    :param descr: Element description (otional).
    :type descr: str
    
    :param shortname: Element short name (otional).
    :type shortname: str    '''
    
    def __init__(self, name, parent=None, addr=None, size=1, descr=None, shortname=None):
        '''Initialize'''
...
#         self.log.debug('Initialize')
    ...
    def __str__(self):
        try:
            return '{0:s} {1:!s} (size:{2:d})'.format(self.name, self._stringData())
        except:
            return self.name
    ...
    def toStringChildren(self, indent=0, prefix='', end='', depth=0):
    ...
    def toString(self, indent=0, prefix='', end='', depth=0):
        ...
        try:
            sChildren = ''
            sData = self._stringData()
            if sData != '':
                sData = " : " + str(sData)
            if depth > 0:
                sChildren = self.toStringChildren(indent+1, depth=depth-1)
            if sChildren != '':
                sChildren = '\n'+str(sChildren)
