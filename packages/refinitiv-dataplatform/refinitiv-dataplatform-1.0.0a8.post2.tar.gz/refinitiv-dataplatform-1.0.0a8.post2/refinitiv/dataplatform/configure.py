...
def _read_config_file(path):
    ...
    #   skip reading config file
        logging.getLogger().debug(f'WARNING!!! SKIPPING reading config file at {path} because the config file does not exist.')
        return {}
    ...
    try:
        ...
        logging.getLogger().error(f'ERROR!!! The config file at {path} has error while reading the config file.\n'
                        + f'{e!r}' )
        sys.exit(-1)
...
_config_files_paths = [c for c in [
    ...
    _get_filepath(rootdir=os.path.dirname(__file__), filename=_config_filename_template % "default"),  # DEFAULT_CONFIG_FILE
    ] if c]
