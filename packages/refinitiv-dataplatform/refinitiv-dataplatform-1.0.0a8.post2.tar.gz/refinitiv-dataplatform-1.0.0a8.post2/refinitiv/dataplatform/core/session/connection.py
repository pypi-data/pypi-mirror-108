...
class RefinitivDataConnection(PlatformConnection):
    ...
    async def get_stream_connection_configuration(self, stream_connection_name:str):
        ...
        #       WebSocket url
        streaming_connection_endpoint_websocket_url = self._session.get_streaming_websocket_endpoint_url(stream_connection_name)
        #       discovery endpoint
        ...
        #   valid stream discovery endpoint url
            self._session.debug(f'using discovery endpoint url : {streaming_connection_discovery_endpoint_url}')
            ...
            stream_service_information = await service_discovery_handler.get_stream_service_information()
    ...
    def _initialize_authentication_handler_thread(self):
        self._authentication_token_handler_thread = AuthenticationTokenHandlerThread(self._session, 
        ...
        while True:
            ...
            #   error occur when request an authorization 
                ...
                #   do nothing because the server-mode is disabled
                    self._session.error(f'ERROR!!! failed to request an authorization to platfrom.\n' 
                                            f'Unexpected error {sys.exc_info()[0]}.\n'
                                            '(auto-reconnection is disbled)')

                    raise PlatformSessionError(-1, 'ERROR!!! Authentication handler failed to request a access token.\n'
                                                    f'{sys.exc_info()[0]}.')
                    
                else:
                #   retry again
                    self._session.warning('WARNING!!! retring after failed to request an authorization to platfrom.\n'
                                            f'Unexpected error {sys.exc_info()[0]}.\n'
            ...
            if is_authorize_failed or self._authentication_token_handler_thread.is_error():
            #   failed to request an authorized in the thread loop
                if not self._session.server_mode:
                #   do nothing because the server-mode is disabled
                    self._session.error('ERROR!!! CANNOT authorize to RDP authentication endpoint.\n'
                    ...
                    self._session.warning('WARNING!!! retring after authentication token thread failed.\n'
                                            f'Unexpected error {self._authentication_token_handler_thread.last_exception}.')
                    ...
                    #   possible the authentication token handler thread cannot stop properly
                        self._session.warning('WARNING!!! CANNOT properly stop authentication token handler thread .\n'
                                            f'Unexpected error {sys.exc_info()[0]}.')
            ...
            #   successfully request an authorization
                self._session.debug('Successfully request an authorization by authentication token handler thread.')
                break
...
class DeployedConnection(PlatformConnection):
    ...
    async def get_stream_connection_configuration(self, stream_connection_name:str):
        ...
        #   read from config file.
            ...
            self._session.debug(f'using the Refinitiv realtime url at {realtime_distribution_system_url}')
            ...
            self._session.debug(f'      realtime_distribution_system scheme   = {realtime_distribution_system_url_parse.scheme}')
            self._session.debug(f'      realtime_distribution_system endpoint = {realtime_distribution_system_url_parse.hostname}')
            self._session.debug(f'      realtime_distribution_system port     = {realtime_distribution_system_url_parse.port}')
        ...
        #   use the hostname and port parameter from session
            deployed_platform_hostname, deployed_platform_port = self._session._deployed_platform_host.split(':')

            #   build the StreamServiceInformation
            stream_service_information = StreamServiceInformation(scheme='ws', 
    ...
    async def waiting_for_stream_ready(self, open_state):
        ...
        await self._session.wait_for_streaming('streaming/pricing/main', 'OMM') and open_state()
...
class RefinitivDataAndDeployedConnection(DeployedConnection, RefinitivDataConnection):
    ...
    async def get_stream_connection_configuration(self, stream_connection_name:str):
        ...
        #   using the realtime distribution system
            return await DeployedConnection.get_stream_connection_configuration(self, stream_connection_name)
        else:
        #   using the platform streaming
            return await RefinitivDataConnection.get_stream_connection_configuration(self, stream_connection_name)
