...
from .connection import DeployedConnection, RefinitivDataAndDeployedConnection, RefinitivDataConnection
...
class PlatformSession(Session):
    ...
    def __init__(
        ...
        if grant is not None and self._deployed_platform_host:
            ...
            self.debug('connecting to Refinitiv Data Platform (RDP) and Deployed Platform.......')
    ...
    def stream_auto_reconnection(self):
        return configure.config.get_bool(configure.keys.platform_auto_reconnect(self._session_name))

    @property
    def server_mode(self):
        return configure.config.get_bool(configure.keys.platform_server_mode(self._session_name))
    ...
    def authentication_token_endpoint_url(self):
        #   authentication url
        authentication_path = configure.config.get_str(configure.keys.platform_auth_uri(self._session_name))
        #   token path
        token_path = configure.config.get_str(configure.keys.platform_token_uri(self._session_name))
        
        #   build authentication token url
        authentication_token_endpoint_url = urljoin(urljoin(self._get_rdp_url_root(), authentication_path), token_path)
        self.debug(f'authentication_token_endpoint_url : {authentication_token_endpoint_url}')
        
        #   done
        return authentication_token_endpoint_url

    def _get_auth_token_uri(self):
        base_auth_uri = configure.config.get_str(configure.keys.platform_auth_uri(self._session_name))
        token_uri = configure.config.get_str(configure.keys.platform_token_uri(self._session_name))
    ...
    def _initialize_realtime_distribution_system_config(self):
        ...
        #   user doesn't specific the deployed_platform_host, so check realtime distribution system in config file as the deployed platform hostname
            ...
            try:
                platform_realtime_distribution_system_url = configure.get_str(platform_realtime_distribution_system_url_key)
            except KeyError:
            #   the realtime distribution system doesn't specify in the config file
            #       so, do nothing
                self.debug(f'WARNING!!! the Refinitiv realtime distribution system doesn\'t specify.')
                return

            self.debug(f'using the Refinitiv realtime distribution system : url at {platform_realtime_distribution_system_url}')

            #   construct host for host name and port
            self._deployed_platform_host = urllib.parse.urlparse(platform_realtime_distribution_system_url).netloc
            self.debug(f'      deployed_platform_host = {self._deployed_platform_host}')

        else:
        #   using the deployed platform host from user specific
            self.debug(f'using the specific deployed platform host : host {self._deployed_platform_host}')
    ...
    async def _get_stream_connection_configuration(self, stream_connection_name:str):
        ...
        """
        return await self._connection.get_stream_connection_configuration(stream_connection_name)
    ...
    async def http_request_async(self, url: str, method=None, headers=None,
        ...
        return await self._connection.http_request_async(url, method=method, headers=headers,
