...
class AuthenticationTokenHandlerThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            while not self._stop_event.is_set():
                assert self._authentication_token_information is not None \
                        and self._token_expires_in_secs is not None \
                        and self._token_requested_time is not None, 'ERROR!!! Something is wrong in the _request_refresh_token function.'
                ...
                if debug_refresh_token_printing_time < now:
                    ...
                    debug_refresh_token_printing_time = now + self._token_expires_in_secs / 6.5
        ...
        #   get an exception from authentication process, so stop
            self._session.error(f'ERROR!!! authentication handler raise an exception.\n{e!r}')
    ...
    def authorize(self):
        ...
        assert not self.is_error(), f'ERROR!!! AuthenticationTokenHandlerThread has an error.\n{self._last_exception}'
        ...
        #   thread doesn't start yet, so start it
            assert not self.is_alive(), 'ERROR!!! authentication thread has been started.'
    ...
    def _update_authentication_access_token(self, access_token):
        ...
        self._loop.call_soon_threadsafe(self._session.set_stream_authentication_token(access_token))
    ...
    def _request_refresh_token(self):
        ...
        while True:
            ...
            #   somthing wrong when request the refresh token
                self._session.error(f'ERROR!!! something wrong while requesting a refresh token.\n   {e}')
        ...
        #   failed
            ...
            #   server mode is enable, retry by request token by password if it cannot refresh token.
                ...
                while True:
                    ...
                    assert isinstance(self._grant, GrantPassword), 'it is not a GrantPassword, so we cannot re-authorize with authentication server.'
                    try:
                    ...
                    #   somthing wrong when request the refresh token
                        self._session.error(f'ERROR!!! something wrong while requesting a new token by username/password.\n   {e}')
                        self._session.debug(f'          try request token again with username/password again.')
                    ...
                    #   server error code, so retry with password
                        assert 500 <= response.status_code < 600, 'Received the server error after request a new authorization to authentication server.'
        ...
        assert refreshed_token_information is not None, 'ERROR!!! Something is wrong in the above _request_refresh_token function.'
    ...
    def _request_token_by_refresh_token(self, client_id:str,
        ...
        #   build request header
        assert self._authentication_token_information.access_token is not None, 'AuthenticationTokenHandlerThread._request_token_by_refresh_token() does not have a refresh token.'
        request_header = {'Content-Type' : 'application/x-www-form-urlencoded',
                            'Accept' : 'application/json',
                        }
    ...
    def __request_token(self, request_header, request_data):
        ...
        assert response is not None, 'AuthenticationTokenHandlerThread.__request_token() got a None response.'
        self._session.debug(f'HTTP response {response.status_code}: {response.text}')

        #   extract header and check the content type
        headers = response.headers
        if 'Content-Type' in headers and u'/json' in headers.get('Content-Type'):
            #   parse the response   
    ...
    def _parse_request_token_response(self, response):
        ...
        #   successfully request a new token
            ...
            assert 'access_token' in response_data, 'AuthenticationTokenHandlerThread._parse_request_token_response() "access_token" not in response'
            assert 'expires_in' in response_data, 'AuthenticationTokenHandlerThread._parse_request_token_response() "expires_in" not in response'
            assert 'scope' in response_data, 'AuthenticationTokenHandlerThread._parse_request_token_response() "scope" not in response'
            assert 'token_type' in response_data, 'AuthenticationTokenHandlerThread._parse_request_token_response() "token_type" not in response'
