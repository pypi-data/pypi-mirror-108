...
class StreamingChain(Openable, LogReporter):
    ...
    async def _do_open_async(self, with_updates=True):
        ...
        self._session.info(f'StreamingChain :: Processing decode chain record {self._name}.')
        ...
        while name:
            ...
            if record:
                ...
                self._session.error(f"StreamingChain :: Stopped to process chain record "
            ...
            self._session.info(f'StreamingChain :: Processing decode next chain record {name}.')
        ...
        self._session.info(f'StreamingChain :: DONE - Processing decode chain record {self._name}.')
    ...
    def _process_chain_record(self, name, message):
        ...
        """
        self._session.info(f'StreamingChain :: Processing response chain record {name}, message = {message}.')
    ...
    def _parse_chain_record(self, name, fields):
        ...
        """
        self._session.debug(f'StreamingChain :: Parsing response field of chain record {name}, field = {fields}.')
        ...
        if ChainRecord17Chars.isValidChainRecord(fields):
            ...
            self._session.error(f'StreamingChain :: Cannot parse chain {name} because it is an invalid chain.')
            #   do nothing, done
            return
        ...
        self._session.debug(f'StreamingChain :: '
    ...
    def _update_chain_record(self, name, update):
        ...
        self._session.info(f'StreamingChain :: Updating response field of chain record {name}, update = {update}.')
        ...
        if not chain_record:
            ...
            #       skip it
            self._session.warning(f'StreamingChain :: Skipping to update an invalid chain record = {name}.')
            return
        ...
        self._session.info(f'StreamingChain :: chain record update info = {chain_record_update_info}.')
    ...
    def _process_remaining_update_messages(self):
        ...
        #   done
        self._session.info('StreamingChain :: DONE - Processing remaining update messages.')
    ...
    def _add_constituent(self, index, constituent):
        ...
        self._session.debug(f'StreamingChain.add_constituent(index = {index}, constituent = {constituent})')
    ...
    def _append_constituent(self, index, constituent):
        ...
        """
        self._session.debug(f'StreamingChain.append_constituent(index = {index}, constituent = {constituent})')
    ...
    def _remove_constituent(self, index, constituent):
        ...
        self._session.debug(f'StreamingChain.remove_constituent(index = {index}, constituent = {constituent})')
        assert 0 <= index < len(self._constituentList)
    ...
    def _update_constituent(self, index, oldConstituent, newConstituent):
        ...
        self._session.debug(f'StreamingChain.update_constituent('
    ...
    def _on_refresh_handler(self, stream, message):
        ...
        self._session.info(f'StreamingChain.on_refresh(stream = {stream.name}, message = {message})')
    ...
    def _on_update_handler(self, stream, update):
        ...
        self._session.info(f'StreamingChain.on_update(stream = {stream.name}, update = {update})')
        ...
        if not self._is_complete_decoded.done():
            ...
            #   do nothing
            self._session.warning('StreamingChain :: waiting to update because chain decode does not completed.')
            return
    ...
    def _on_status_handler(self, stream, status):
        ...
        self._session.info(f'StreamingChain.on_status(stream = {stream.name}, status = {status})')
    ...
    def _on_error_handler(self, stream, error):
        ...
        self._session.info(f'StreamingChain.on_error(stream = {stream.name}, error = {error})')
    ...
    def _dispatch_event(self, callback, *args, **kwargs):
        ...
        try:
            ...
            self._session.error(f'{callback} user function on streaming chain {self._name} raised error {e}')
