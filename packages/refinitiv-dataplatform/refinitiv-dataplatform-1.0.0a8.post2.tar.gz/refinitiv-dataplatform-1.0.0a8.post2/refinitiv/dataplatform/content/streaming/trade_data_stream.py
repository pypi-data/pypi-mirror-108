...
class TradeDataStream(TradeDataStreamCache, Openable):
    ...
    def __init__(self, universe:list, 
        ...
        TradeDataStreamCache.__init__(self, self._session, max_order_summary_cache, max_order_events_cache)
        ...
        self._stream = RDPItemStream(session=self._session,
                                        ...
                                        view=[view.value for view in self._views] if self._views is not None else None, 
    ...
    def _parameters(self):
        ...
        parameters =  {'universeType':self._universe_type.value,
    ...
    def _do_pause(self):
        # for override
        raise NotImplementedError('ERROR!!! Currently, TradeDataStream does not support pause.')

    def _do_resume(self):
        # for override
        raise NotImplementedError('ERROR!!! Currently, TradeDataStream does not support resume.')
    ...
    def get_order_summary(self, universe:list=None,
        ...
        #   do filter by universe and/or order time
            ...
            #   filter by start date/time, add for a roundoff error when compare datetime by a microsecond
                df = df[df.OrderTime >= (start_datetime+timedelta(microseconds=-1)).strftime('%Y-%m-%dT%H:%M:%S.%f')]
    ...
    def get_order_events(self, universe:list=None, 
        ...
        #   do filter by universe
            ...
            if start_datetime:
            #   filter by start date/time, add for a roundoff error when compare datetime by a microsecond
                df = df[df.EventTime >= (start_datetime+timedelta(microseconds=-1)).strftime('%Y-%m-%dT%H:%M:%S.%f')]
    ...
    def _on_complete(self, stream):
        ...
        if self._on_complete_cb:
            ...
            try:
                ...
                self._session.error(f'StreamingTDS _on_complete callback raised exception: {e!r}')
                self._session.debug(f"{traceback.format_exc()}")
