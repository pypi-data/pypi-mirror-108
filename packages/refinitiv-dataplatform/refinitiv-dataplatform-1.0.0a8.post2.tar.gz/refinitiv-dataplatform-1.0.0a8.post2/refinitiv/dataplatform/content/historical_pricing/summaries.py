...
class Definition:
    ...
    def get_data(self, session=None, on_response=None):
        ...
        historical_pricing = HistoricalPricing(session=_session, on_response=on_response)
    ...
    async def get_data_async(self, session=None, on_response=None):
        ...
        if _session is None:
            raise AttributeError('A Session must be started')
        historical_pricing = HistoricalPricing(session=_session, on_response=on_response)
        response = await historical_pricing._get_summaries_async(universe=self.universe,
                                                                 interval=self.interval,
                                                                 start=self.start,
                                                                 end=self.end,
                                                                 adjustments=self.adjustments,
