...
class OMMStreamConnection(StreamConnection):
    ...
    async def __send_and_wait_for_login_or_authentication_message(self, auth_message):
        ...
        self.debug(f'Sending the authentication message. url={self._connection_config.url}, message={auth_message}')
        with self._ws_lock:
    ...
    def _set_stream_authentication_token(self, authentication_token):
        ...
        self.debug(f'OMMStreamConnection.authentication_token() - authentication message = {auth_message}')

        #   send the authentication message
        self.info(f'Send new authentication message to websocket[{self._login_stream_event_id}] - {self._login_stream_event_id}')
    ...
    def _on_messages(self, messages):
        ...
        try:
            ...
            self._session.debug(f'OMMStreamConnection._on_message: JSONDecodeError with message={messages[:50]})')
    ...
    def _process_response_message(self, message):
        ...
        if message_domain == 'Login':
            ...
            self._process_service_response_message(message_id,
                                                   message_type,
                                                   message)
            return
    ...
    def _process_service_response_message(self,
                                          message_id,
                                          message_type,
                                          service_message):
        #   filter by message types
