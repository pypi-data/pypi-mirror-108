...
class StreamingPrices(Openable, _LogReporter):
    ...
    async def _do_open_async(self, with_updates=True):
        ...
        """
        self._session.debug(f'StreamingPrices : open streaming on {self.params.universe}')
    ...
    async def _do_close_async(self):
        self._session.debug(f'StreamingPrices : close streaming on {str(self.params.universe)}')
