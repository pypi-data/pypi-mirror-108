...
class RDPStream(Stream):
    ...
    async def _do_close_async(self, **kwargs):
        ...
        try:
            await asyncio.wait_for(self._wait_for_unsubscribe_response(), 
                                        timeout=self._session.websocket_close_response_timeout_secs, 
                                        loop=self._session._loop)
        except asyncio.TimeoutError:
            self._session.warning('WARNING!!! timeout occurred before received un-subsribed response.')
    ...
    def _get_subscription_request_message(self):
        ...
        universe = self.universe if self.extended_params is None or 'universe' not in self.extended_params else self.extended_params['universe']
        service = self.service if self.extended_params is None or  'service' not in self.extended_params else self.extended_params['service']
        view = self.view if self.extended_params is None or 'view' not in self.extended_params else self.extended_params['view']
    ...
    async def _subscribe_async(self):
        ...
        result = await self._session.wait_for_streaming_reconnection(self.api, self.protocol_name)
        ...
        with self._stream_lock:
            ...
            #   state contains in the ack message
                ...
                #   received an closed stream
                    ...
                    if self._unsubscribe_response_future is not None and not self._unsubscribe_response_future.done():
                        #   this is a unsubscribe ack for this stream, so set the future to be True
                        self._unsubscribe_response_future.set_result(True)
                        
                else:
                #   unsupported state
                    self._session.log(1, f'Stream {self._stream_id} [{self._name}] - Receive unsupported stream state {stream_state}')
    ...
    def _on_alarm(self, message):
        with self._stream_lock:
            if self._state is StreamState.Open:
                self._session.log(1, f'Stream {self.stream_id} [{self.name}] - Receive alarm {message}')
