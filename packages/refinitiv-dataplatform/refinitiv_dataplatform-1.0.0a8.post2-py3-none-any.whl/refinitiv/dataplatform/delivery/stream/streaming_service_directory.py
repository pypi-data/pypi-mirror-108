...
class StreamingServiceDirectoryData:
    ...
    def update(self, service_description):
        if service_description:
            ...
            entries = service_description.get('FilterList').get('Entries') if service_description.get('FilterList') else None
            for entry_description in entries:
                elements = entry_description.get('Elements')
                if elements:
                    self._name = elements.get('Name', self._name)
                    self._support_qos_range = elements.get('SupportsQoSRange', self._support_qos_range)
                    self._qos = elements.get('QoS', self._qos)
                    self._capabilities = elements.get('Capabilities', self._capabilities)
                    ...
                    self._accepting_consumer_status = elements.get('AcceptingConsumerStatus',
                                                                   self._accepting_consumer_status)
                    self._vendor = elements.get('Vendor', self._vendor)
                    self._service_state = elements.get('ServiceState', self._service_state)
...
class StreamingServiceDirectory(OMMStream):
    ...
    def __getitem__(self, index):
        if isinstance(index, str):
            return self._service_map[self._name_to_id[index]] if index in self._name_to_id else None
        else:
            ...
            for entry in entries:
                ...
                if action == ServiceEvent.Add.value:
                    if service_key in self._service_map:
                        ...
                        self._session.log(1, f"Add twice same service {service.name}({service.key}): {entry}")
                        name = entry.get('Elements').get('Name', None) if entry.get('Elements') else None
                        if name != service.name:
                            self._session.log(1, f"But with different service name: {name}")
                    else:
                    ...
                    if service_key in self._service_map:
                        ...
                        self._session.log(1, f"Remove unknown service {service_key}: {entry}")
                    ...
                    if service_key in self._service_map:
                        ...
                        self._session.log(1, f"Update was received for an unknown service {service_key}: {entry}")
                    ...
                    if service_key in self._service_map:
                        ...
                        self._session.log(1, f"Status was received for an unknown service {service_key}: {entry}")
                    ...
                    if service_key in self._service_map:
                        ...
                        self._session.log(1, f"Error was received for an unknown service {service_key}: {entry}")
    ...
    def _notify(self, event_type:ServiceEvent, service:StreamingServiceDirectoryData):
        try:
            ...
            self._session.log(logging.ERROR, f"{event_type} callback raised exception: {e!r}")
