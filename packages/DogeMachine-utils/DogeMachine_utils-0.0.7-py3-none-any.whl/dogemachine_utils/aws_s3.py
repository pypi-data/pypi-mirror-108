...
def get_report_object_key_path(url: str, scan_name: str, tool_name: str, date_string: str, time_string: str, file_extension: str):
    """
    ...
    :param date_string: Like 2012-03-17
    :param time_string: Like "%H%MZ"
    ...
    # 2012-03-17T1748Z
    object_key = f"{url_object_component}/" \
                 f"{scan_name}/" \
                 f"{date_string}/" \
                 f"{tool_name}/" \
...
def save_results_file_to_s3_from_aws_lambda(
    ...
    # TODO: Get a good naming convention going for this. I want to be able to save the results to a file name that I want, I think. And a format of my choice.
    print(contents)
    ...
    def save_results():
        ...
        response = write_simple_object_to_s3(
            content=contents,
            bucket_name=bucket,
            object_key=f"{tool_name}/{target_url}/{get_year_month_day_string()}/{get_current_time_string()}-results.xml",
            s3_client=s3_client
        )
        return response
