...
class ZMQTestMixin(TestCase):
    ...
    def setUp(self):
        ...
        if self.protocol == "tcp://" and not settings.BROKER_SUB.startswith("tcp"):
            settings.override(BROKER_SUB=f"tcp://127.0.0.1:{find_free_port()}",
    ...
    def rpc(self, channel: str, type: str, request, timeout=3, receive_response=True, resource=None) -> Optional[dict]:
        ...
        rpc_request = _TestRPCRequest(name="testclient", type=type, rpc_id=rpc_id,
                                      resource=resource or settings.RESOURCE, request=request)
