...
def create_video(list_comp_data, path_video, job_id):
    ...
    final_vid.close()
    if duration_f > 0 and inoutroComp is not None:
        path_video = merge_intro_outro(path_video, inoutroComp.intro, inoutroComp.outro)
    for comp in arr_comps:
        comp.close()
    return path_video
...
class CompilationComp(Component):
    ...
    def get_audio(self):
        return MusicHelper.create_compilation_songs(self.music_data, self.job_id)
