...
if os.getenv('__INIT_LOG_HANDLER__') is None:
    ...
    _json_configs_file_path = os.path.join(_current_dir, 'configs', 'logger.json')
    if os.path.isfile(_yaml_configs_file_path):
        try:
            with open(_yaml_configs_file_path, 'r') as _yaml_configs_file:
                _configs_dict = yaml.safe_load(_yaml_configs_file)
        except Exception:
            logger.exception(f"Failed to load '{_yaml_configs_file_path}' configs file.")
            exit(2)
    elif os.path.isfile(_json_configs_file_path):
        try:
            with open(_json_configs_file_path, 'r') as _json_configs_file:
                _configs_dict = json.load(_json_configs_file)
        except Exception:
            logger.exception(f"Failed to load '{_json_configs_file_path}' configs file.")
            exit(2)
    ...
    os.environ['__INIT_LOG_HANDLER__'] = str(_log_handler_id)
