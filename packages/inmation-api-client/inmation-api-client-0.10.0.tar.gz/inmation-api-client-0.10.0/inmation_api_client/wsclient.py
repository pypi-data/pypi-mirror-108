...
class WSClient(EventEmitter):
    ...
    async def connect(self, url, options, ssl=None):
        """ Connect to the WebSocket server.
        Args:
            url (str): URL of the WebSocket
            options (object): Options object
            ssl (bool | SSLContext): Use secure websocket (ssl=None: Insecure, ssl=True: Secure, ssl=ssl.SSLContext object: Advanced SSL config)
        """
        ...
        if not (isinstance(ssl, (bool, SSLContext)) or ssl is None):
            raise Error('The <<ssl>> argument must be of type bool, SSLContext or None')
        ...
        try:
            ...
            self._ws = await connect(self.url, close_timeout=options.tim or 10, loop=self.ioloop, max_size=None, ssl=self.ssl)
    ...
    async def reconnect(self):
        ...
        await self.connect(self.url, self.options, self.ssl)
