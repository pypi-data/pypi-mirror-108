...
class BioLib:
    ...
    def push(author_and_app_name, app_path):
        ...
        new_app_version_json = BiolibAppApi.push_app_version(app['public_id'], zip_binary, author, app_name)
    ...
    def start_compute_node(port, host, eif_path=None):
        ...
        webserver.start_webserver(port=port, host=host, specified_biolib_host=BioLib.host,
                                  run_silent=BioLib.log_level > BioLib.TRACE_LOGGING, specified_eif_path=eif_path)
    ...
    def run_local_docker_app(input_files, args, module, source_files_temp_dir):
        ...
        cli_utils.print_module_output(stdout, stderr, exit_code)
...
class BioLibApp:
    ...
    def run_cloud_job(self, job_id, module_input):
        ...
        biolib_logger.debug(f'Cloud: Job created with id {cloud_job["public_id"]}')
    ...
    def __call__(self, args=None, stdin=None, files=None, output_path='biolib_results'):
        try:
            ...
            biolib_logger.error('An error has occured')
            biolib_logger.error(exception.message)
            raise exception
