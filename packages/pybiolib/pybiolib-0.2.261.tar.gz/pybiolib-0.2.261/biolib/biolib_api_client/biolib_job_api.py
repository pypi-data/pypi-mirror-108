...
class BiolibJobApi:
    ...
    @staticmethod
    def create(app_version_id):
        response = requests.post(
            f'{BiolibApiClient.get().base_url}/jobs/',
            json={'app_version_id': app_version_id},
    ...
    def create_cloud_job(module_name, job_id):
        ...
        for retry in range(4):
            try:
                response = requests.post(
                    f'{BiolibApiClient.get().base_url}/jobs/cloud/',
                ...
                if retry > 3:
                    raise BioLibError('Reached retry limit for cloud job creation') from exception
                time.sleep(1)

        if not response:
            raise BioLibError('Could not get response from newly created compute node')
        ...
        for _ in range(max_retry_attempts):
            response = requests.get(
                f'{BiolibApiClient.get().base_url}/jobs/cloud/{cloud_job["public_id"]}/status/',
                auth=BearerAuth(BiolibApiClient.get().access_token)
            )
            cloud_job = response.json()
            if cloud_job.get('is_compute_node_ready', False):
            ...
            biolib_logger.info('Cloud: Reserved compute node not ready, retrying...')
        ...
        for _ in range(status_max_retry_attempts):
            ...
            for status_update in status_json['status_updates']:
                if 'log_message' in status_update:
                    biolib_logger.info(f'{compute_type}: {status_update["log_message"]}')

                if status_update.get('log_message') == status_to_await:
                    status_reached = True

            if 'error_code' in status_json:
                # TODO: Figure out a way to get the error messages
                raise BioLibError(f'{compute_type}: {status_json["error_code"]}')
    ...
    @staticmethod
    def get_enclave_json(biolib_host):
        response = requests.get(
            f'{biolib_host}/info-files/biolib-enclave.json',
        )
        return response.json()
