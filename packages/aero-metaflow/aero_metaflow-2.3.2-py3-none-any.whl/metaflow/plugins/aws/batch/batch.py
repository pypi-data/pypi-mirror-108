...
TRACKING_URL = "https://dev.aeroplatform.co.uk/tracking?vcpus={vcpus}&memory={memory}"
        ...
        def wait_for_slot(pre_job):
            ...
            while job_blocked:
                ...
                if resp.status_code == 200:
                    ...
                    if pre_job.cpu + current_vcpus > cpu_limit:
                        echo(f"New job of {pre_job.cpu} would push CPU usage to {pre_job.cpu + current_vcpus}, which is higher than the limit of {cpu_limit}")
                    else:
                        echo(f"Requesting {pre_job.cpu} vcpus, current usage now {pre_job.cpu + current_vcpus}")
                        job_blocked = False
