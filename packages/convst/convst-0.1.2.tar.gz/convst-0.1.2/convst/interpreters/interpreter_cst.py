...
class interpeter_cst():
    ...
    def _interpret_dt(self, X):
        ...
        X_split = np.zeros((n_nodes, self.X_train_cst.shape[0]), dtype=np.bool_)
        y_split = np.zeros((n_nodes, self.X_train_cst.shape[0]), dtype=np.int8) - 1
        ...
        df['y'] = self.y_train
        pal = {k : sns.color_palette(n_colors=self.n_classes)[k] for k in range(self.n_classes)}
        for node_id in node_index:
            if tree.feature[node_id] == _tree.TREE_UNDEFINED:
                ...
                ax[0].set_xlabel("Shapelet n° {}".format(tree.feature[node_id]))
