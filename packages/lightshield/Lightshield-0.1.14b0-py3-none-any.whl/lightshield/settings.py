...
class JsonConfig:  # pragma: no cover
    ...
    def get(self, key, default=None):
        ...
        if value:
            self.logging.debug("Got %s from environment." % key)
            return_val = value
        elif key in self._config.keys():
            self.logging.debug("Got %s from config file." % key)
            return_val = self._config[key]
        else:
            return_val = default
        self.logging.debug(return_val)
        return return_val
...
# Dependent services
# Redis
REDIS_HOST = CONFIG.get("BUFFER_HOST", "localhost")
REDIS_PORT = CONFIG.get("BUFFER_PORT", 6379)
