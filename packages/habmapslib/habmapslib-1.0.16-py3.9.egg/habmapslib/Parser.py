        ...
        for el in definitions:
            if el != '':
                if el == '$time':
                    ...
                    # Validamos si el id del hab cumple con los filtros
                    tmpid = trace[i]
                    for validid in self.ch.config['frame']['habidfilter']:
                        logging.debug('Validando id ' + tmpid + ' con ' + validid)
                        rres = re.search(validid, tmpid)
                        if rres:
                            logging.debug('El id ' + tmpid + ' es válido para enviar')
                            hm.setHabId(tmpid)
                            isHabIdValid = True
                            break
                        else:
                            logging.debug('El id ' + tmpid + ' no es válido para ' + validid)
                # Parseo de señales
                else:
                    try:
