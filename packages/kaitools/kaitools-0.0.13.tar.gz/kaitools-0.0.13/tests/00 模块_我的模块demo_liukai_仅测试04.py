...
def 文本_取中间文本(str, front, behind):
    ...
    print(re.search(front + '(.*?)' + behind, str, re.S))  # <_sre.SRE_Match object; span=(0, 4), match='1234'>
    # res =re.search(front + '(.*?)' + behind, str, re.S).group() #1234
    print(re.search(front + '(.*?)' + behind, str, re.S).groups())  # 1234  ('4',)
    print(re.search(front + '(.*?)' + behind, str, re.S).group())  # 1234
    # for i in  res:print (i)
    print(re.search(front + '(.*?)' + behind, str, re.S).group(0))  # 1234
    print(re.search(front + '(.*?)' + behind, str, re.S).group(1))  # 23
