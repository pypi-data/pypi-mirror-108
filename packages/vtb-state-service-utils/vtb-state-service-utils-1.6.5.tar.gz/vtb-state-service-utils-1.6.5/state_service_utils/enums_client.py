...
class EnumsClient:
    __slots__ = ('_data', '_url', '_hash_sum', '_timeout', '_cache_file', '_is_mocked')

    def __init__(self, url, timeout, is_mocked=False, cache_file=None):
        self._url = url
        ...
        self.update_from_cache() if is_mocked else self.update_from_remote()
...
def _initialize() -> EnumsClient:
    ...
    return EnumsClient(url, timeout, is_mocked, cache_file)
