...
class TestConfigureListCommand(unittest.TestCase):
    ...
    def test_configure_list_command_nothing_set(self):
        ...
        context.full_config = {
            'profiles': {'default': {'region': 'ALTUS_REGION'}}}
        stream = six.StringIO()
    ...
    def test_configure_from_config_file(self):
        ...
        config_file_vars = {
            'foo': 'bar',
            ALTUS_ACCESS_KEY_ID_KEY_NAME: 'key_id',
            ALTUS_PRIVATE_KEY_KEY_NAME: 'mysecretkey'
        }
        context = FakeContext(
            all_variables={'config_file': '/config/location'},
            config_file_vars=config_file_vars)
        context.context_var_map = {'region': ('region', "ALTUS_REGION")}
        context.full_config = {
            'profiles': {'default': {'region': 'ALTUS_REGION'}}}
        stream = six.StringIO()
        self.configure_list = ConfigureListCommand(stream)
    ...
    def test_configure_from_multiple_sources(self):
        ...
        context.context_var_map = {
            'region': ('region', 'ALTUS_REGION'),
            'profile': ('profile', 'ALTUS_DEFAULT_PROFILE')}
        context.full_config = {
            'profiles': {'default': {'region': 'ALTUS_REGION'}}}
        stream = six.StringIO()
        self.configure_list = ConfigureListCommand(stream)
        ...
        self.assertRegexpMatches(
            rendered, 'profile\\s+myprofilename\\s+env\\s+ALTUS_DEFAULT_PROFILE')
    ...
    def test_profile_set_in_context(self):
        ...
        context.context_var_map = {'region': ('region', "ALTUS_REGION")}
        context.full_config = {
            'profiles': {'dev': {'region': 'ALTUS_REGION'}}}
        stream = six.StringIO()
        self.configure_list = ConfigureListCommand(stream)
        self.configure_list(context, args=[], parsed_globals=None)
        rendered = stream.getvalue()
